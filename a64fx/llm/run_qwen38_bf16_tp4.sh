#!/bin/bash
# Stage and measure Qwen3.8-27B BF16 on exactly four A64FX nodes.
set -euo pipefail
...
export TF_HIER_BARRIER=${TF_HIER_BARRIER:-0} TF_BF16PV_PREFETCH=${TF_BF16PV_PREFETCH:-0}
# The pair-interleaved experimental layout diverges from the row-major token
# stream at token 14.  Keep the source-equivalent layout as the safe default.
export TP_STAGE_BF16_PV=${TP_STAGE_BF16_PV:-0}
...
case "$MODE" in
    plan|stage)
        ...
        ;;
    stream|null|check|source-check|bench|profile)
        ...
        ;;
    *) echo "usage: $0 {plan|stage|stream|null|check|source-check|bench|profile}" >&2; exit 2 ;;
esac
