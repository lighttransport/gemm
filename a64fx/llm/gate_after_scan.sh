...
DS4F_LAYERS=8 DS4F_VERIFY_GATE=1 DS4F_VG_NTOK=16 PROMPT_FILE=sweep_k_prompt.txt MAX_NEW=4 \
  DS4F_GEN_LOG=$PWD/vg2.log DS4F_GEN_SENTINEL=$PWD/vg2_s.txt ./run_ds4fbase_gen_12n.sh >/dev/null 2>&1 || true
# the gate prints via logmsg() -> ds4f_ep_rank00.txt, NOT stdout
grep -hE "match, common prefix|token :|verify:" ds4f_ep_rank00.txt > gate_after_scan.txt 2>/dev/null
