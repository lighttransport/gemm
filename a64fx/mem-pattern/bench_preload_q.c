/*
 * Pre-load Q Values Optimization
 *
 * Hypothesis: The ld1rw -> SDOT dependency chain limits performance.
 * Solution: Pre-load all 6 Q values into registers, then do all 24 SDOT.
 *
 * This uses z4-z7 for K tiles, z0-z5 (temp) for Q broadcasts.
 * Actually we need to be careful about register allocation.
 */
...
// Pre-load all Q values before SDOT
// Uses z0-z3 for K, z4-z7 for Q (only need 6, use z4-z7 + reuse)
__attribute__((noinline))
void preload_q_kernel(const int8_t* Q, const int8_t* K, int32_t* S) {
    __asm__ volatile(
        "ptrue p0.b\n\t"
        "ptrue p1.s\n\t"
        ...
        "1:\n\t"
        // Pre-load all 6 Q broadcasts FIRST
        "ld1rw {z4.s}, p1/z, [x5]\n\t"     // Q row 0
        "ld1rw {z5.s}, p1/z, [x6]\n\t"     // Q row 1
        "ld1rw {z6.s}, p1/z, [x7]\n\t"     // Q row 2
        "ld1rw {z7.s}, p1/z, [x11]\n\t"    // Q row 3
        // Need 2 more Q rows - use temporary regs
        // Actually we can do rows 4,5 after first set of SDOT
        ...
        // Now all loads are done, Q values in z4-z7, K in z0-z3
        // Do rows 0-3 SDOT
        "sdot z8.s, z0.b, z4.b\n\t"  "sdot z9.s, z1.b, z4.b\n\t"
        ...
        // Load Q rows 4,5 while SDOT is in flight
        "ld1rw {z4.s}, p1/z, [x12]\n\t"    // Q row 4
        "ld1rw {z5.s}, p1/z, [x13]\n\t"    // Q row 5
...
// Baseline for comparison (interleaved loads)
__attribute__((noinline))
void baseline_kernel(const int8_t* Q, const int8_t* K, int32_t* S) {
    __asm__ volatile(
        "ptrue p0.b\n\t"
        "ptrue p1.s\n\t"
        "eor z8.d, z8.d, z8.d\n\t"   "eor z9.d, z9.d, z9.d\n\t"
...
int main() {
    ...
    printf("==============================================\n");
    printf("Pre-load Q Values Optimization\n");
    printf("==============================================\n\n");
    ...
    for (int i = 0; i < warmup; i++) baseline_kernel(Q, K, S);

    printf("Approach                            Ticks Efficiency    SDOT/tick\n");
    printf("------------------------------------------------------------------\n");

    uint64_t start = rdcycle();
    for (int i = 0; i < iters; i++) baseline_kernel(Q, K, S);
    ...
    printf("Baseline (interleaved ld/sdot)      %6.1f    %5.1f%%       %6.2f\n",
           ticks_baseline, efficiency, sdot_per_tick);

    for (int i = 0; i < warmup; i++) preload_q_kernel(Q, K, S);

    start = rdcycle();
    for (int i = 0; i < iters; i++) preload_q_kernel(Q, K, S);
    end = rdcycle();
    double ticks_preload = (double)(end - start) / iters;
    sdot_per_tick = 1536.0 / ticks_preload;
    efficiency = sdot_per_tick / 40.0 * 100.0;
    printf("Pre-load Q (batch loads first)      %6.1f    %5.1f%%       %6.2f\n",
           ticks_preload, efficiency, sdot_per_tick);

    printf("\n");
    if (ticks_preload < ticks_baseline) {
        printf("Pre-load Q is %.1f%% faster.\n",
               (ticks_baseline - ticks_preload) / ticks_baseline * 100.0);
    } else {
        printf("Pre-load Q is %.1f%% slower.\n",
               (ticks_preload - ticks_baseline) / ticks_baseline * 100.0);
    }

    printf("\n95%% target = 40.4 ticks (38.0 SDOT/tick)\n");
