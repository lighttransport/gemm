/*
 * LD with predicate=0 for zeroing registers
 *
 * Hypothesis: ld1b with pfalse uses LD/ST pipe, not FPU pipe
 * This could be better than EOR which uses FL* pipe
 *
 * Three approaches:
 * 1. MOV z.s, #0     - uses EXA pipe (competes with FPU)
 * 2. EOR z.d, z.d, z.d - uses FL* pipe (still competes with SDOT)
 * 3. LD1B with pfalse  - uses LD/ST pipe (no FPU contention!)
 */
...
// Phase 2 with LD + pfalse zeroing (uses LD/ST pipe, not FPU!)
__attribute__((noinline))
void phase2_ld_zero(const int32_t* S, const int8_t* V, int32_t* O) {
    __asm__ volatile(
        "ptrue p0.b\n\t"
        "ptrue p1.s\n\t"
        "pfalse p2.b\n\t"  // All-false predicate for zeroing
        "mov x15, #4\n\t"
        "mov x16, %[V]\n\t"
        "mov x17, %[O]\n\t"
        "2:\n\t"
        // LD with pfalse zeroing - uses LD/ST pipe, completely avoids FPU!
        // Address doesn't matter since predicate is all-false (no actual load)
        "ld1b {z8.b}, p2/z, [x16]\n\t"   "ld1b {z9.b}, p2/z, [x16]\n\t"
        "ld1b {z10.b}, p2/z, [x16]\n\t"  "ld1b {z11.b}, p2/z, [x16]\n\t"
        "ld1b {z12.b}, p2/z, [x16]\n\t"  "ld1b {z13.b}, p2/z, [x16]\n\t"
        "ld1b {z14.b}, p2/z, [x16]\n\t"  "ld1b {z15.b}, p2/z, [x16]\n\t"
        ...
        : "memory", "x4", "x5", "x6", "x7", "x10", "x11", "x12", "x13",
          ...
          "z0", "z1", "z2", "z3", "z4", "z8", "z9", "z10", "z11", "z12", "z13", "z14", "z15",
          "z16", "z17", "z18", "z19", "z20", "z21", "z22", "z23",
          "z24", "z25", "z26", "z27", "z28", "z29", "z30", "z31", "p0", "p1", "p2"
    );
}

int main() {
    printf("==============================================\n");
    printf("LD+pfalse vs EOR vs MOV for Zeroing Registers\n");
    printf("==============================================\n\n");
    ...
    printf("Testing register zeroing methods:\n");
    printf("  MOV z.s, #0     - EXA pipe (competes with FPU)\n");
    printf("  EOR z.d, z.d    - FL* pipe (competes with SDOT)\n");
    printf("  LD1B + pfalse   - LD/ST pipe (no FPU contention!)\n\n");

    // Warmup
    for (int i = 0; i < 100; i++) {
        phase2_mov(S, V, O);
        phase2_eor(S, V, O);
        phase2_ld_zero(S, V, O);
    }
    ...
    double sdot_mov = phase2_sdot / ticks_mov;
    ...
    double sdot_eor = phase2_sdot / ticks_eor;
    ...
    double ticks_ld = (double)(end - start) / iters;
    double sdot_ld = phase2_sdot / ticks_ld;
    double eff_ld = sdot_ld / peak_sdot_per_tick * 100;

    printf("%-20s %10s %12s %10s\n", "Method", "Ticks", "SDOT/tick", "Efficiency");
    printf("----------------------------------------------------------\n");
    printf("%-20s %10.1f %12.2f %9.1f%%\n", "MOV z.s, #0", ticks_mov, sdot_mov, eff_mov);
    printf("%-20s %10.1f %12.2f %9.1f%%\n", "EOR z.d, z.d", ticks_eor, sdot_eor, eff_eor);
    printf("%-20s %10.1f %12.2f %9.1f%%\n", "LD1B + pfalse", ticks_ld, sdot_ld, eff_ld);

    printf("\n");
    printf("Improvement over MOV:\n");
    printf("  EOR:      %.1f%% faster\n", (ticks_mov - ticks_eor) / ticks_mov * 100);
    printf("  LD+pfalse: %.1f%% faster\n", (ticks_mov - ticks_ld) / ticks_mov * 100);

    if (ticks_ld < ticks_eor) {
        printf("\nLD+pfalse is best! Uses LD/ST pipe, no FPU contention.\n");
    } else if (ticks_eor < ticks_ld) {
        printf("\nEOR is better. LD+pfalse may have other overhead.\n");
    }
