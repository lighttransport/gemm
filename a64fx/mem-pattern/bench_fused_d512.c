/*
 * Optimized Fused GEMM for D=512, MR=6
 * Single function, no intermediate call overhead
 */
...
// Combined fused kernel for D=512
void fused_mr6_d512(
    const int8_t* Q,      // [6, 512]
    const int8_t* K,      // [128, 64, 4] interleaved (D_groups=128)
    const int8_t* V,      // [64, 512, 4] interleaved
    int32_t* O            // [6, 512]
) {
    // S on stack [6, 64] = 1.5KB
    ...
    __asm__ volatile(
        ...
        // ============== Phase 1: Q[6,512] @ K^T[512,64] -> S[6,64] ==============
        // D_groups = 128, unroll by 2 = 64 iterations
        "mov z8.s, #0\n\t"
        "mov z9.s, #0\n\t"
        "mov z10.s, #0\n\t"
        "mov z11.s, #0\n\t"
        ...
        // ============== Phase 2: S[6,64] @ V[64,512] -> O[6,512] ==============
        // 8 D-tiles of 64 columns each
        "mov x15, #8\n\t"
        "mov x16, %[V]\n\t"
        "mov x17, %[O]\n\t"
        ...
        // Next D-tile: reset O row ptr, advance column
        // Need to go back 5*2048=10240 bytes, split since >4095
        "sub x17, x17, #4096\n\t"
        "sub x17, x17, #4096\n\t"
        "sub x17, x17, #2048\n\t"
        "add x17, x17, #64\n\t"
...
int main() {
    printf("==============================================\n");
    printf("Optimized Fused GEMM D=512, MR=6\n");
    printf("==============================================\n\n");
    ...
    // Total SDOTs
    // Phase 1: 128 D_groups × 24 SDOTs = 3072
    // Phase 2: 8 D_tiles × 16 N_groups × 24 SDOTs = 3072
    int total_sdot = 3072 + 3072;

    printf("Configuration:\n");
    printf("  Phase 1: Q[6,512] @ K^T[512,64] -> S[6,64]\n");
    printf("  Phase 2: S[6,64] @ V[64,512] -> O[6,512]\n");
    printf("  Total: %d SDOTs\n\n", total_sdot);

    // Warmup
    for (int i = 0; i < 100; i++) {
        fused_mr6_d512(Q, K, V, O);
    }
    ...
    printf("  Ticks: %.1f\n", ticks);
    printf("  SDOT/tick: %.2f\n", sdot_per_tick);
    printf("  Est. SDOT/cycle: %.2f (assuming 20 ticks/cycle)\n", sdot_per_tick / 20.0 * 20);
    printf("  Efficiency: %.1f%% (vs 2.0 SDOT/cy peak)\n", (sdot_per_tick / 40.0) * 100);
