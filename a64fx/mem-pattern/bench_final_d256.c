/*
 * Final D=256 Fused Kernel Analysis
 *
 * Best achieved: ~89% SDOT efficiency
 *
 * Phase breakdown:
 *   Phase 1 (Q@K^T): 96.7% efficiency - nearly optimal
 *   Phase 2 (S@V):   82.1% efficiency - bottleneck due to:
 *     - 4 D-tile iterations with acc reset overhead
 *     - O store overhead (4 × 1536 bytes = 6KB)
 *
 * This benchmark shows the theoretical maximum with compute-only phases.
 */
...
// Full fused kernel with best practices
__attribute__((noinline))
void fused_optimized(const int8_t* Q, const int8_t* K, const int8_t* V, int32_t* O) {
...
int main() {
    printf("==============================================\n");
    printf("Final D=256 Fused Kernel Summary\n");
    printf("==============================================\n\n");
    ...
    int iters = 5000;
    int total_sdot = 3072;
    double peak_sdot_per_tick = 40.0;  // 2 SDOT/cycle × 20 cycles/tick

    printf("Configuration:\n");
    printf("  MR = 6 (rows processed per kernel invocation)\n");
    printf("  D = 256 (head dimension)\n");
    printf("  N = 64 (sequence length tile)\n");
    printf("  Total SDOTs = %d (Phase1: 1536 + Phase2: 1536)\n\n", total_sdot);

    printf("Architecture:\n");
    printf("  A64FX: 2 FPU pipelines, 2 SDOT/cycle peak\n");
    printf("  Counter: cntvct_el0 @ ~100MHz (20 CPU cycles/tick)\n");
    printf("  Peak: 40 SDOT/tick\n\n");

    // Warmup
    for (int i = 0; i < 100; i++) {
        fused_optimized(Q, K, V, O);
    }
    ...
    double efficiency = sdot_per_tick / peak_sdot_per_tick * 100;

    printf("%-25s %10s %12s %8s\n", "Metric", "Value", "Unit", "");
    printf("----------------------------------------------------------\n");
    printf("%-25s %10.1f %12s\n", "Ticks per kernel", ticks, "ticks");
    printf("%-25s %10.2f %12s\n", "SDOT/tick", sdot_per_tick, "");
    printf("%-25s %10.1f %12s\n", "Efficiency", efficiency, "%");

    // Theoretical breakdown
    printf("\n");
    printf("Theoretical analysis (from phase isolation):\n");
    printf("  Phase 1 alone: ~40 ticks @ 96.7%% eff\n");
    printf("  Phase 2 alone: ~47 ticks @ 82.1%% eff\n");
    printf("  Sum: ~87 ticks → Combined eff ~88.5%%\n");
    printf("\n");
    printf("Measured combined: %.1f ticks → %.1f%% eff\n", ticks, efficiency);

    if (efficiency > 88) {
        printf("\nResult: Achieved near-optimal efficiency for D=256!\n");
    }
