/*
 * Fused GEMM MR=6: D dimension sweep
 * Test efficiency with D=256, 512, 1024
 *
 * Hypothesis: Larger D amortizes loop overhead, improving efficiency
 */
...
// Optimized MR=6 kernel with 2x unroll (parameterized D)
void fused_mr6_phase1(
    const int8_t* Q,      // [6, D]
    const int8_t* K,      // [D_groups, 64, 4] interleaved
    int32_t* S,           // [6, 64]
    int D_groups          // D/4
) {
    __asm__ volatile(
...
// Phase 2: S @ V -> O (same for all D, since S is always [6,64])
void fused_mr6_phase2(
    const int32_t* S,     // [6, 64]
    const int8_t* V,      // [64, D, 4] interleaved
    int32_t* O,           // [6, D]
    int D,                // Output D dimension
    int O_stride          // O row stride in bytes
) {
    int D_tiles = D / 64;  // Number of 64-column tiles
    ...
    __asm__ volatile(
        ...
        // S row pointers (stride 256 bytes = 64 INT32)
        "mov x5, %[S]\n\t"
        "add x6, x5, #256\n\t"
        "add x7, x5, #512\n\t"
        "add x11, x5, #768\n\t"
        ...
        :
        : [S] "r"(S), [V] "r"(V), [O] "r"(O),
          [D_tiles] "r"((long)(D/64)), [O_stride] "r"((long)(D*4))
...
void run_benchmark(int D, int N, int iters) {
    int D_groups = D / 4;
    int D_tiles = D / 64;
    ...
    // Working set size
    int ws_kb = (6 * D + D_groups * N * 4 + N * D * 4) / 1024;

    // Warmup
    for (int i = 0; i < 100; i++) {
        fused_mr6_phase1(Q, K, S, D_groups);
        fused_mr6_phase2(S, V, O, D, D * 4);
    }
    ...
    // Total SDOTs:
    // Phase 1: D_groups × 24 SDOTs
    // Phase 2: D_tiles × 16 × 24 SDOTs = D_tiles × 384 SDOTs
    int phase1_sdot = D_groups * 24;
    int phase2_sdot = D_tiles * 16 * 24;
    int total_sdot = phase1_sdot + phase2_sdot;

    double sdot_per_tick = total_sdot / ticks;
    double efficiency = sdot_per_tick / 40.0 * 100;  // ~40 SDOT/tick at peak

    printf("D=%-4d  %6.1f ticks  %6d SDOT  %6.2f SDOT/tick  %5.1f%%  WS=%dKB\n",
           D, ticks, total_sdot, sdot_per_tick, efficiency, ws_kb);
...
int main() {
    printf("==============================================\n");
    printf("Fused GEMM MR=6: D Dimension Sweep\n");
    printf("==============================================\n\n");

    printf("Configuration: Q[6,D] @ K^T[D,64] -> S[6,64] -> S @ V[64,D] -> O[6,D]\n");
    printf("MR=6, N=64, 2x unroll\n\n");

    int N = 64;
    int iters = 5000;

    printf("%-6s %12s %10s %14s %8s %s\n",
           "D", "Ticks", "SDOTs", "SDOT/tick", "Eff", "Working Set");
    printf("------------------------------------------------------------------\n");

    run_benchmark(128, N, iters);
    run_benchmark(256, N, iters);
    run_benchmark(512, N, iters);
    run_benchmark(1024, N, iters);

    printf("\nNotes:\n");
    printf("  - Efficiency relative to ~40 SDOT/tick (~2 SDOT/cycle × 20 tick/cycle)\n");
    printf("  - L1D = 64KB, L2 = 8MB\n");
    printf("  - Larger D amortizes merge/store overhead\n");
