...
static void run_sdot4_omp(float*c,const fp4_i8_activation*a,const fp4_matrix*w,size_t src,int threads){
    double dt[7];fp4_gemv_i8_sdot4_omp(c,a,w,threads);
    for(int r=0;r<7;++r){double t=now();fp4_gemv_i8_sdot4_omp(c,a,w,threads);dt[r]=now()-t;}
    for(int i=1;i<7;++i){double x=dt[i];int j=i;while(j&&dt[j-1]>x){dt[j]=dt[j-1];--j;}dt[j]=x;}
    double med=dt[3];printf("kernel=fp4_i8_sdot4 threads=%d A_G=%d median_ms=%.3f gflops=%.2f packed_GB/s=%.2f checksum=%.7g\n",
      threads,a->scale_group,med*1e3,2.0*w->n*w->k/med/1e9,src/med/1e9,c[w->n/3]);
}
...
static double run_pair_tbl_omp(float*c,const fp4_pair_activation*a,const fp4_matrix*w,size_t src,int threads){
    double dt[7];fp4_gemv_pair_tbl_omp(c,a,w,threads);
    for(int r=0;r<7;++r){double t=now();fp4_gemv_pair_tbl_omp(c,a,w,threads);dt[r]=now()-t;}
    for(int i=1;i<7;++i){double x=dt[i];int j=i;while(j&&dt[j-1]>x){dt[j]=dt[j-1];--j;}dt[j]=x;}
    double med=dt[3];printf("kernel=fp4_pair_tbl threads=%d A_G=%d median_ms=%.3f gflops=%.2f packed_GB/s=%.2f checksum=%.7g\n",
      threads,a->scale_group,med*1e3,2.0*w->n*w->k/med/1e9,src/med/1e9,c[w->n/3]);return med;
}
...
#pragma omp parallel num_threads(12)
    ...
    if(getenv("FP4_SDOT4_ONLY")){float*af=aa((size_t)k*4),*c=aa((size_t)n*4);
      fp4_i8_activation qa={0};if(!af||!c)return 1;
      for(int i=0;i<k;++i)af[i]=(float)((int)(rnd()&255)-128)/512;
      if(fp4_i8_activation_prepare(&qa,af,k,4))return 1;run_sdot4_omp(c,&qa,&w,source_sdot4,12);
      fp4_i8_activation_free(&qa);free(af);free(c);fp4_matrix_free(&w);return 0;}
    ...
    int ms[]={1,6,8,12,24,128};for(int z=0;z<6;++z){int m=ms[z];
      ...
      if(m==1){int ag=argc>4?atoi(argv[4]):4;float*fa=aa((size_t)k*4);fp4_i8_activation qa={0};
        ...
        run_sdot_omp(c,&qa,&w,(size_t)n*k+w.scales_n32_count*2,12);
        if(ag==4)run_sdot4_omp(c,&qa,&w,source_sdot4,12);
