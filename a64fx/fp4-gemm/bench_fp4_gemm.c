...
static int run_gemm(float*c,const _Float16*a,const fp4_matrix*w,int m,int kc,int threads){return use_n32?fp4_gemm_f16_n32(c,a,w,m,kc):fp4_gemm_f16(c,a,w,m,kc,threads);}
static double bench_one(const char*name,const fp4_matrix*w,int m,int kc,int threads,int reps,double*rel,double*mx){_Float16*a=aligned_alloc(256,((size_t)m*w->k*2+255)&~255ULL);float*c=aligned_alloc(256,((size_t)m*w->n*4+255)&~255ULL);for(size_t i=0;i<(size_t)m*w->k;++i)a[i]=(_Float16)(rnd()*0.25f);run_gemm(c,a,w,m,kc,threads);double best=1e30,sum=0;for(int q=0;q<reps;++q){double t=now_sec();run_gemm(c,a,w,m,kc,threads);double d=now_sec()-t;sum+=d;if(d<best)best=d;}sampled_error(c,a,w,m,rel,mx);double med=sum/reps;double gf=2.0*m*w->n*w->k/med/1e9;printf("tensor=%s kernel=%s format=%s M=%d N=%d K=%d kc=%d thr=%d ms=%.3f best_ms=%.3f gflops=%.2f rel_l2=%.3e max_abs=%.3e\n",name,use_n32?"n32":"row",fp4_format_name(w->format),m,w->n,w->k,kc,threads,med*1e3,best*1e3,gf,*rel,*mx);free(a);free(c);return med;}
...
static int run_weight(const char*name,const uint8_t*blob,record*wr,record*sr,int native,int threads,const int*ms,int nm,int reps){float*base=native?dequant_native_mx(blob,wr,sr):dequant_fp8(blob,wr,sr);if(!base){fprintf(stderr,"dequant failed: %s\n",name);return 1;}int n=(int)wr->shape[0],k=(int)wr->shape[1]*(native?2:1),kcs[]={32,64,128,256,0};for(int f=0;f<3;++f){fp4_matrix p;if(f==0&&native){if(native_mx(&p,blob,wr,sr))return 1;}else{if(fp4_matrix_alloc(&p,(fp4_format)f,n,k)||fp4_quantize_f32(&p,base))return 1;}if(use_n32&&fp4_matrix_prepare_n32(&p))return 1;double qrel,qmax;quant_error(base,&p,&qrel,&qmax);printf("quant tensor=%s format=%s source=%s rel_l2=%.3e max_abs=%.3e\n",name,fp4_format_name((fp4_format)f),native?"native-mxfp4":"checkpoint-fp8",qrel,qmax);for(int mi=0;mi<nm;++mi)for(int ci=0;ci<5;++ci)if(!kcs[ci]||kcs[ci]<=k){double rel,mx;bench_one(name,&p,ms[mi],kcs[ci],threads,reps,&rel,&mx);}fp4_matrix_free(&p);}free(base);return 0;}

static int scan_weight(const uint8_t*blob,record*wr,record*sr,int threads,double sec[3],double flop[3]){float*base=dequant_native_mx(blob,wr,sr);if(!base)return 1;int n=(int)wr->shape[0],k=(int)wr->shape[1]*2;_Float16*a=aligned_alloc(256,((size_t)k*2+255)&~255ULL);float*c=aligned_alloc(256,((size_t)n*4+255)&~255ULL);if(!a||!c)return 1;for(int i=0;i<k;++i)a[i]=(_Float16)(rnd()*.25f);for(int f=0;f<3;++f){fp4_matrix p;if(f==0){if(native_mx(&p,blob,wr,sr))return 1;}else if(fp4_matrix_alloc(&p,(fp4_format)f,n,k)||fp4_quantize_f32(&p,base))return 1;if(use_n32&&fp4_matrix_prepare_n32(&p))return 1;double t=now_sec();run_gemm(c,a,&p,1,32,threads);sec[f]+=now_sec()-t;flop[f]+=2.0*n*k;fp4_matrix_free(&p);}free(a);free(c);free(base);return 0;}

int main(int argc,char**argv){const char*dir="/local/u14346/ds4f-fp4-gemm";int threads=48,experts=256,reps=3,full=0,skip_attn=0,skip_experts=0;for(int i=1;i<argc;++i){if(!strcmp(argv[i],"--data")&&i+1<argc)dir=argv[++i];else if(!strcmp(argv[i],"--threads")&&i+1<argc)threads=atoi(argv[++i]);else if(!strcmp(argv[i],"--experts")&&i+1<argc)experts=atoi(argv[++i]);else if(!strcmp(argv[i],"--reps")&&i+1<argc)reps=atoi(argv[++i]);else if(!strcmp(argv[i],"--full"))full=1;else if(!strcmp(argv[i],"--skip-attention"))skip_attn=1;else if(!strcmp(argv[i],"--skip-experts"))skip_experts=1;else if(!strcmp(argv[i],"--kernel")&&i+1<argc){const char*k=argv[++i];if(!strcmp(k,"n32"))use_n32=1;else if(strcmp(k,"row")){fprintf(stderr,"bad kernel %s\n",k);return 2;}}else{fprintf(stderr,"usage: %s [--data DIR] [--threads N] [--experts N] [--reps N] [--full] [--kernel row|n32] [--skip-attention|--skip-experts]\n",argv[0]);return 2;}}
    if(use_n32&&threads!=1){fprintf(stderr,"n32 kernel is single-core; use --threads 1\n");return 2;}
