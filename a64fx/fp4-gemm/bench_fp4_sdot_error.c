...
int main(int ac,char**av){const char*raw=ac>1?av[1]:"/local/u14346/ds4f-fp4-gemm/layer0.raw";
 ...
 int fd=open(raw,O_RDONLY);if(fd<0){perror(raw);return 1;}double worst=0,mincos=1,worstlin=0;
 for(int wi=1;wi<=3;++wi){char wn[256],sn[256];snprintf(wn,sizeof(wn),"layers.0.ffn.experts.0.w%d.weight",wi);snprintf(sn,sizeof(sn),"layers.0.ffn.experts.0.w%d.scale",wi);rec wr,sr;
  ...
  int n=wr.n,k=wr.k*2;fp4_matrix w;if(fp4_matrix_alloc(&w,FP4_MX,n,k)||read_full(fd,w.codes,wr.bytes,wr.off)||read_full(fd,w.scales,sr.bytes,sr.off)||fp4_matrix_prepare_n32(&w)||fp4_matrix_prepare_u8(&w)||fp4_matrix_prepare_sdot(&w)||fp4_matrix_prepare_pair(&w)||fp4_matrix_prepare_sdot4(&w))return 1;
  float*a=aligned_alloc(256,(size_t)k*4),*ref=aligned_alloc(256,(size_t)n*4),*got=aligned_alloc(256,(size_t)n*4),*pair=aligned_alloc(256,(size_t)n*4),*lin=aligned_alloc(256,(size_t)n*4);fp4_i8_activation qa={0};fp4_pair_activation pa={0};if(!a||!ref||!got||!pair||!lin)return 1;
  double tw=0,tc=1,tl=0;for(int d=0;d<4;++d)for(int seed=0;seed<3;++seed){fill(a,k,d,seed);
   for(int r=0;r<n;++r){double z=0,lz=0;for(int b=0;b<k;b+=32){float am=0;
      for(int x=b;x<b+32;++x)am=fmaxf(am,fabsf(fp4_dequant_value(&w,r,x)));
      float step=am*(1.0f/7.0f);for(int x=b;x<b+32;++x){float v=fp4_dequant_value(&w,r,x);
        int q=step>0?(int)lrintf(v/step):0;if(q>7)q=7;if(q<-7)q=-7;
        z+=(double)a[x]*v;lz+=(double)a[x]*q*step;}}
     ref[r]=(float)z;lin[r]=(float)lz;}
   ...
   double re,co,pdiff,pcos,lre,lco;err(ref,got,n,&re,&co);err(got,pair,n,&pdiff,&pcos);err(ref,lin,n,&lre,&lco);
   if(pdiff>2e-6){fprintf(stderr,"pair/sdot mismatch %.6g\n",pdiff);return 1;}
   if(ag==4){if(fp4_gemv_i8_sdot4_omp(pair,&qa,&w,12))return 1;
     err(got,pair,n,&pdiff,&pcos);if(pdiff>2e-6){fprintf(stderr,"packed/sdot mismatch %.6g\n",pdiff);return 1;}}
   if(re>tw)tw=re;if(co<tc)tc=co;if(lre>tl)tl=lre;}
  printf("tensor=w%d N=%d K=%d A_G=%d worst_rel_l2=%.6g min_cos=%.9f linear_i4_rel=%.6g\n",wi,n,k,ag,tw,tc,tl);if(tw>worst)worst=tw;if(tc<mincos)mincos=tc;if(tl>worstlin)worstlin=tl;
  fp4_i8_activation_free(&qa);fp4_pair_activation_free(&pa);fp4_matrix_free(&w);free(a);free(ref);free(got);free(pair);free(lin);}
 close(fd);printf("summary A_G=%d worst_rel_l2=%.6g min_cos=%.9f linear_i4_rel=%.6g\n",ag,worst,mincos,worstlin);return 0;}
