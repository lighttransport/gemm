...
int main(int ac,char**av){const char*raw=ac>1?av[1]:"/local/u14346/ds4f-fp4-gemm/layer0.raw";
 ...
 for(int wi=1;wi<=3;++wi){char wn[256],sn[256];snprintf(wn,sizeof(wn),"layers.0.ffn.experts.0.w%d.weight",wi);snprintf(sn,sizeof(sn),"layers.0.ffn.experts.0.w%d.scale",wi);rec wr,sr;
  ...
  int n=wr.n,k=wr.k*2;fp4_matrix w;if(fp4_matrix_alloc(&w,FP4_MX,n,k)||read_full(fd,w.codes,wr.bytes,wr.off)||read_full(fd,w.scales,sr.bytes,sr.off)||fp4_matrix_prepare_n32(&w)||fp4_matrix_prepare_u8(&w)||fp4_matrix_prepare_sdot(&w))return 1;
  float*a=aligned_alloc(256,(size_t)k*4),*ref=aligned_alloc(256,(size_t)n*4),*got=aligned_alloc(256,(size_t)n*4);fp4_i8_activation qa={0};if(!a||!ref||!got)return 1;
  double tw=0,tc=1;for(int d=0;d<4;++d)for(int seed=0;seed<3;++seed){fill(a,k,d,seed);
   ...
   if(fp4_i8_activation_prepare(&qa,a,k,ag)||fp4_gemv_i8_sdot_omp(got,&qa,&w,12))return 1;double re,co;err(ref,got,n,&re,&co);if(re>tw)tw=re;if(co<tc)tc=co;}
  printf("tensor=w%d N=%d K=%d A_G=%d worst_rel_l2=%.6g min_cos=%.9f\n",wi,n,k,ag,tw,tc);if(tw>worst)worst=tw;if(tc<mincos)mincos=tc;
  fp4_i8_activation_free(&qa);fp4_matrix_free(&w);free(a);free(ref);free(got);}
