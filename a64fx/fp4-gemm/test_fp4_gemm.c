...
int main(void){
    ...
    enum{M=7,N=32,K=64}; float*w=malloc((size_t)N*K*4),*ref=malloc((size_t)M*N*4),*got=malloc((size_t)M*N*4);
    _Float16*a=malloc((size_t)M*K*2);if(!w||!ref||!got||!a)return 1;
    for(int i=0;i<N*K;++i)w[i]=rnd()*0.25f;for(int i=0;i<M*K;++i)a[i]=(_Float16)(rnd()*0.5f);
    for(int f=0;f<3;++f){fp4_matrix p;if(fp4_matrix_alloc(&p,(fp4_format)f,N,K)||fp4_quantize_f32(&p,w))return 1;
        fp4_gemm_reference(ref,a,&p,M,1);
        for(int kc=0;kc<=64;kc+=32){if(fp4_gemm_f16(got,a,&p,M,kc,2))return 1;double num=0,den=0;
            for(int i=0;i<M*N;++i){double d=got[i]-ref[i];num+=d*d;den+=(double)ref[i]*ref[i];}
            double rel=sqrt(num/(den+1e-30));printf("%s kc=%d rel_l2=%.6g\n",fp4_format_name((fp4_format)f),kc,rel);
            if(!isfinite(rel)||rel>0.08)return 1;}
        fp4_matrix_free(&p);}
    free(w);free(ref);free(got);free(a);puts("FP4 GEMM tests: PASS");return 0;
}
