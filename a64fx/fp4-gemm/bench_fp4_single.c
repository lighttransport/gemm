...
int main(int argc,char**argv){int n=argc>1?atoi(argv[1]):2048,k=argc>2?atoi(argv[2]):4096;
    ...
    int ms[]={1,6,12,24,48,128},kcs[]={256,0};
    ...
    for(int f=0;f<3;++f){fp4_matrix w;if(fp4_matrix_alloc(&w,(fp4_format)f,n,k)||
        ...
        for(int mi=0;mi<6;++mi){int m=ms[mi];_Float16*a=aa((size_t)m*k*2);float*c=aa((size_t)m*n*4);
            ...
            for(int ci=0;ci<2;++ci){int kc=kcs[ci];fp4_gemm_f16_l2(c,a,&w,m,kc);
                double best=1e9;for(int rep=0;rep<5;++rep){double t=now();fp4_gemm_f16_l2(c,a,&w,m,kc);double d=now()-t;if(d<best)best=d;}
                printf("format=%s kernel=l2panel M=%d kc=%d ms=%.3f gflops=%.2f\n",fp4_format_name((fp4_format)f),m,kc,best*1e3,2.0*m*n*k/best/1e9);}
