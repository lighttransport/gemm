...
#undef N
    glm53f_st_close(st);x=a256(H*4);gv=a256(in*4);uv=a256(in*4);act=a256(in*4);part=a256(H*4);out[0]=a256(H*4);out[1]=a256(H*4);for(int j=0;j<H;j++)x[j]=(float)(((j*29+7)%257)-128)/128.0f;float ph[2][3],el[2];for(int p=0;p<2;p++){glm53f_dense_scalar_reference=getenv("GLM53F_DENSE_SCALAR_CHECK")&&p==1;double t0=MPI_Wtime();mv(gv,g,gs,x,in,H);mv(uv,u,us,x,in,H);
#pragma omp parallel for schedule(static)
        for(int j=0;j<in;j++){float a=gv[j]>10?10:gv[j],b=uv[j]>10?10:uv[j]<-10?-10:uv[j];act[j]=a/(1+expf(-a))*b;}double t1=MPI_Wtime();
#pragma omp parallel for schedule(static)
        for(int r=0;r<H;r++)part[r]=dot(d+(size_t)r*in,ds+(size_t)(r/B)*lb,act,in);double t2=MPI_Wtime();MPI_Allreduce(part,out[p],H,MPI_FLOAT,MPI_SUM,MPI_COMM_WORLD);double t3=MPI_Wtime();ph[p][0]=t1-t0;ph[p][1]=t2-t1;ph[p][2]=t3-t2;el[p]=t3-t0;}glm53f_dense_scalar_reference=0;glm53f_dense_ffn_context_12n view={rank,i0,in,lb,hb,g,u,d,gs,us,ds,gv,uv,act,part,NULL,NULL,NULL,NULL};float*api=a256(H*4);int api_ok=!glm53f_dense_ffn_sublayer_12n(&view,api,x)&&!memcmp(api,out[getenv("GLM53F_DENSE_SCALAR_CHECK")?0:1],H*4);double se=0,sr=0,ma=0;for(int j=0;j<H;j++){double de=(double)out[1][j]-out[0][j];se+=de*de;sr+=(double)out[0][j]*out[0][j];if(fabs(de)>ma)ma=fabs(de);}double rel=sqrt(se/(sr+1e-30));int scalar_check=getenv("GLM53F_DENSE_SCALAR_CHECK")!=NULL;int ok=(scalar_check?rel<2e-5:!memcmp(out[0],out[1],H*4))&&api_ok,all;for(int j=0;j<H;j++)ok&=isfinite(out[0][j]);MPI_Allreduce(&ok,&all,1,MPI_INT,MPI_MIN,MPI_COMM_WORLD);float me,mp[3];MPI_Allreduce(&el[1],&me,1,MPI_FLOAT,MPI_MAX,MPI_COMM_WORLD);MPI_Allreduce(ph[1],mp,3,MPI_FLOAT,MPI_MAX,MPI_COMM_WORLD);double ss=0;for(int j=0;j<H;j++)ss+=(double)out[0][j]*out[0][j];if(!rank)printf("GLM53F_DENSE_FFN_12N layer=%d slice=%d+%d weight_MiB_rank=%.3f max_ms=%.3f gate_up_ms=%.3f down_ms=%.3f ar_ms=%.3f rms=%.9g scalar_rel_l2=%.9g scalar_max_abs=%.9g repeat=%s callback=%s %s\n",layer,i0,in,((size_t)2*in*H+H*in+(size_t)2*lb*hb*4+(size_t)(H/B)*lb*4)/1048576.0,me*1e3f,mp[0]*1e3f,mp[1]*1e3f,mp[2]*1e3f,sqrt(ss/H),rel,ma,all?(scalar_check?"SCALAR_PASS":"BIT_EXACT"):"FAIL",api_ok?"BIT_EXACT":"FAIL",all?"PASS":"FAIL");free(api);MPI_Finalize();return all?0:1;}
#endif
