#!/bin/bash
# GLM-5.2 FP8 full-layer chunked-prefill benchmark on 96 A64FX nodes
# with an 8x12 torus placement. This stages the FP8 model, then exercises
# glm5_forward_prefill_chunk through GLM5_PREFILL_SYNTH + GLM5_PCHUNK.
#
# Useful overrides:
#   GLM5_PREFILL_SYNTH=1024       number of synthetic prompt tokens
#   GLM5_PCHUNK_SWEEP="32 64"     chunk sizes to sweep
#   GLM5_THREAD_SWEEP="12 24"     OpenMP thread counts to sweep
#   GLM5_MAXPOS=1048576           long-context arena target
#   GLM5_TOPO_ONLY=1              validate allocation/topology only
...
WORK="$GLM5/prefill_fp8_run_${JOB_TAG}_96n"
...
# 96-way sharding gives 21/22 shared-expert columns per rank; MXFP8 scale
# blocks are 32 columns, so keep shared experts replicated by default.
...
echo "=== GLM5.2 FP8 prefill 96n torus: NP=$NP layers=$RUN_LAYERS synth=$PREFILL_SYNTH maxpos=$GLM5_MAXPOS model=$GLM5_MODEL_DIR job=${PJM_JOBID:-?} ==="
...
for t in 1 2 3 4 5; do
    ...
    echo "[prefill-fp8-96] topo try $t failed; retry"
    sleep 3
done
...
if [ "${GLM5_TOPO_ONLY:-0}" = 1 ]; then
    ...
    echo "=== prefill-fp8-96 topo-only done $(date) ==="
    exit 0
fi
...
echo "=== prefill-fp8-96 torus done $(date) ==="
