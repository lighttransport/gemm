#!/bin/bash
# GLM-5.2 12-node generation smoke. This stages a layer slice and exercises
# prompt-id prefill, greedy decode, output-id writing, and offline detokenization.
# A partial-layer run is for plumbing only; meaningful text requires full 192-node.
...
echo "=== GLM5.2 gen 12n: NP=$NP stage_layers=$STAGE_LAYERS run_layers=$RUN_LAYERS max_new=$GLM5_MAX_NEW job=${PJM_JOBID:-?} ==="
date

"$GLM5/check_glm5_model.sh" "$GLM5_MODEL_DIR" --tokenizer || exit 2
test -s "$GLM5/prompt_ids.txt" || { echo "FATAL: missing $GLM5/prompt_ids.txt"; exit 2; }

rm -f tofu_topo.txt glm5_stage_rank*.txt glm5_ep_*.txt glm5_ep_rank00.txt gen_ids.txt
...
for t in 1 2 3 4 5; do
    ...
    echo "[gen12] topo try $t failed; retry"
    sleep 3
done
...
echo "--- gen ids ---"
cat "$GLM5/gen_ids.txt" 2>/dev/null
echo "--- decoded ---"
python3 "$GLM5/glm5_tokenizer.py" decode-file "$GLM5/gen_ids.txt" 2>/dev/null || true
echo "=== gen12 done $(date) ==="
