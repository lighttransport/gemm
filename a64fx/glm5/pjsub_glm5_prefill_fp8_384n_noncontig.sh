#!/bin/bash
# GLM-5.2 FP8 full-layer chunked-prefill benchmark on 384 non-contiguous
# A64FX nodes. Optimized batched parallel prefill (chunked, CP-parallel combine,
# 5-token FP8 GEMM, comm overlap). Defaults to the 1M-context CP config; intended
# for synthetic long-context (e.g. GLM5_PREFILL_SYNTH=1048576) throughput tests.

#PJM -g hp250467
#PJM -L "rscgrp=small,node=384,elapse=03:00:00"
...
WORK="$GLM5/prefill_fp8_run_${JOB_TAG}_384n_noncontig"
...
echo "=== GLM5.2 FP8 prefill 384n noncontig: NP=$NP layers=$RUN_LAYERS synth=$PREFILL_SYNTH maxpos=$GLM5_MAXPOS model=$GLM5_MODEL_DIR job=${PJM_JOBID:-?} ==="
...
for t in 1 2 3 4 5; do
    ...
    echo "[prefill-fp8-384-noncontig] topo try $t failed; retry"
    sleep 3
done
...
if [ "${GLM5_TOPO_ONLY:-0}" = 1 ]; then
    echo "=== prefill-fp8-384-noncontig topo-only done $(date) ==="
    exit 0
fi
...
echo "=== prefill-fp8-384 noncontig done $(date) ==="
