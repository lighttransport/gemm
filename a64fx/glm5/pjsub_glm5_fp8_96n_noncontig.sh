#!/bin/bash
# GLM-5.2 FP8 real-weight run on 96 A64FX nodes with non-contiguous placement.
# Stages ~/models/glm52-fp8 (141 safetensor shards) to /local/glm5_$JOBID,
# then runs the GLM5_REAL=1 EP+TP forward with prompt generation.
...
echo "=== GLM5.2 FP8 real 96n noncontig: NP=$NP stage_layers=$STAGE_LAYERS run_layers=$RUN_LAYERS model=$GLM5_MODEL_DIR job=${PJM_JOBID:-?} ==="
date
...
for t in 1 2 3 4 5; do
    ...
    echo "[fp8-96-noncontig] topo try $t failed; retry"
    sleep 3
done
...
echo "--- run GLM5_REAL=1 FP8 noncontig ($(date)) ---"
...
echo "=== fp8 96n noncontig done $(date) ==="
