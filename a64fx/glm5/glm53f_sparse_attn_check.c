...
#pragma omp parallel for schedule(static)
    for(int r=0;r<rows;r++) y[r]=glm53f_dot_fp8_block128(w+(size_t)r*cols,s+(size_t)(r/128)*(cols/128),x,cols);
}
...
int main(int argc,char**argv){
    ...
    uint16_t *qan,*kvb,*wk,*knw,*knb,*gate,*ape,*wqb,*wp;
    float *hidden,*qres,*query,*latent,*key,*gcache,*iq,*iw,*pool,*attn,*out[2]; int *selected[2];
    if(argc<2||tokens<1||tokens>(index_only?4096:64)){fprintf(stderr,"usage: %s MODEL_DIR [tokens=9]\n",argv[0]);return 2;}
...
#define GET(V,S,T) do{L(S);V=(T*)tensor(st,n,&z);if(!(V))return 2;}while(0)
    ...
    GET(kvas,"kv_a_proj_with_mqa.weight_scale_inv",float); GET(kvb,"kv_b_proj.weight",uint16_t);
...
#undef L
    ...
    pool=alloc256((size_t)((tokens/KPOOL)+1)*IDIM*4);attn=alloc256((size_t)HEADS*VD*4);
    ...
    if(!hidden||!qres||!query||!latent||!key||!gcache||!iq||!iw||!pool||!attn||!out[0]||!out[1]||!selected[0]||!selected[1])return 2;
    ...
    double ss=0;int finite=1;for(int i=0;i<H;i++){ss+=(double)out[0][i]*out[0][i];finite&=isfinite(out[0][i]);}
    printf("GLM53F_SPARSE_ATTN mode=%s tokens=%d selected=%d excluded=%d first=%d last=%d repeat=%s finite=%s rms=%.9g sec=%.6f %s\n",
           index_only?"INDEX":"FULL",tokens,selected_n[0],tokens-selected_n[0],selected[0][0],selected[0][selected_n[0]-1],stable?"BIT_EXACT":"FAIL",finite?"YES":"NO",sqrt(ss/H),elapsed[1],stable&&finite?"PASS":"FAIL");
