#!/bin/bash
# GLM-5.2 FP8 full-layer chunked-prefill benchmark on 24 non-contiguous
# A64FX nodes. This usually queues faster than torus placement but may pay
# higher uTofu communication cost.

#PJM -g hp250467
#PJM -L "rscgrp=small-s2,node=24,elapse=03:00:00"
...
WORK="$GLM5/prefill_fp8_run_${JOB_TAG}_24n_noncontig"
...
echo "=== GLM5.2 FP8 prefill 24n noncontig: NP=$NP layers=$RUN_LAYERS synth=$PREFILL_SYNTH maxpos=$GLM5_MAXPOS model=$GLM5_MODEL_DIR job=${PJM_JOBID:-?} ==="
...
for t in 1 2 3 4 5; do
    ...
    echo "[prefill-fp8-24-noncontig] topo try $t failed; retry"
    sleep 3
done
...
if [ "${GLM5_TOPO_ONLY:-0}" = 1 ]; then
    echo "=== prefill-fp8-24-noncontig topo-only done $(date) ==="
    exit 0
fi
...
echo "=== prefill-fp8-24 noncontig done $(date) ==="
