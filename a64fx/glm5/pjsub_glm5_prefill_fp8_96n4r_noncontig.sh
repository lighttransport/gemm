#!/bin/bash
# GLM-5.2 FP8 full-layer chunked-prefill benchmark on 96-node 4-ranks-per-node non-contiguous
...
WORK="$GLM5/prefill_fp8_run_${JOB_TAG}_96n4r_noncontig"
...
echo "=== GLM5.2 FP8 prefill 96n 4ranks/node noncontig: NP=$NP layers=$RUN_LAYERS synth=$PREFILL_SYNTH maxpos=$GLM5_MAXPOS model=$GLM5_MODEL_DIR job=${PJM_JOBID:-?} ==="
...
for t in 1 2 3 4 5; do
    ...
    echo "[prefill-fp8-96n4r-noncontig] topo try $t failed; retry"
    sleep 3
done
...
if [ "${GLM5_TOPO_ONLY:-0}" = 1 ]; then
    echo "=== prefill-fp8-96n4r-noncontig topo-only done $(date) ==="
    exit 0
fi
...
echo "=== prefill-fp8-96n4r noncontig done $(date) ==="
