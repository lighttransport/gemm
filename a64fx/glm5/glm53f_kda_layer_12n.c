...
#define PART(F,S,T,OFF,N) do{name(n,layer,S);w.F=(T*)a256((size_t)(N)*sizeof(T));read_part(st,n,(size_t)(OFF)*sizeof(T),w.F,(size_t)(N)*sizeof(T),rank);}while(0)
    ...
    PART(fa,"f_a_proj.weight",uint16_t,0,(size_t)D*H);PART(ga,"g_a_proj.weight",uint16_t,0,(size_t)D*H);PART(on,"o_norm.weight",uint16_t,0,D);PART(op,"o_proj.weight",uint16_t,0,(size_t)H*QKV);
#undef PART
    glm53f_st_close(st);
...
#pragma omp parallel for schedule(static)
        for(int r=0;r<H;r++)partial[r]=dot1(w.op+(size_t)r*QKV+(size_t)h0*D,normed,qd);
