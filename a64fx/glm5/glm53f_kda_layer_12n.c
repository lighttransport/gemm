...
#pragma omp parallel for schedule(static)
    for(int r=0;r<H;r++)out[r]=dot1(w->op+(size_t)r*qd,c->normed,qd);double t2=MPI_Wtime();c->phase[0]=t1-t0;c->phase[1]=t2-t1;c->phase[2]=0;return 0;}
int glm53f_kda_sublayer_12n(void*context,float*out,const float*x){glm53f_kda_context_12n*c=context;if(!c||kda_local(c,c->partial,x))return-1;double t=MPI_Wtime();int rc=MPI_Allreduce(c->partial,out,H,MPI_FLOAT,MPI_SUM,MPI_COMM_WORLD);c->phase[2]=MPI_Wtime()-t;return rc==MPI_SUCCESS?0:-1;}
int glm53f_kda_sublayer_batch_12n(glm53f_kda_context_12n*c,float*out,const float*x,int tokens){if(!c||!out||!x||tokens<1||tokens>5)return-1;for(int i=0;i<tokens;i++)if(kda_local(c,c->batch_partial+(size_t)i*H,x+(size_t)i*H))return-1;double t=MPI_Wtime();int rc=MPI_Allreduce(c->batch_partial,out,tokens*H,MPI_FLOAT,MPI_SUM,MPI_COMM_WORLD);c->phase[2]=MPI_Wtime()-t;return rc==MPI_SUCCESS?0:-1;}
