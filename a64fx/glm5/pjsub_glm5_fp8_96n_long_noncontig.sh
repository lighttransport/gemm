#!/bin/bash
# GLM-5.2 FP8 long-context generation validation on 96 non-contiguous A64FX nodes.
# Builds a 1024+ token prompt by repeating prompt_fp8_1k.txt if needed, requests
# exactly 1024 output tokens, and reports prefill/decode throughput plus text.
...
echo "=== GLM5.2 FP8 long gen 96n noncontig: NP=$NP maxpos=$GLM5_MAXPOS max_new=$GLM5_MAX_NEW model=$GLM5_MODEL_DIR job=${PJM_JOBID:-?} ==="
date
...
cd "$RUN_DIR" || exit 2
rm -f tofu_topo.txt glm5_stage_rank*.txt glm5_ep_*.txt glm5_ep_rank00.txt gen_ids.txt prompt_ids_long.txt prompt_long.txt

: > prompt_long.txt
PROMPT_TOK=0
for rep in 1 2 3 4; do
    cat "$GLM5/prompt_fp8_1k.txt" >> prompt_long.txt
    printf "\n\n" >> prompt_long.txt
    python3 "$GLM5/glm5_tokenizer.py" chat-file prompt_long.txt > prompt_ids_long.txt || exit 2
    PROMPT_TOK=$(wc -w < prompt_ids_long.txt)
    echo "prompt repetition $rep -> tokens: $PROMPT_TOK"
    [ "$PROMPT_TOK" -ge 1024 ] && break
done
[ "$PROMPT_TOK" -ge 1024 ] || { echo "FATAL: prompt is below 1024 tokens after repetition"; exit 2; }
TOTAL_CTX=$((PROMPT_TOK + GLM5_MAX_NEW))
[ "$TOTAL_CTX" -le "$GLM5_MAXPOS" ] || { echo "FATAL: prompt+max_new=$TOTAL_CTX exceeds GLM5_MAXPOS=$GLM5_MAXPOS"; exit 2; }

export GLM5_PROMPT_IDS="$RUN_DIR/prompt_ids_long.txt"
export GLM5_GEN_OUT="$RUN_DIR/gen_ids.txt"
echo "prompt tokens: $PROMPT_TOK target_new=$GLM5_MAX_NEW total_ctx=$TOTAL_CTX"
...
for t in 1 2 3 4 5; do
    ...
    echo "[fp8-long-noncontig] topo try $t failed; retry"
    sleep 3
done
...
echo "--- run long GLM5_REAL=1 FP8 noncontig ($(date)) ---"
...
echo "--- decoded head ---"
python3 "$GLM5/glm5_tokenizer.py" decode-file "$GLM5_GEN_OUT" 2>/dev/null | sed -n '1,120p' || true
echo "=== fp8 long 96n noncontig done $(date) ==="
