...
WORK="$GLM5/prefill_fp8_run_${JOB_TAG}_96n2r_noncontig"
...
for t in 1 2 3 4 5; do
    ...
    echo "[prefill-fp8-96n2r-noncontig] topo try $t failed; retry"
    sleep 3
done
...
if [ "${GLM5_TOPO_ONLY:-0}" = 1 ]; then
    echo "=== prefill-fp8-96n2r-noncontig topo-only done $(date) ==="
    exit 0
fi
...
echo "=== prefill-fp8-96n2r noncontig done $(date) ==="
