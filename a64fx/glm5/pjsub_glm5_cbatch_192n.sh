#!/bin/bash
# GLM-5.2 full 192-node continuous-batch generation for agentic coding prompts.
...
WORK="$GLM5/cbatch_run_${PJM_JOBID:-manual}_192n"
mkdir -p "$WORK" || exit 2
cd "$WORK" || exit 2
...
export GLM5_MAX_NEW=${GLM5_MAX_NEW:-64}
...
echo "=== GLM5.2 cbatch 192n: NP=$NP layers=$RUN_LAYERS slots=$GLM5_CBATCH_SLOTS max_new=$GLM5_MAX_NEW prompts=$PROMPTS job=${PJM_JOBID:-?} ==="
echo "workdir=$WORK"
date
...
for t in 1 2 3 4 5; do
    ...
    echo "[cbatch192] topo try $t failed (1907?); retry"
    sleep 3
done
