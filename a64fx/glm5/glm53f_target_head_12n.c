...
glm53f_target_head_context_12n*glm53f_target_head_create_12n(const char*model){int rank,nr;glm53f_st_context*st;glm53f_target_head_context_12n*c;MPI_Comm_rank(MPI_COMM_WORLD,&rank);MPI_Comm_size(MPI_COMM_WORLD,&nr);if(nr!=12)return NULL;c=calloc(1,sizeof(*c));if(!c)return NULL;c->rank=rank;c->r0=(int)((long long)V*rank/nr);c->rn=(int)((long long)V*(rank+1)/nr)-c->r0;st=glm53f_st_open(model);if(!st)goto fail;c->norm=a256(H*2);if(glm53f_st_read(st,"model.language_model.norm.weight",0,c->norm,H*2))goto fail;c->head=a256((size_t)c->rn*H*2);if(glm53f_st_read(st,"lm_head.weight",(size_t)c->r0*H*2,c->head,(size_t)c->rn*H*2))goto fail;glm53f_st_close(st);c->hidden=a256(H*4);c->x=a256(H*4);c->logits=a256((size_t)c->rn*4);return c;fail:if(st)glm53f_st_close(st);glm53f_target_head_free_12n(c);return NULL;}
