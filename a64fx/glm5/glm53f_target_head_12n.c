...
struct glm53f_target_head_context_12n{int rank,r0,rn;uint16_t*norm,*head;float*q2_head,*hidden,*x,*logits;double phase[3];};
...
glm53f_target_head_context_12n*glm53f_target_head_create_with_norm_12n(const char*model,const char*norm_name){int rank,nr;glm53f_st_context*st=NULL;glm53f_target_head_context_12n*c;MPI_Comm_rank(MPI_COMM_WORLD,&rank);MPI_Comm_size(MPI_COMM_WORLD,&nr);if(nr!=12)return NULL;c=calloc(1,sizeof(*c));if(!c)return NULL;c->rank=rank;c->r0=(int)((long long)V*rank/nr);c->rn=(int)((long long)V*(rank+1)/nr)-c->r0;st=glm53f_st_open(model);if(!st)goto fail;c->norm=a256(H*2);if(glm53f_st_read(st,norm_name,0,c->norm,H*2))goto fail;const char*stage=getenv("GLM53F_Q2_HEAD_STAGE");if(stage&&*stage){char path[4096];FILE*f;snprintf(path,sizeof(path),"%s/rank%02d.f32",stage,rank);f=fopen(path,"rb");if(!f)goto fail;c->q2_head=a256((size_t)c->rn*H*4);size_t count=(size_t)c->rn*H;if(fread(c->q2_head,sizeof(float),count,f)!=count||fgetc(f)!=EOF){fclose(f);goto fail;}fclose(f);}else{c->head=a256((size_t)c->rn*H*2);if(glm53f_st_read(st,"lm_head.weight",(size_t)c->r0*H*2,c->head,(size_t)c->rn*H*2))goto fail;}glm53f_st_close(st);st=NULL;c->hidden=a256((size_t)5*H*4);c->x=a256((size_t)5*H*4);c->logits=a256((size_t)5*c->rn*4);return c;fail:if(st)glm53f_st_close(st);glm53f_target_head_free_12n(c);return NULL;}
...
#pragma omp parallel for schedule(static)
    for(int r=0;r<c->rn;r++)c->logits[r]=c->q2_head?dot_f32(c->q2_head+(size_t)r*H,c->x,H):dot_bf16(c->head+(size_t)r*H,c->x,H);in.value=-INFINITY;in.index=-1;for(int r=0;r<c->rn;r++){int id=c->r0+r;if(c->logits[r]>in.value||(c->logits[r]==in.value&&id<in.index)){in.value=c->logits[r];in.index=id;}}double t2=MPI_Wtime();int rc=MPI_Allreduce(&in,&best,1,MPI_FLOAT_INT,MPI_MAXLOC,MPI_COMM_WORLD);double t3=MPI_Wtime();c->phase[0]=t1-t0;c->phase[1]=t2-t1;c->phase[2]=t3-t2;*token=best.index;*value=best.value;return rc==MPI_SUCCESS?0:-1;}
int glm53f_target_head_argmax_batch_12n(glm53f_target_head_context_12n*c,const float*streams,int tokens,int*token,float*value){if(!c||!streams||!token||!value||tokens<1||tokens>5)return-1;struct pair{float value;int index;}in[5],best[5];double t0=MPI_Wtime();float invs[5];for(int t=0;t<tokens;t++){double ss=0;float*h=c->hidden+(size_t)t*H;const float*s=streams+(size_t)t*HC*H;
#pragma omp parallel for reduction(+:ss)
...
#pragma omp parallel for collapse(2) schedule(static)
    ...
    double t1=MPI_Wtime();if(c->q2_head){
#pragma omp parallel for collapse(2) schedule(static)
