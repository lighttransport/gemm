...
int main(int argc,char**argv){int rows=argc>1?atoi(argv[1]):4096,cols=argc>2?atoi(argv[2]):384;
    uint16_t*w=aligned_buffer((size_t)rows*cols*2);float*x=aligned_buffer((size_t)TOKENS*cols*4),*ref=aligned_buffer((size_t)TOKENS*rows*4),*bat=aligned_buffer((size_t)TOKENS*rows*4);if(rows<1||cols<1||!w||!x||!ref||!bat)return 2;
