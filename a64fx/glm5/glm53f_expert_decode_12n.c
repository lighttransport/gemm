...
int main(int argc, char **argv) {
    ...
    posix_memalign((void **)&up, 256, 8 * 1024 * sizeof(float));
    posix_memalign((void **)&act, 256, 8 * 512 * sizeof(float));
    posix_memalign((void **)&out, 256, 8 * 4096 * sizeof(float));
    ...
    if (!rank) printf("GLM53F_EXPERT_DECODE_12N tokens=%d layers=%d expert_parts_rank=%d weight_GiB_rank=%.3f max_tasks=%ld wall_ms_tok=%.3f compute_ms_tok=%.3f combine_ms_tok=%.3f wire_us_call=%.3f wire_ms_tok=%.3f arrival_ms_tok=%.3f tok_s=%.3f checksum=%.9g\n",
        tokens, NLAYERS, manifest_entries / (NLAYERS * 4), blob_bytes / 1073741824.0, max_tasks,
    ...
    free(sum); free(out); free(act); free(up); free(x); free(blob); free(table);
    MPI_Finalize();
    return 0;
}
