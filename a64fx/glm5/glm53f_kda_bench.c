...
int main(void) {
    ...
    float *state=NULL, *q=NULL, *k=NULL, *v=NULL, *decay=NULL, *out=NULL, *work=NULL;
    ...
    if(H<1||H>64||posix_memalign((void**)&state,256,bytes)||posix_memalign((void**)&q,256,(size_t)NH*D*4)||
       ...
       posix_memalign((void**)&decay,256,(size_t)NH*D*4)||
...
#pragma omp parallel for schedule(static)
    for(size_t i=0;i<(size_t)NH*D;i++) {
        q[i]=k[i]=v[i]=(float)((int)(i%17)-8)*0.01f;
        decay[i]=-.0025f*(float)(1+(i%D));
    }
    for(r=0;r<reps;r++) { double t=now_sec();
#pragma omp parallel for schedule(static)
        for(h=0;h<NH;h++) glm53f_kda_step_vec_streamed(state+(size_t)h*D*D,q+(size_t)h*D,k+(size_t)h*D,
            v+(size_t)h*D,decay+(size_t)h*D,0.5f,D,D,out+(size_t)h*D,work+(size_t)h*D);
    ...
    free(work);free(out);free(decay);free(v);free(k);free(q);free(state); return 0;
}
