...
int glm53f_mtp_forward_12n(glm53f_mtp_context_12n*c,int token,const float*hidden,int*draft,float*logit){if(!c||!hidden||!draft||!logit)return-1;if(glm53f_embedding_streams_12n(c->embedding,token,c->embed_streams))return-1;mtp_norm(c->pair,c->embed_streams,c->enorm);mtp_norm(c->pair+MTP_H,hidden,c->hnorm);
#pragma omp parallel for schedule(static)
...
#pragma omp parallel for schedule(static)
    for(int i=0;i<MTP_H;i++)c->fusion[i]+=c->sublayer[i];for(int s=0;s<MTP_STREAMS;s++)memcpy(c->head_streams+(size_t)s*MTP_H,c->fusion,MTP_H*4);return glm53f_target_head_argmax_12n(c->head,c->head_streams,draft,logit);}
