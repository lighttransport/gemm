...
int glm53f_sparse_sublayer_batch_12n(glm53f_sparse_context_12n*c,float*out,const float*x,int tokens){if(!c||!out||!x||tokens<1||tokens>4||c->length+tokens>c->capacity)return-1;for(int t=0;t<tokens;t++)if(glm53f_sparse_sublayer_12n(c,out+(size_t)t*H,x+(size_t)t*H))return-1;return 0;}
