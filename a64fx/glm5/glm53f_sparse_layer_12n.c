...
static int sparse_sublayer_local_no_reduce(glm53f_sparse_context_12n*c,float*out,const float*x){if(!c||sparse_attention_local(c,c->attn,x))return-1;int local_cols=c->hn*VD,local_blocks=local_cols/128;
    ...
    glm53f_mv_fp8_block128_bits(out,c->op,c->ops,c->attn,H,local_cols);
    return 0;}
int glm53f_sparse_sublayer_12n(void*context,float*out,const float*x){glm53f_sparse_context_12n*c=context;if(!c||!out||sparse_sublayer_local_no_reduce(c,c->partial,x))return-1;return glm53f_sum_allreduce_12n(c->partial,out,H);}
int glm53f_sparse_sublayer_batch_12n(glm53f_sparse_context_12n*c,float*out,const float*x,int tokens){if(!c||!out||!x||tokens<1||tokens>5||c->length+tokens>c->capacity)return-1;float *packed=a256((size_t)tokens*H*sizeof(float));if(!packed)return-1;for(int t=0;t<tokens;t++)if(sparse_sublayer_local_no_reduce(c,packed+(size_t)t*H,x+(size_t)t*H)){free(packed);return-1;}int rc=glm53f_sum_allreduce_12n(packed,out,tokens*H);free(packed);return rc;}
