/* Dequantize the Q2 GGUF input embedding into rank-local F32 row shards. */
#ifndef _GNU_SOURCE
#define _GNU_SOURCE
#endif
...
static void die(int rank, const char *what) {
    fprintf(stderr, "rank=%d glm53f_q2_embed_stage: %s: %s\n", rank, what,
...
int main(int argc, char **argv) {
    ...
    if (!stat(path, &st) && st.st_size == (off_t)((size_t)rows * HIDDEN * sizeof(float))) {
        printf("SENTINEL glm53f_q2_embed_stage=REUSE rank=%d rows=%d\n", rank, rows);
    ...
    for (uint64_t i = 0; i < g->n_tensors; ++i) {
        if (g->tensors[i].name.str && !strcmp(g->tensors[i].name.str, "token_embd.weight")) {
            ti = &g->tensors[i];
    ...
    if (!ti || ti->n_dims != 2 || ti->dims[0] != HIDDEN || ti->dims[1] != VOCAB)
        die(rank, "token_embd.weight contract");
    if (ti->type >= GGML_TYPE_COUNT || ggml_type_info[ti->type].block_size <= 0 ||
        HIDDEN % ggml_type_info[ti->type].block_size || !dequant_row)
        die(rank, "unsupported embedding type");
    ...
    if (ofd < 0) die(rank, "create staged embedding");
    ...
    for (int r0 = 0; r0 < rows; r0 += CHUNK_ROWS) {
        ...
        if (read_exact(fd, base + (uint64_t)(row0 + r0) * row_bytes, raw,
                       (size_t)nr * row_bytes)) die(rank, "read embedding rows");
        for (int r = 0; r < nr; ++r)
            if (dequant_row(ti->type, raw + (size_t)r * row_bytes,
                            out + (size_t)r * HIDDEN, HIDDEN))
                die(rank, "dequantize embedding row");
        size_t bytes = (size_t)nr * HIDDEN * sizeof(float);
        if (write(ofd, out, bytes) != (ssize_t)bytes) die(rank, "write staged embedding");
        hash ^= fnv1a(out, (size_t)nr * HIDDEN);
        hash *= UINT64_C(1099511628211);
    }
    if (fdatasync(ofd) || close(ofd) || rename(tmp, path)) die(rank, "commit staged embedding");
    FILE *mf = fopen(manifest, "w");
    if (!mf) die(rank, "write embedding manifest");
    fprintf(mf, "# GLM53F_Q2_EMBED_V1 rank=%d ranks=12 row0=%d rows=%d hidden=%d type=%s bytes=%zu fnv1a=%016" PRIx64 "\n",
            rank, row0, rows, HIDDEN, ggml_type_name(ti->type),
            (size_t)rows * HIDDEN * sizeof(float), hash);
    fclose(mf);
    printf("SENTINEL glm53f_q2_embed_stage=OK rank=%d row0=%d rows=%d type=%s bytes=%zu hash=%016" PRIx64 "\n",
