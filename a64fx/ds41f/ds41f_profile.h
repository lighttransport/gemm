...
#define DS41F_PROFILE_PHASES(X) \
    ...
    X(SPARSE_PACK) X(SPARSE_QK) X(SPARSE_SOFTMAX) X(SPARSE_PV) X(TP_COMM) X(TP_GROUP) X(INPUT_CACHE_HIT) X(INPUT_CACHE_MISS) \
    X(SHARED_W13) X(SHARED_W2) X(SHARED_REDUCE) X(ATTN_PREPACK) X(ATTN_ONLINE) X(HANDOFF_WAIT)
