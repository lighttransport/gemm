#!/bin/bash
# MiniMax-M3 CP (context-parallel KV) + int4-KV @ 1M context on 48 A64FX nodes (MXFP8 weights).
...
# Submit: ssh fugaku 'cd ~/work/gemm/ds4p && pjsub --no-check-directory a64fx/m3/pjsub_m3_cp_1m_48n.sh'
...
export M3_EP_SIZE=$NP M3_TP=1 M3_MSA=1 M3_CP=1 M3_INT4_KV=1
export LLM_THREADS=12 OMP_NUM_THREADS=12

echo "=== M3 CP+int4 1M 48n (4x4x3): NP=$NP job=${PJM_JOBID:-?} $(date) ==="
...
M3_REAL=1 M3_MAXPOS=1048576 M3_MAX_NEW=48 M3_PROMPT_IDS="$M3/prompt_ids.txt" M3_GEN_OUT="$M3/gen_cp_1m.txt" \
...
echo "--- Run1 gen ids ---"; cat "$M3/gen_cp_1m.txt" 2>/dev/null
