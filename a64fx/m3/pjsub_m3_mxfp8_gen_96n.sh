...
# Detokenize: python3 a64fx/m3/m3_tokenizer.py decode-file a64fx/m3/gen_mxfp8.txt
#
# Submit: ssh fugaku 'cd ~/work/gemm/ds4p && pjsub --no-check-directory a64fx/m3/pjsub_m3_mxfp8_gen_96n.sh'

#PJM -g hp250467
#PJM -L "rscgrp=small-s2,node=8x12:torus,elapse=01:30:00"
...
echo "=== M3-MXFP8 gen 96n (8x12): NP=$NP job=${PJM_JOBID:-?} $(date) ==="
...
echo "--- generate ($(date)) ---"
M3_REAL=1 M3_PROMPT_IDS="$M3/prompt_ids.txt" M3_GEN_OUT="$M3/gen_mxfp8.txt" \
...
echo "--- gen ids ---"; cat "$M3/gen_mxfp8.txt" 2>/dev/null
echo "=== done $(date) ==="
