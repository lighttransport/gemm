...
static inline int k3_prefill_gemm_bf16_pv(float *out,const float *x,int batch,
    ...
    for(int token0=0;token0<batch;token0+=256){int panel=batch-token0;
    if(panel>256)panel=256;int mblocks=(panel+K3_PREFILL_MR-1)/K3_PREFILL_MR;
