...
RSCGRP=${RSCGRP:-}
NODES=${NODES:-12}
ELAPSE=${ELAPSE:-06:00:00}

case "$FRONTEND_PORT" in
    ''|*[!0-9]*) echo "FRONTEND_PORT must be numeric" >&2; exit 2 ;;
esac

ssh "$REMOTE" bash -s -- "$REMOTE_REPO" "$JOB_SCRIPT" "$FRONTEND_PORT" \
    "$RSCGRP" "$NODES" "$ELAPSE" <<'REMOTE_SH'
set -euo pipefail
repo=$1
job_script=$2
frontend_port=$3
...
pjsub_args=(--no-check-directory)
if [[ -n "$rscgrp" ]]; then
    pjsub_args+=(-L "rscgrp=$rscgrp,node=$nodes,elapse=$elapse")
    echo "RESOURCE_OVERRIDE=rscgrp=$rscgrp,node=$nodes,elapse=$elapse"
fi
pjsub "${pjsub_args[@]}" \
