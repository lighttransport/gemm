// origin: a64fx/fused-gemm/pack_matrices.c (vendored snapshot for a64fx/vlm M1)
...
// Pack a single MR-row block of A — same layout as pack_A_fp32 produces
// for block `mb`, but callable from inside an OMP loop so the pack
// parallelises across threads instead of running serially before the GEMM.
void pack_A_fp32_block(int mb, int M, int K, int K_rounded,
    ...
    for (int k = 0; k < K; k++) {
        for (int m = 0; m < m_count; m++)
            dst[k * MR + m] = A[(size_t)(m_start + m) * lda + k];
        for (int m = m_count; m < MR; m++) dst[k * MR + m] = 0.0f;
    }
    for (int k = K; k < K_rounded; k++)
        for (int m = 0; m < MR; m++) dst[k * MR + m] = 0.0f;
}
