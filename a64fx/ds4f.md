...
| `DS4F_PREFILL_GEMM` / `DS4F_PREFILL_K` | 0 / 32 | 1 = batch gen prefill through the MHC+Tier-B2 verify path in chunks of K≤32 (comm ÷K, dense→M=K GEMM). **+33% prefill** (13.3→17.7 tok/s), COHERENT not bit-identical (GEMM reassoc); opt-in, not-with-MTP. See Decode-perf wins |
...
**Prefill (`9504cd69`, opt-in +33%):** production gen prefill (MHC+Tier-B2) ran token-by-token because the batched `ds4f_forward` GEMM is die-guarded under MHC/Tier-B2. `DS4F_PREFILL_GEMM` routes it through `ds4f_forward_verify` (the MHC+Tier-B2 batched forward built for spec decode — now Q8-correct + ATTN_GEMM'd) in chunks of K≤32: the per-layer EP all-reduce fires once per K tokens (comm 16.5→7.4%, ar_calls 216k→2.4k) and the dense projections become an M=K GEMM. Attn/tb2/mHC stay per-position (the non-amortizable floor). **13.28 → 17.72 tok/s (+33%)**, coherent (K-tile GEMM reassoc, seeds the same decode), NaN=0. The batched-verify path (which *failed* as a decode/spec lever on accept rate) pays off as a **prefill** lever.
