/*
 * Benchmark for polynomial exp2 kernel
 */
...
int main(int argc, char** argv) {
    ...
    printf("=== EXP2 Polynomial Kernel Benchmark ===\n");
    ...
    float* out_poly = aligned_alloc_safe(256, n * sizeof(float));
    ...
    exp2_poly_simple_v2(in_f, out_poly, n);
    ...
    for (int i = 0; i < n; i++) {
        int ulp = ulp_error(out_ref[i], out_poly[i]);
    ...
    printf("Correctness: %s\n", max_ulp < 100 ? "PASS" : (max_ulp < 1000 ? "ACCEPTABLE" : "FAIL"));

    /* Show a few samples */
    printf("\nSample values (first 10):\n");
    for (int i = 0; i < 10; i++) {
        printf("  in[%d]=%.4f: ref=%.6e, poly=%.6e, ulp=%d\n",
               i, in_f[i], out_ref[i], out_poly[i],
               ulp_error(out_ref[i], out_poly[i]));
    }
    printf("\n");
    ...
    exp2_poly_softmax_v2(in_i, out_poly, n, scale, max_val);
