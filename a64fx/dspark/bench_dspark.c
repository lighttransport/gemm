...
int main(int argc,char**argv){
    if(argc!=3){fprintf(stderr,"usage: %s DRAFT_DIR TARGET_DIR\n",argv[0]);return 2;}
    ...
    dspark_state*s=NULL;dspark_state_options so={8192};rc=dspark_state_create(&s,m,&so,err,sizeof(err));if(rc){fprintf(stderr,"state: %s\n",err);dspark_model_free(m);return 1;}
