...
static void run_sdot(float*out,const fp8_i8_activation*a,const fp8_i8_matrix*w){
 double dt[7];fp8_i8_gemv_sdot_omp(out,a,w,12);for(int j=0;j<7;++j){double t=now();fp8_i8_gemv_sdot_omp(out,a,w,12);dt[j]=now()-t;}
 for(int i=1;i<7;++i){double x=dt[i];int j=i;while(j&&dt[j-1]>x){dt[j]=dt[j-1];--j;}dt[j]=x;}double t=dt[3];
 printf("kernel=i8_sdot_i32acc median_ms=%.3f gflops=%.2f source_GB/s=%.2f checksum=%.7g\n",t*1e3,2.0*w->n*w->k/t/1e9,((size_t)w->n*w->k+(size_t)(w->n/128)*(w->k/128)*4)/t/1e9,out[w->n/3]);}
int main(int ac,char**av){int n=ac>1?atoi(av[1]):32768,k=ac>2?atoi(av[2]):4096;fp8_matrix w;if(fp8_matrix_alloc(&w,n,k))return 1;
 ...
 if(group==128&&lane_group==128){int act_group=ac>5?atoi(av[5]):128;fp8_i8_activation qa={0};t=now();
   if(fp8_i8_matrix_prepare_sdot(&wa)||fp8_i8_activation_prepare_group(&qa,a,k,act_group))return 1;
   printf("sdot_pack_and_activation_ms=%.3f act_G=%d\n",(now()-t)*1e3,act_group);run_sdot(out,&qa,&wa);
   fp8_i8_activation_free(&qa);}
