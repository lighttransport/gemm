...
extern "C" int pixal3d_write_ply(const char *path, const pixal3d_result *r) {
    std::string temporary;
    try {
        px::require(path && r && r->vertices && r->normals && r->uvs && r->triangles,
                    "Missing mesh attributes");
        px::require(r->vertex_count > 0 && r->triangle_count > 0, "Invalid PLY dimensions");
        px::require(size_t(r->vertex_count) <= std::numeric_limits<uint32_t>::max(),
                    "PLY vertex count exceeds uint32 indices");
        for (int i = 0; i < r->vertex_count; ++i)
            for (int c = 0; c < 3; ++c)
                px::require(std::isfinite(r->vertices[3 * i + c]) &&
                                std::isfinite(r->normals[3 * i + c]),
                            "Non-finite PLY vertex attribute");
        for (int i = 0; i < r->vertex_count * 2; ++i)
            px::require(std::isfinite(r->uvs[i]), "Non-finite PLY UV");
        for (int i = 0; i < r->triangle_count * 3; ++i)
            px::require(r->triangles[i] < uint32_t(r->vertex_count), "PLY index out of bounds");

        temporary = std::string(path) + ".partial";
        std::ofstream output(temporary, std::ios::binary);
        px::require(bool(output), "Cannot open PLY output");
        output << "ply\nformat binary_little_endian 1.0\n"
               << "comment generated by native Pixal3D\n"
               << "element vertex " << r->vertex_count << "\n"
               << "property float x\nproperty float y\nproperty float z\n"
               << "property float nx\nproperty float ny\nproperty float nz\n"
               << "property float s\nproperty float t\n"
               << "element face " << r->triangle_count << "\n"
               << "property list uchar uint vertex_indices\nend_header\n";
        for (int i = 0; i < r->vertex_count; ++i) {
            output.write(reinterpret_cast<const char *>(r->vertices + 3 * i), 3 * sizeof(float));
            output.write(reinterpret_cast<const char *>(r->normals + 3 * i), 3 * sizeof(float));
            output.write(reinterpret_cast<const char *>(r->uvs + 2 * i), 2 * sizeof(float));
        }
        for (int i = 0; i < r->triangle_count; ++i) {
            const uint8_t count = 3;
            output.write(reinterpret_cast<const char *>(&count), sizeof(count));
            output.write(reinterpret_cast<const char *>(r->triangles + 3 * i), 3 * sizeof(uint32_t));
        }
        output.close();
        px::require(bool(output), "Cannot write PLY");
        ...
        std::fprintf(stderr, "Pixal3D PLY: %s\n", e.what());
        return -1;
    }
}
