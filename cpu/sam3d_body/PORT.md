...
the PyTorch reference dump. DINOv3 C/CUDA inference is a FP32-activation
encoder with large matrices stored as F16 for throughput; it matches a
PyTorch float32-backbone reference tightly. The older upstream-config
BF16 dumps remain useful for measuring deployment drift, but they are
not the C math gate.
...
| DINOv3 encoder    | GREEN      | 2.08e-4 | 9.29e-7  | verify_dinov3 via `dinov3_encode_from_normalized` bypass (skips u8 round-trip) against a `--backbone-dtype float32` rectangular 512x384 ref. `gen_image_ref.py` now writes `backbone_dtype.txt`; float32 refs use a tight 1e-3 / 1e-5 default gate, while legacy BF16-config refs keep the looser deployment-drift gate. |
| DINOv3 encoder (CUDA) | GREEN  | 8.23e-5 | 6.68e-7  | `cuda/sam3d_body/verify_dinov3` (debug_set_normalized_input → run_encoder → diff (1,1280,32,24) ref). 32-block ViT forward in NVRTC: patch_embed_sam3d + prepend_special_tokens + pos_embed_add + per-block LN/QKV-gemm/rotate-half-RoPE-on-patches/KV-transpose/flash-attn/proj/layerscale-add(LS1)/LN/w1+w2/silu_mul/w3/layerscale-add(LS2) + final layernorm. F16 weights via `sb_upload_f16`; default precision is `fp16` and CUDA tracks the float32 PyTorch ref within the tight 1e-3 / 1e-5 gate. Explicit `--precision bf16` remains a diagnostic mode with a looser 1.5 / 1.5e-2 gate. |
...
| preprocess_image  | GREEN      | 4.77e-7 | 3.25e-8  | sam3d_body_preprocess_image now mirrors OpenCV `warpAffine(INTER_LINEAR)`'s fixed-point path (AB_BITS=10, INTER_BITS=5) before HWC→CHW + ImageNet norm. `verify_preprocess_image` accepts full `gen_image_ref.py` dumps and uses the rectangular DINOv3 aspect rule when diffing `dinov3_input.npy`. |
...
| runner self-driven | GREEN     | 3.17e-5 | 6.7e-6 | **Step 8c-ii complete.** sam3d_body_run_encoder computes TopdownAffine (compute_bbox_affine) + default cam_int (default_cam_int with optional focal_hint override) + OpenCV-exact preprocess_image → dinov3_encode_from_normalized. sam3d_body_run_decoder auto-populates dec_image_emb_chw (ray_cond_emb post the HW→CHW permute dropping CLS+4 register tokens and adding `prompt_encoder.no_mask_embed.weight`), dec_image_pe_chw (get_dense_pe), dec_init_x/dec_init_xpe (build_tokens from condition_info + init_pose + init_camera + invalid_prompt_token), and dec_cam_batch (cam_int / bbox_center / bbox_scale / ori_img_size / img_size / affine_trans / use_intrin_center=0 / default_scale_factor=1.0) when no debug-override is set. Rectangular DINOv3 ray conditioning now mirrors upstream's transposed `meshgrid(indexing="xy")` + flatten/reinterpret behavior; `verify_ray_cond_xyz` max_abs=1.79e-7. CPU raw residual is verts 3.17e-5, kp3d 3.15e-5, kp2d 9.95e-3 px. Raw-image DINOv3 mode defaults to 0.02 for verts/kp3d and 0.5 px for kp2d; reference-driven mode remains strict. |
| End-to-end (CUDA) | GREEN      | 3.20e-5 | 6.7e-6  | **Step 9 complete.** `cuda/sam3d_body/test_cuda_sam3d_body <sft> <img> --mhr-assets <dir> -o out.obj` produces V=18439 / F=36874 OBJ end-to-end via `cuda_sam3d_body_run_encoder` (host preprocess → on-device DINOv3) → `cuda_sam3d_body_run_decoder` (GPU ray_cond_emb + build_tokens + 6×decoder_layer + GPU norm_final+heads, with CPU MHR-in-the-loop). Encoder caches `self_{center,scale,warp,cam_int}` so decoder uses identical TopdownAffine values and adds the upstream no-mask prompt embedding before ray conditioning. CUDA raw residual is verts 3.20e-5, kp3d 3.15e-5, kp2d 9.46e-3 px. Cross-port equality remains strict; PyTorch raw-image parity uses the 0.02 / 0.5 px raw gate. |
...
| 2    | CPU DINOv3 encoder (verify_dinov3 green)      | float32-backbone refs max_abs ≤ 1e-3; mean ≤ 1e-5 |
| 3    | CUDA DINOv3 encoder                            | **GREEN (2026-05-12)**: 3a NVRTC+safetensors upload (32 blocks, D=1280, FFN=5120) + 3b 32-block ViT forward (LN→QKV-proj→rotate-half RoPE on Q/K patch slots→KV transpose→flash-attn→proj→LS1+res→LN→SwiGLU(w1·silu * w2 → w3)→LS2+res, final LN) + 3c verify_dinov3 (CUDA) green vs float32 PyTorch ref. Rectangular 512x384 max_abs=8.23e-5 / mean=6.68e-7; CPU is 2.08e-4 / 9.29e-7. |
...
| 8    | End-to-end CPU → .obj (verify_end_to_end green)| **GREEN (ref-driven + self-driven)**: ref-driven (8c-i + 8d) vs `body_out_vertices.npy` max_abs=8.9e-7 / `body_out_keypoints_3d.npy` max_abs=3.6e-7 / `body_out_keypoints_2d.npy` max_abs=3.1e-4 px. Self-driven DINOv3 raw-image mode defaults to 0.02 / 0.5 px gates and passes the 512x384 dump without manual threshold flags; ViT-H raw-image mode defaults to 0.15 / 200 px and passes the fixed-bbox dump. |
| 9    | End-to-end CUDA → .obj                         | **GREEN (2026-05-12)**: `test_cuda_sam3d_body` produces V=18439/F=36874 OBJ matching CPU port to float noise on the same input. `cuda/sam3d_body/verify_end_to_end` now uses the same raw-image default gates as CPU: DINOv3 0.02 / 0.5 px, ViT-H 0.15 / 200 px. Both pass their matching 512x384 dumps without manual threshold flags. Cross-port pitfalls fixed: host OpenCV-exact `sam3d_body_preprocess_image`, final `pred_vertices` Y/Z negation, rectangular DINOv3 ray-cond reinterpret, and the no-mask prompt embedding before ray conditioning. |
...
- **DINOv3 rectangular input refs.** DINOv3 uses RoPE rather than
  learned pos-embed interpolation. CPU `common/dinov3.h` and CUDA
  sam3d-body now accept patch-aligned rectangular DINOv3 inputs via
  explicit H/W plumbing. `ref/sam3d-body/gen_image_ref.py` can generate
  non-square dumps with `--image-width/--image-height`; tested
  384x512 (`dinov3_input.npy` = `(1,3,512,384)`,
  `dinov3_tokens.npy` = `(1,1280,32,24)`). The generator now defaults to
  `--backbone-dtype float32`, writes `backbone_dtype.txt`, and both CPU/CUDA
  `verify_dinov3` tighten automatically to 1e-3 / 1e-5 for those refs. The
  old upstream BF16-config refs explain the previous ~3.7e-1 max_abs outlier:
  PyTorch FP32 itself differs from PyTorch BF16 by that amount on this dump.
  Decoder self-input verification is green for the same dump after splitting
  dense PE/keypoint crop correction by backbone: ViT-H keeps its square-crop
  x coordinate convention, rectangular DINOv3 uses normal HxW grid
  normalization.
  Raw-image CPU/CUDA end-to-end on the corrected 512x384 dump now uses
  much tighter DINOv3-driven gates by default (0.02 / 0.5 px); the
  reference-driven decoder/MHR path remains at f32-floor gates. The
  previous 0.12 / 10 px raw drift was split between non-OpenCV-exact
