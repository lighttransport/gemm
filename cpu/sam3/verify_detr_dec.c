/* Diff sam3_runner's DETR decoder outputs vs reference dumps.
 *
 * Usage: verify_detr_dec --ckpt <sam3.model.safetensors> --image <img>
 *                        [--refdir /tmp/sam3_ref_cat]
 *
 * Runs ViT + FPN + text + DETR enc + DETR dec, diffing:
 *   pred_boxes.npy       (1, 200, 4) xyxy
 *   presence_logits.npy  (1, 1)      last-layer presence logit (scalar)
 */
...
int main(int argc, char **argv)
{
    ...
    fprintf(stderr, "running ViT + FPN + text + DETR enc + dec ...\n");
    ...
    if (sam3_run_detr_dec(ctx) != 0) return 9;

    const float *ours_boxes = sam3_get_detr_dec_boxes(ctx);
    const float *ours_pres  = sam3_get_detr_dec_presence(ctx);

    /* Boxes. */
    snprintf(path, sizeof(path), "%s/pred_boxes.npy", refdir);
    float *ref_boxes = (float *)read_npy(path, &nd, d, &esz);
    if (!ref_boxes) { fprintf(stderr, "cannot read pred_boxes.npy\n"); return 10; }
    size_t nb = (size_t)d[1] * d[2];  /* 200*4 */
    double mx = 0, sd = 0, sa = 0, sb = 0;
    for (size_t i = 0; i < nb; i++) {
        double diff = fabs((double)ref_boxes[i] - (double)ours_boxes[i]);
        if (diff > mx) mx = diff;
        sa += ref_boxes[i]; sb += ours_boxes[i]; sd += diff;
    }
    fprintf(stderr, "pred_boxes: max_abs=%.4e mean_abs=%.4e "
            "(ref mean=%.4e ours mean=%.4e)\n",
            mx, sd / nb, sa / nb, sb / nb);
    free(ref_boxes);

    /* Presence logits (last layer). */
    snprintf(path, sizeof(path), "%s/presence_logits.npy", refdir);
    float *ref_pres = (float *)read_npy(path, &nd, d, &esz);
    if (ref_pres) {
        fprintf(stderr, "presence_logits: ref=%.4e ours (last)=%.4e  "
                "per-layer ours=[%.4e,%.4e,%.4e,%.4e,%.4e,%.4e]\n",
                ref_pres[0], ours_pres[5],
                ours_pres[0], ours_pres[1], ours_pres[2],
                ours_pres[3], ours_pres[4], ours_pres[5]);
        free(ref_pres);
    }
