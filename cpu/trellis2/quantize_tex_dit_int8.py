#!/usr/bin/env python3
"""Quantize TRELLIS.2 Stage-3 texture DiT block linears to INT8 W8A8.

The emitted format mirrors the Qwen-Image SmoothQuant path:

  <base>.weight        I8   [out, in]
  <base>.weight_scale  F32  [out]
  <base>.smooth_scale  F32  [in]   (omitted with --no-smooth)

At runtime the HIP runner computes:
  Y = int8(X / smooth_scale) @ int8(W * smooth_scale)^T
      * xscale[token] * weight_scale[out] + bias
...
def main():
    ...
    ap.add_argument("src", help="BF16/F32 tex DiT safetensors")
    ap.add_argument("out", help="Output INT8 SmoothQuant safetensors")
    ap.add_argument("--calib", nargs="*", default=[], help="T2_TEX_CALIB_DUMP safetensors")
    ...
    with safe_open(args.src, framework="pt", device="cpu") as f:
        ...
        quant_keys = {
            ...
            for b in block_ids
            for suffix in TEX_BLOCK_LINEARS
        }
    ...
    metadata = {
        "format": "trellis2-tex-dit-int8-smoothquant-w8a8",
        "alpha": str(args.alpha),
        "smooth": "0" if args.no_smooth else "1",
        "linears": ",".join(TEX_BLOCK_LINEARS),
    }
    save_file(out, args.out, metadata=metadata)

    print(f"Quantized {n_quant} tex DiT block linears to INT8.", file=sys.stderr)
