...
int main(int argc,char **argv){
    ...
    int M=argc>2?atoi(argv[2]):1024,nthr=argc>3?atoi(argv[3]):32,ncmg=argc>4?atoi(argv[4]):2,zc=argc>5?atoi(argv[5]):1,w4a8=argc>6?atoi(argv[6]):1;setenv("DS4F_PREFILL_LAST_LOGITS","1",0);setenv("DS4F_PROF","1",0);
    ...
    m->gpu_dense_ctx=c;m->gpu_dense_gemm=cuda_ds4f_dense_gemm_tensor;m->gpu_shared_ffn=cuda_ds4f_dense_shared_ffn;m->gpu_oproj=cuda_ds4f_dense_oproj;m->gpu_dense_mixed=1;
    ...
    printf("phase ms/token:");for(int i=0;i<=DS4F_P_HEAD;i++)if(m->prof[i]>0)printf(" %s=%.3f",ds4f_prof_names[i],m->prof[i]*1000/M);printf("\n");
