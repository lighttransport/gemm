 ...
 * cuda_sam3d_debug_ss_dit_forward, isolating per-call DiT drift from
...
int main(int argc, char **argv)
{
    ...
    if (cuda_sam3d_ss_dit_info(ctx, &n_blocks, &dim, &cond_ch, &is_shortcut) != 0) {
        fprintf(stderr, "cuda_sam3d_ss_dit_info failed\n");
    ...
    int n_lat = cuda_sam3d_ss_dit_n_latents();
    if (n_lat != 5) {
    ...
    for (int i = 0; i < n_lat; i++) {
        ...
        int expect = cuda_sam3d_ss_dit_lat_elts(i);
    ...
    if (cuda_sam3d_debug_ss_dit_forward(ctx,
                                        (const float *const *)inputs,
                                        outputs,
                                        cond, n_cond_tokens,
                                        t, d) != 0) {
        fprintf(stderr, "cuda_sam3d_debug_ss_dit_forward failed\n");
