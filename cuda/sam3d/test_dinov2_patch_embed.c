...
int main(int argc, char **argv)
{
    ...
    CUfunction fn = NULL;
    if (cuModuleGetFunction(&fn, mod, "dinov2_patch_embed_f32") != CUDA_SUCCESS) {
    ...
    CUdeviceptr d_b   = cu_upload_raw(h_b,   (size_t)dim * sizeof(float));
    CUdeviceptr d_out = 0;
    ...
    cuCtxSynchronize();
    cuMemcpyDtoH(h_dst, d_out, n_out * sizeof(float));
    ...
    cuMemFree(d_img); cuMemFree(d_w); cuMemFree(d_b); cuMemFree(d_out);
    cuModuleUnload(mod);
    cuCtxDestroy(cu_ctx);
    return ok ? 0 : 7;
}
