...
int main(int argc, char **argv)
{
    ...
    CUfunction fn_index = NULL, fn_conv = NULL;
    if (cuModuleGetFunction(&fn_index, mod, "slat_build_coord_index64_i32") != CUDA_SUCCESS ||
    ...
    CUdeviceptr d_feats = cu_upload_raw(feats, (size_t)N * in_C * sizeof(float));
    CUdeviceptr d_weight = cu_upload_raw(weight, (size_t)out_C * 27 * in_C * sizeof(float));
    CUdeviceptr d_bias = cu_upload_raw(bias, (size_t)out_C * sizeof(float));
    CUdeviceptr d_index = 0, d_out = 0;
    if (!d_coords || !d_feats || !d_weight || !d_bias ||
        cuMemAlloc(&d_index, (size_t)64 * 64 * 64 * sizeof(int)) != CUDA_SUCCESS ||
        cuMemAlloc(&d_out, (size_t)N * out_C * sizeof(float)) != CUDA_SUCCESS) {
    ...
    if (cuLaunchKernel(fn_index, (N + 255) / 256, 1, 1, 256, 1, 1, 0, 0, idx_args, NULL) != CUDA_SUCCESS) {
    ...
    int total = N * out_C;
    if (cuLaunchKernel(fn_conv, (total + 255) / 256, 1, 1, 256, 1, 1, 0, 0, conv_args, NULL) != CUDA_SUCCESS) {
        fprintf(stderr, "conv launch failed\n");
        return 6;
    }
    cuCtxSynchronize();
    cuMemcpyDtoH(dst, d_out, (size_t)N * out_C * sizeof(float));
    ...
    if (repeat > 0) {
        ...
        for (int r = 0; r < repeat; r++) {
            if (cuMemsetD8(d_index, 0xff, (size_t)64 * 64 * 64 * sizeof(int)) != CUDA_SUCCESS) return 5;
            if (cuLaunchKernel(fn_index, (N + 255) / 256, 1, 1, 256, 1, 1, 0, 0, idx_args, NULL) != CUDA_SUCCESS) return 6;
            if (cuLaunchKernel(fn_conv, (total + 255) / 256, 1, 1, 256, 1, 1, 0, 0, conv_args, NULL) != CUDA_SUCCESS) return 6;
        }
        cuCtxSynchronize();
    ...
    cuMemFree(d_coords); cuMemFree(d_feats); cuMemFree(d_weight); cuMemFree(d_bias);
    cuMemFree(d_index); cuMemFree(d_out);
    cuModuleUnload(mod);
    cuCtxDestroy(cu_ctx);
    return ok ? 0 : 9;
}
