...
int main(int argc, char **argv)
{
    ...
    CUfunction fn_idx = NULL, fn_ln = NULL, fn_silu = NULL, fn_conv = NULL;
    CUfunction fn_modln = NULL, fn_resadd = NULL;
    if (cuModuleGetFunction(&fn_idx, mod, "slat_build_coord_index64_i32") != CUDA_SUCCESS ||
    ...
    CUdeviceptr d_nw = cu_upload_raw(nw, (size_t)C * sizeof(float));
    CUdeviceptr d_nb = cu_upload_raw(nb, (size_t)C * sizeof(float));
    CUdeviceptr d_shift = cu_upload_raw(shift, (size_t)C * sizeof(float));
    CUdeviceptr d_scale = cu_upload_raw(scale, (size_t)C * sizeof(float));
    CUdeviceptr d_w1 = cu_upload_raw(w1, WC * sizeof(float));
    CUdeviceptr d_b1 = cu_upload_raw(b1, (size_t)C * sizeof(float));
    CUdeviceptr d_w2 = cu_upload_raw(w2, WC * sizeof(float));
    CUdeviceptr d_b2 = cu_upload_raw(b2, (size_t)C * sizeof(float));
    CUdeviceptr d_index = 0, d_h1 = 0, d_h2 = 0, d_h3 = 0, d_h4 = 0;
...
#define RUN_PIPELINE() do { \
    ...
    if (cuLaunchKernel(fn_idx, (N + 255) / 256, 1, 1, 256, 1, 1, 0, 0, a_idx, NULL) != CUDA_SUCCESS) return 6; \
    void *a_ln[] = { &d_h1, &d_x, &d_nw, &d_nb, &N, &C, &eps, &affine }; \
    if (cuLaunchKernel(fn_ln, N, 1, 1, threads, 1, 1, (unsigned)ln_smem, 0, a_ln, NULL) != CUDA_SUCCESS) return 6; \
    void *a_s1[] = { &d_h1, &n_elem }; \
    if (cuLaunchKernel(fn_silu, (n_elem + 255) / 256, 1, 1, 256, 1, 1, 0, 0, a_s1, NULL) != CUDA_SUCCESS) return 6; \
    void *a_c1[] = { &d_coords, &d_h1, &d_index, &d_w1, &d_b1, &N, &C, &C, &d_h2 }; \
    if (cuLaunchKernel(fn_conv, (total_conv + 255) / 256, 1, 1, 256, 1, 1, 0, 0, a_c1, NULL) != CUDA_SUCCESS) return 6; \
    void *a_ml[] = { &d_h3, &d_h2, &d_shift, &d_scale, &N, &C, &eps }; \
    if (cuLaunchKernel(fn_modln, N, 1, 1, threads, 1, 1, (unsigned)m_smem, 0, a_ml, NULL) != CUDA_SUCCESS) return 6; \
    void *a_s2[] = { &d_h3, &n_elem }; \
    if (cuLaunchKernel(fn_silu, (n_elem + 255) / 256, 1, 1, 256, 1, 1, 0, 0, a_s2, NULL) != CUDA_SUCCESS) return 6; \
    void *a_c2[] = { &d_coords, &d_h3, &d_index, &d_w2, &d_b2, &N, &C, &C, &d_h4 }; \
    if (cuLaunchKernel(fn_conv, (total_conv + 255) / 256, 1, 1, 256, 1, 1, 0, 0, a_c2, NULL) != CUDA_SUCCESS) return 6; \
    void *a_ra[] = { &d_h4, &d_x, &n_elem }; \
    if (cuLaunchKernel(fn_resadd, (n_elem + 255) / 256, 1, 1, 256, 1, 1, 0, 0, a_ra, NULL) != CUDA_SUCCESS) return 6; \
} while (0)
...
#undef RUN_PIPELINE
    ...
    cuMemFree(d_coords); cuMemFree(d_x); cuMemFree(d_nw); cuMemFree(d_nb);
    cuMemFree(d_shift); cuMemFree(d_scale); cuMemFree(d_w1); cuMemFree(d_b1);
    cuMemFree(d_w2); cuMemFree(d_b2); cuMemFree(d_index);
