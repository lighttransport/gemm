 ...
 * are injected via cuda_sam3d_debug_override_dinov2 so this stage's
...
int main(int argc, char **argv)
{
    ...
    if (cuda_sam3d_debug_override_dinov2(ctx, dino, total_ntok, dino_dim) != 0) {
        fprintf(stderr, "debug_override_dinov2 failed\n");
        cuda_sam3d_destroy(ctx); free(dino); free(pmap); free(cond_ref);
        return 5;
    }
    free(dino);

    if (pmap) {
        if (cuda_sam3d_set_pointmap(ctx, pmap, pw, ph) != 0) {
            fprintf(stderr, "set_pointmap failed\n");
            cuda_sam3d_destroy(ctx); free(pmap); free(cond_ref);
            return 5;
        }
        free(pmap);
    }

    int rc = cuda_sam3d_run_cond_fuser(ctx);
    if (rc != 0) {
        fprintf(stderr, "cuda_sam3d_run_cond_fuser rc=%d\n", rc);
        cuda_sam3d_destroy(ctx); free(cond_ref); return 6;
    }

    int on = 0, oc = 0;
    cuda_sam3d_get_cond_tokens(ctx, NULL, &on, &oc);
    float *ours = (float *)malloc((size_t)on * oc * sizeof(float));
    cuda_sam3d_get_cond_tokens(ctx, ours, &on, &oc);
