...
"}\n"
"\n"
"static __device__ __forceinline__ unsigned short sam3d_f32_to_bf16_bits(float f) {\n"
"    unsigned int x = __float_as_uint(f);\n"
"    unsigned int r = 0x7fffu + ((x >> 16) & 1u);\n"
"    return (unsigned short)((x + r) >> 16);\n"
"}\n"
"extern \"C\" __global__ void cast_f32_to_f16_sam3d(const float *src, unsigned short *dst, int n) {\n"
...
"    dst[i] = h;\n"
"}\n"
"extern \"C\" __global__ void cast_f32_to_bf16_sam3d(const float *src, unsigned short *dst, int n) {\n"
"    int i = blockIdx.x * blockDim.x + threadIdx.x;\n"
"    if (i >= n) return;\n"
"    dst[i] = sam3d_f32_to_bf16_bits(src[i]);\n"
"}\n"
"extern \"C\" __global__ void add_bias_rows_f32(float *x, const float *b, int N, int D) {\n"
"    int i = blockIdx.x * blockDim.x + threadIdx.x;\n"
"    int total = N * D;\n"
"    if (i >= total) return;\n"
"    x[i] += b[i - (i / D) * D];\n"
...
/* flash_attn_sep_hd64_f32 — tiled fp32 attention for SS-DiT shape streams.
 *
 * Q/K/V are separate row-major [N, H*64] buffers. One warp computes one
 * query/head, while four warps in a block share K/V tiles.
 */
"#define CS3D_FA_WARPS 4\n"
"#define CS3D_FA_BKV 32\n"
"#define CS3D_FA_HD 64\n"
"#define CS3D_FA_EPT 2\n"
"extern \"C\" __global__ void flash_attn_sep_hd64_f32(\n"
"    float *__restrict__ out,\n"
"    const float *__restrict__ q, const float *__restrict__ k,\n"
"    const float *__restrict__ v,\n"
"    int N_q, int N_k, int H, int D_h, float scale) {\n"
"    int h = blockIdx.x;\n"
"    if (h >= H || D_h != CS3D_FA_HD) return;\n"
"    int warp_id = threadIdx.x >> 5;\n"
"    int lane = threadIdx.x & 31;\n"
"    int qi = (int)blockIdx.y * CS3D_FA_WARPS + warp_id;\n"
"    int E = H * D_h;\n"
"\n"
"    float q_r[CS3D_FA_EPT];\n"
"    for (int e = 0; e < CS3D_FA_EPT; e++) {\n"
"        int d = lane * CS3D_FA_EPT + e;\n"
"        q_r[e] = (qi < N_q) ? q[(size_t)qi * E + (size_t)h * D_h + d] : 0.0f;\n"
"    }\n"
"    float m_i = -1.0e30f;\n"
"    float l_i = 0.0f;\n"
"    float o_r[CS3D_FA_EPT];\n"
"    for (int e = 0; e < CS3D_FA_EPT; e++) o_r[e] = 0.0f;\n"
"\n"
"    extern __shared__ float smem[];\n"
"    float *smK = smem;\n"
"    float *smV = smem + CS3D_FA_BKV * CS3D_FA_HD;\n"
"    int n_threads = CS3D_FA_WARPS * 32;\n"
"    int n_tiles = (N_k + CS3D_FA_BKV - 1) / CS3D_FA_BKV;\n"
"    for (int tile = 0; tile < n_tiles; tile++) {\n"
"        int kv_base = tile * CS3D_FA_BKV;\n"
"        for (int idx = threadIdx.x; idx < CS3D_FA_BKV * CS3D_FA_HD; idx += n_threads) {\n"
"            int kj = idx / CS3D_FA_HD;\n"
"            int d = idx - kj * CS3D_FA_HD;\n"
"            int kv_tok = kv_base + kj;\n"
"            smK[idx] = (kv_tok < N_k) ? k[(size_t)kv_tok * E + (size_t)h * D_h + d] : 0.0f;\n"
"            smV[idx] = (kv_tok < N_k) ? v[(size_t)kv_tok * E + (size_t)h * D_h + d] : 0.0f;\n"
"        }\n"
"        __syncthreads();\n"
"        for (int kj = 0; kj < CS3D_FA_BKV; kj++) {\n"
"            float dot = 0.0f;\n"
"            for (int e = 0; e < CS3D_FA_EPT; e++)\n"
"                dot += q_r[e] * smK[kj * CS3D_FA_HD + lane * CS3D_FA_EPT + e];\n"
"            for (int off = 16; off > 0; off >>= 1)\n"
"                dot += __shfl_xor_sync(0xffffffffu, dot, off);\n"
"            float score = (kv_base + kj < N_k) ? dot * scale : -1.0e30f;\n"
"            float new_m = fmaxf(m_i, score);\n"
"            float alpha = expf(m_i - new_m);\n"
"            float p = expf(score - new_m);\n"
"            l_i = l_i * alpha + p;\n"
"            for (int e = 0; e < CS3D_FA_EPT; e++)\n"
"                o_r[e] = o_r[e] * alpha + p * smV[kj * CS3D_FA_HD + lane * CS3D_FA_EPT + e];\n"
"            m_i = new_m;\n"
"        }\n"
"        __syncthreads();\n"
"    }\n"
...
"        for (int e = 0; e < CS3D_FA_EPT; e++) {\n"
"            int d = lane * CS3D_FA_EPT + e;\n"
"            out[(size_t)qi * E + (size_t)h * D_h + d] = o_r[e] * inv_l;\n"
"        }\n"
"    }\n"
"}\n"
