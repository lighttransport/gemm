...
int main(int argc, char **argv)
{
    ...
    CUfunction fn_modln = NULL, fn_gemm = NULL, fn_gelu = NULL, fn_gated = NULL;
    ...
    CUdeviceptr d_w1 = cu_upload_raw(w1, (size_t)hidden * dim * sizeof(float));
    CUdeviceptr d_b1 = cu_upload_raw(b1, (size_t)hidden * sizeof(float));
    CUdeviceptr d_w2 = cu_upload_raw(w2, (size_t)dim * hidden * sizeof(float));
    CUdeviceptr d_b2 = cu_upload_raw(b2, (size_t)dim * sizeof(float));
    CUdeviceptr d_h = 0, d_mh = 0, d_mh2 = 0;
...
#define RUN_MLP() do { \
    ...
    if (cuLaunchKernel(fn_modln, N, 1, 1, threads, 1, 1, (unsigned)smem, 0, a_ln, NULL) != CUDA_SUCCESS) return 6; \
    ...
    if (cuLaunchKernel(fn_gemm, gx2, gy2, 1, 16, 16, 1, 0, 0, a_fc2, NULL) != CUDA_SUCCESS) return 6; \
...
#undef RUN_MLP
    ...
    cuMemFree(d_gate); cuMemFree(d_w1); cuMemFree(d_b1); cuMemFree(d_w2);
    cuMemFree(d_b2); cuMemFree(d_h); cuMemFree(d_mh); cuMemFree(d_mh2);
    cuModuleUnload(mod);
    cuCtxDestroy(cu_ctx);
    return ok ? 0 : 9;
}
