 ...
 *
 * Single-header. Define CUDA_SAM3D_PPE_GPU_IMPLEMENTATION in exactly
 * one translation unit before including. Mirrors the shape of
 * cuda_sam3d_dinov2_gpu.h but for the small PPE module.
 *
...
#ifndef CUDA_SAM3D_PPE_GPU_H_
#define CUDA_SAM3D_PPE_GPU_H_

#include <stddef.h>
#include "../cuew.h"
#include "../../common/sam3d_cond_fuser.h"  /* sam3d_ppe_model */
#include "cuda_sam3d_ppe_forward.h"          /* cs3d_ppe_w     */
...
#endif /* CUDA_SAM3D_PPE_GPU_H_ */

/* ============================ implementation ============================ */
#ifdef CUDA_SAM3D_PPE_GPU_IMPLEMENTATION
                            ...
                            CUdeviceptr *out_d, size_t *out_bytes)
{
    ...
    free(buf);
    if (!d) {
        fprintf(stderr, "cs3d_ppe_gpu: cuMemAlloc failed for %s (%zu bytes)\n",
                name, nb);
        return -1;
    }
    *out_d = d;
...
#endif /* CUDA_SAM3D_PPE_GPU_IMPLEMENTATION */
