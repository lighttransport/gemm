 ...
 *
 * Phase 1a: cuda_sam3d_run_dinov2 currently wraps the CPU encoder
...
int main(int argc, char **argv)
{
    ...
    if (cuda_sam3d_set_image_rgba(ctx, rgba, iw, ih) != 0 ||
        (msk && cuda_sam3d_set_mask(ctx, msk, mw, mh) != 0)) {
    ...
    int rc = cuda_sam3d_run_dinov2(ctx);
    if (rc != 0) {
        fprintf(stderr, "cuda_sam3d_run_dinov2 rc=%d\n", rc);
        free(ref); cuda_sam3d_destroy(ctx); return 6;
    }

    int out_n = 0, out_c = 0;
    cuda_sam3d_get_dinov2_tokens(ctx, NULL, &out_n, &out_c);
    float *ours = (float *)malloc((size_t)out_n * out_c * sizeof(float));
    cuda_sam3d_get_dinov2_tokens(ctx, ours, &out_n, &out_c);
