 ...
 * is injected via cuda_sam3d_debug_override_ss_latent so this stage's
...
int main(int argc, char **argv)
{
    ...
    if (!ctx) {
        fprintf(stderr, "cuda_sam3d_create failed\n");
        free(nchw); free(ref); return 5;
    }

    const int lat_dims[4] = {8, 16, 16, 16};
    if (cuda_sam3d_debug_override_ss_latent(ctx, nchw, lat_dims) != 0) {
        fprintf(stderr, "debug_override_ss_latent failed\n");
        cuda_sam3d_destroy(ctx); free(nchw); free(ref); return 5;
    }
    free(nchw);

    if (cuda_sam3d_run_ss_decode(ctx) != 0) {
        fprintf(stderr, "cuda_sam3d_run_ss_decode failed\n");
        cuda_sam3d_destroy(ctx); free(ref); return 6;
    }

    int out_dims[3] = {0};
    cuda_sam3d_get_occupancy(ctx, NULL, out_dims);
    size_t on = (size_t)out_dims[0] * out_dims[1] * out_dims[2];
    float *out = (float *)malloc(on * sizeof(float));
    if (!out) { cuda_sam3d_destroy(ctx); free(ref); return 5; }
    cuda_sam3d_get_occupancy(ctx, out, out_dims);
