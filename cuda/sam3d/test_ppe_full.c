...
int main(int argc, char **argv)
{
    ...
    /* ---- Device path. ---- */
    CUdeviceptr d_pmap  = cu_upload_raw(h_pmap, n_pmap * sizeof(float));
    CUdeviceptr d_lin_w = cu_upload_raw(h_lin_w, n_lin_w * sizeof(float));
    CUdeviceptr d_lin_b = cu_upload_raw(h_lin_b, (size_t)D * sizeof(float));
    CUdeviceptr d_inv   = cu_upload_raw(h_inv, (size_t)D * sizeof(float));
    CUdeviceptr d_cls   = cu_upload_raw(h_cls, (size_t)D * sizeof(float));
    CUdeviceptr d_pew   = cu_upload_raw(h_pew, n_pew * sizeof(float));
    CUdeviceptr d_ln1w  = cu_upload_raw(h_ln1w, (size_t)D * sizeof(float));
    CUdeviceptr d_ln1b  = cu_upload_raw(h_ln1b, (size_t)D * sizeof(float));
    CUdeviceptr d_qkvw  = cu_upload_raw(h_qkvw, n_qkvw * sizeof(float));
    CUdeviceptr d_qkvb  = cu_upload_raw(h_qkvb, (size_t)(3*D) * sizeof(float));
    CUdeviceptr d_projw = cu_upload_raw(h_projw, n_projw * sizeof(float));
    CUdeviceptr d_projb = cu_upload_raw(h_projb, (size_t)D * sizeof(float));
    CUdeviceptr d_ln2w  = cu_upload_raw(h_ln2w, (size_t)D * sizeof(float));
    CUdeviceptr d_ln2b  = cu_upload_raw(h_ln2b, (size_t)D * sizeof(float));
    ...
    CUdeviceptr d_fc2b  = cu_upload_raw(h_fc2b, (size_t)D * sizeof(float));
    ...
    cuMemFree(d_pmap); cuMemFree(d_lin_w); cuMemFree(d_lin_b); cuMemFree(d_inv);
    cuMemFree(d_cls); cuMemFree(d_pew);
    cuMemFree(d_ln1w); cuMemFree(d_ln1b); cuMemFree(d_qkvw); cuMemFree(d_qkvb);
    cuMemFree(d_projw); cuMemFree(d_projb);
