 ...
 *   (a) the runner's `cuda_sam3d_run_ss_dit` (now GPU-driven)
...
int main(int argc, char **argv)
{
    ...
    if (cuda_sam3d_debug_override_cond(ctx, cond, N_c, cond_ch) != 0) {
        fprintf(stderr, "override_cond failed\n");
        cuda_sam3d_destroy(ctx); free(cond); return 4;
    }
    fprintf(stderr, "[verify_ss_dit_ode] running GPU ODE (steps=%d)...\n", steps);
    if (cuda_sam3d_run_ss_dit(ctx) != 0) {
        fprintf(stderr, "GPU run_ss_dit failed\n");
        cuda_sam3d_destroy(ctx); free(cond); return 5;
    }
    int dims[4] = {0};
    int n_lat_dims = 4;
    int gpu_numel = 8 * 16 * 16 * 16;
    float *gpu_lat = (float *)malloc((size_t)gpu_numel * sizeof(float));
    if (cuda_sam3d_get_ss_latent(ctx, gpu_lat, dims) != 0) {
        fprintf(stderr, "get_ss_latent failed\n");
        free(gpu_lat); cuda_sam3d_destroy(ctx); free(cond); return 6;
    }
    (void)n_lat_dims;
    ...
    if (!w) {
        ...
        free(gpu_lat); cuda_sam3d_destroy(ctx); free(cond); return 7;
    }
    ...
    if (sam3d_cpu_ss_dit_run_ode(w, cond, N_c, steps, seed, cfg_scale,
        ...
        cuda_sam3d_destroy(ctx); free(cond); return 8;
    }
