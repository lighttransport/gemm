...
int main(int argc, char **argv)
{
    ...
    CUfunction fn = 0;
    if (cuModuleGetFunction(&fn, mod, "timestep_embed_cossin_f32") != CUDA_SUCCESS) {
    ...
    CUdeviceptr d_out = 0;
    if (cuMemAlloc(&d_out, (size_t)freq_dim * sizeof(float)) != CUDA_SUCCESS) return 5;
    ...
    for (int k = 0; k < n_ts; k++) {
        ...
        cuCtxSynchronize();
        cuMemcpyDtoH(h_dst, d_out, (size_t)freq_dim * sizeof(float));
