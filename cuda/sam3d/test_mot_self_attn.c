 ...
 * concat buffers via cuMemcpyDtoD rather than adding a concat kernel —
...
int main(int argc, char **argv)
{
    ...
    CUfunction fn_gemm = 0, fn_split = 0, fn_rms = 0, fn_sdpa = 0;
    ...
    if (cuModuleGetFunction(&fn_rms,   mod, "multi_head_rmsnorm_f32") != CUDA_SUCCESS) return 4;
    ...
    CUdeviceptr d_sqkvw = cu_upload_raw(sw_s.qkv_w, (size_t)qkv_dim * dim * sizeof(float));
    CUdeviceptr d_sqkvb = cu_upload_raw(sw_s.qkv_b, (size_t)qkv_dim * sizeof(float));
    CUdeviceptr d_pqkvw = cu_upload_raw(sw_p.qkv_w, (size_t)qkv_dim * dim * sizeof(float));
    CUdeviceptr d_pqkvb = cu_upload_raw(sw_p.qkv_b, (size_t)qkv_dim * sizeof(float));
    CUdeviceptr d_sgq   = cu_upload_raw(sw_s.gamma_q, (size_t)dim * sizeof(float));
    CUdeviceptr d_sgk   = cu_upload_raw(sw_s.gamma_k, (size_t)dim * sizeof(float));
    CUdeviceptr d_pgq   = cu_upload_raw(sw_p.gamma_q, (size_t)dim * sizeof(float));
    CUdeviceptr d_pgk   = cu_upload_raw(sw_p.gamma_k, (size_t)dim * sizeof(float));
    CUdeviceptr d_soutw = cu_upload_raw(sw_s.out_w, (size_t)dim * dim * sizeof(float));
    CUdeviceptr d_soutb = cu_upload_raw(sw_s.out_b, (size_t)dim * sizeof(float));
    CUdeviceptr d_poutw = cu_upload_raw(sw_p.out_w, (size_t)dim * dim * sizeof(float));
    CUdeviceptr d_poutb = cu_upload_raw(sw_p.out_b, (size_t)dim * sizeof(float));

    CUdeviceptr d_qkvs = 0, d_qkvp = 0;
    CUdeviceptr d_qs = 0, d_ks = 0, d_vs = 0;
    CUdeviceptr d_qp = 0, d_kp = 0, d_vp = 0;
    CUdeviceptr d_kpkv = 0, d_vpkv = 0;
    CUdeviceptr d_hs = 0, d_hp = 0, d_os = 0, d_op = 0;
    cuMemAlloc(&d_qkvs, (size_t)N_s * qkv_dim * sizeof(float));
    cuMemAlloc(&d_qkvp, (size_t)N_p * qkv_dim * sizeof(float));
    ...
    cuMemAlloc(&d_os, (size_t)N_s * dim * sizeof(float));
    cuMemAlloc(&d_op, (size_t)N_p * dim * sizeof(float));
    ...
    /* split. */
    {
        ...
        if (cuLaunchKernel(fn_split, grid, 1, 1, 256, 1, 1, 0, 0, args, NULL) != CUDA_SUCCESS) return 5;
    }
    {
    ...
    int stride = dim;
    {
        ...
        if (cuLaunchKernel(fn_rms, (unsigned)H, (unsigned)N_s, 1, threads, 1, 1, (unsigned)smem, 0, args, NULL) != CUDA_SUCCESS) return 5;
    }
    {
    ...
    }
    {
        ...
        if (cuLaunchKernel(fn_rms, (unsigned)H, (unsigned)N_p, 1, threads, 1, 1, (unsigned)smem, 0, args, NULL) != CUDA_SUCCESS) return 5;
    }
    {
    ...
    unsigned sdpa_threads = 256;
    {
        ...
        if (cuLaunchKernel(fn_sdpa, (unsigned)N_s, (unsigned)H, 1,
                           sdpa_threads, 1, 1, (unsigned)sm, 0, args, NULL) != CUDA_SUCCESS) return 5;
    }
    {
        size_t sm = (sdpa_threads + (size_t)N_kv) * sizeof(float);
        void *args[] = { &d_hp, &d_qp, &d_kpkv, &d_vpkv, &N_p, &N_kv, &H, &D_h, &scale };
        if (cuLaunchKernel(fn_sdpa, (unsigned)N_p, (unsigned)H, 1,
                           sdpa_threads, 1, 1, (unsigned)sm, 0, args, NULL) != CUDA_SUCCESS) return 5;
    }
    /* out_proj. */
    {
    ...
    cuMemcpyDtoH(o_s_dst, d_os, (size_t)N_s * dim * sizeof(float));
    cuMemcpyDtoH(o_p_dst, d_op, (size_t)N_p * dim * sizeof(float));
    ...
    cuMemFree(d_xs); cuMemFree(d_xp);
    cuMemFree(d_sqkvw); cuMemFree(d_sqkvb); cuMemFree(d_pqkvw); cuMemFree(d_pqkvb);
    cuMemFree(d_sgq); cuMemFree(d_sgk); cuMemFree(d_pgq); cuMemFree(d_pgk);
    cuMemFree(d_soutw); cuMemFree(d_soutb); cuMemFree(d_poutw); cuMemFree(d_poutb);
    ...
    cuMemFree(d_hs); cuMemFree(d_hp); cuMemFree(d_os); cuMemFree(d_op);
    cuModuleUnload(mod);
    cuCtxDestroy(cu_ctx);
    return ok ? 0 : 9;
}
