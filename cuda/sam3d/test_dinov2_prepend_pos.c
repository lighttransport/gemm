...
int main(int argc, char **argv)
{
    ...
    CUfunction fn_prep = NULL, fn_pos = NULL;
    if (cuModuleGetFunction(&fn_prep, mod, "dinov2_prepend_cls_reg_f32") != CUDA_SUCCESS) {
        fprintf(stderr, "lookup dinov2_prepend_cls_reg_f32 failed\n"); return 4;
    }
    if (cuModuleGetFunction(&fn_pos, mod, "dinov2_add_pos_embed_f32") != CUDA_SUCCESS) {
    ...
    /* ---- Device path. ---- */
    CUdeviceptr d_buf = cu_upload_raw(h_buf_init, n_buf * sizeof(float));
    CUdeviceptr d_cls = cu_upload_raw(h_cls,      (size_t)dim * sizeof(float));
    CUdeviceptr d_reg = h_reg ? cu_upload_raw(h_reg, (size_t)nr * dim * sizeof(float)) : 0;
    ...
    cuCtxSynchronize();
    cuMemcpyDtoH(h_dst, d_buf, n_buf * sizeof(float));
    ...
    cuMemFree(d_buf); cuMemFree(d_cls); if (d_reg) cuMemFree(d_reg); cuMemFree(d_pe);
    cuModuleUnload(mod);
    cuCtxDestroy(cu_ctx);
    return ok ? 0 : 7;
}
