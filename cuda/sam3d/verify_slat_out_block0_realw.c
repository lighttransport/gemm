/* verify_slat_out_block0_realw - real-weight SLAT out_blocks[0] CUDA gate. */
...
int main(int argc, char **argv) {
    ...
    const sam3d_slat_io_block *bk = &m->out_blocks[0];
    int C_in = bk->C_in;
    int C_out = bk->C_out;
    int dim = m->dim;
    if (bk->updown != SAM3D_SLAT_UPDOWN_UP || !bk->has_skip || C_in <= 0 || C_out <= 0) {
        fprintf(stderr, "unexpected out block0 shape: C_in=%d C_out=%d updown=%d skip=%d\n",
    ...
    char path[512];
    int nd = 0, dims[8] = {0}, is_f32 = 0;
    snprintf(path, sizeof(path), "%s/c_h_after_block_23.npy", refdir);
    ...
    if (!skip || !is_f32 || nd != 2 || dims[0] != N_src) { fprintf(stderr, "bad %s\n", path); return 4; }
    int C_skip = dims[1];
    if (C_x + C_skip != C_in) {
    ...
    int32_t *coords_src = NULL;
    if (load_coords(path, N_src, &coords_src) < 0) return 4;

    snprintf(path, sizeof(path), "%s/c_coords_after_input_block_0.npy", refdir);
    int32_t *coords_dst = NULL;
    int N_dst = load_coords(path, 0, &coords_dst);
    if (N_dst <= 0) return 4;

    snprintf(path, sizeof(path), "%s/c_h_after_out_block_0.npy", refdir);
    float *ref = (float *)npy_load(path, &nd, dims, &is_f32);
    if (!ref || !is_f32 || nd != 2 || dims[0] != N_dst || dims[1] != C_out) { fprintf(stderr, "bad %s\n", path); return 4; }
    ...
    float *cat = (float *)malloc((size_t)N_src * C_in * sizeof(float));
    float *dst = (float *)malloc((size_t)N_dst * C_out * sizeof(float));
    if (!cat || !dst) return 5;
    ...
    if (cu_compile_kernels(&mod, dev, cuda_sam3d_kernel_src,
                           ...
                           "verify_slat_out_block0_realw") < 0) return 7;

    CUfunction fn_gemm = NULL, fn_up = NULL, fn_idx = NULL, fn_ln = NULL, fn_silu = NULL;
    CUfunction fn_conv = NULL, fn_modln = NULL, fn_resadd = NULL;
    if (cuModuleGetFunction(&fn_gemm, mod, "gemm_f32_bias") != CUDA_SUCCESS ||
        cuModuleGetFunction(&fn_up, mod, "slat_upsample2_nearest_f32") != CUDA_SUCCESS ||
    ...
    size_t X_src = (size_t)N_src * C_in;
    size_t X_up = (size_t)N_dst * C_in;
    size_t X_out = (size_t)N_dst * C_out;
    CUdeviceptr d_coords_src = cu_upload_raw(coords_src, (size_t)N_src * 4 * sizeof(int32_t));
    CUdeviceptr d_coords_dst = cu_upload_raw(coords_dst, (size_t)N_dst * 4 * sizeof(int32_t));
    CUdeviceptr d_cat = cu_upload_raw(cat, X_src * sizeof(float));
    CUdeviceptr d_t = cu_upload_raw(t_emb, (size_t)dim * sizeof(float));
    CUdeviceptr d_up = 0, d_index = 0, d_t_silu = 0, d_emb = 0, d_skip = 0, d_h1 = 0, d_h2 = 0, d_h3 = 0, d_h4 = 0;
    if (!d_coords_src || !d_coords_dst || !d_cat || !d_t ||
        cuMemAlloc(&d_up, X_up * sizeof(float)) != CUDA_SUCCESS ||
        ...
        cuMemAlloc(&d_skip, X_out * sizeof(float)) != CUDA_SUCCESS ||
        cuMemAlloc(&d_h1, X_up * sizeof(float)) != CUDA_SUCCESS ||
        cuMemAlloc(&d_h2, X_out * sizeof(float)) != CUDA_SUCCESS ||
        cuMemAlloc(&d_h3, X_out * sizeof(float)) != CUDA_SUCCESS ||
        cuMemAlloc(&d_h4, X_out * sizeof(float)) != CUDA_SUCCESS)
        return 5;
    ...
    int twoCout = 2 * C_out;
    int one = 1;
    int n_elem_in = N_dst * C_in;
    int n_elem_out = N_dst * C_out;
    int total_conv = N_dst * C_out;
    unsigned threads = 256;
    size_t ln_smem = 2 * threads * sizeof(float);
    CUdeviceptr d_scale = d_emb;
    CUdeviceptr d_shift = d_emb + (size_t)C_out * sizeof(float);

#define RUN_OUT_BLOCK0() do { \
    void *a_up[] = { &d_coords_src, &d_cat, &N_src, &C_in, &d_coords_dst, &N_dst, &d_up }; \
    if (cuLaunchKernel(fn_up, (N_dst + 127) / 128, 1, 1, 128, 1, 1, 0, 0, a_up, NULL) != CUDA_SUCCESS) return 8; \
    ...
    void *a_idx[] = { &d_coords_dst, &N_dst, &d_index }; \
    if (cuLaunchKernel(fn_idx, (N_dst + 255) / 256, 1, 1, 256, 1, 1, 0, 0, a_idx, NULL) != CUDA_SUCCESS) return 8; \
    ...
    { unsigned gx = (N_dst + 15) / 16, gy = (C_out + 15) / 16; void *a[] = { &d_skip, &d_up, &d_sw, &d_sb, &N_dst, &C_in, &C_out }; \
      if (cuLaunchKernel(fn_gemm, gx, gy, 1, 16, 16, 1, 0, 0, a, NULL) != CUDA_SUCCESS) return 8; } \
    void *a_ln[] = { &d_h1, &d_up, &d_nw, &d_nb, &N_dst, &C_in, &eps, &affine }; \
    if (cuLaunchKernel(fn_ln, N_dst, 1, 1, threads, 1, 1, (unsigned)ln_smem, 0, a_ln, NULL) != CUDA_SUCCESS) return 8; \
    void *a_s1[] = { &d_h1, &n_elem_in }; \
    if (cuLaunchKernel(fn_silu, (n_elem_in + 255) / 256, 1, 1, 256, 1, 1, 0, 0, a_s1, NULL) != CUDA_SUCCESS) return 8; \
    void *a_c1[] = { &d_coords_dst, &d_h1, &d_index, &d_w1, &d_b1, &N_dst, &C_in, &C_out, &d_h2 }; \
    if (cuLaunchKernel(fn_conv, (total_conv + 255) / 256, 1, 1, 256, 1, 1, 0, 0, a_c1, NULL) != CUDA_SUCCESS) return 8; \
    void *a_ml[] = { &d_h3, &d_h2, &d_shift, &d_scale, &N_dst, &C_out, &eps }; \
    if (cuLaunchKernel(fn_modln, N_dst, 1, 1, threads, 1, 1, (unsigned)ln_smem, 0, a_ml, NULL) != CUDA_SUCCESS) return 8; \
    void *a_s2[] = { &d_h3, &n_elem_out }; \
    if (cuLaunchKernel(fn_silu, (n_elem_out + 255) / 256, 1, 1, 256, 1, 1, 0, 0, a_s2, NULL) != CUDA_SUCCESS) return 8; \
    void *a_c2[] = { &d_coords_dst, &d_h3, &d_index, &d_w2, &d_b2, &N_dst, &C_out, &C_out, &d_h4 }; \
    ...
    RUN_OUT_BLOCK0();
    cuCtxSynchronize();
    if (cuMemcpyDtoH(dst, d_h4, X_out * sizeof(float)) != CUDA_SUCCESS)
        return 8;
    ...
    if (repeat > 0) {
        cuCtxSynchronize();
        double t0 = now_ms();
        for (int r = 0; r < repeat; r++) RUN_OUT_BLOCK0();
        cuCtxSynchronize();
        avg_ms = (now_ms() - t0) / (double)repeat;
    }
#undef RUN_OUT_BLOCK0

    fprintf(stderr,
            "[verify_slat_out_block0_realw] Nsrc=%d Ndst=%d C_in=%d C_out=%d dim=%d max_abs=%.6e mean_abs=%.6e avg=%.4f ms x%d %s (threshold %.1g)\n",
            N_src, N_dst, C_in, C_out, dim, (double)mx, mean,
            avg_ms, repeat, ok ? "OK" : "FAIL", (double)threshold);
    if (bad) fprintf(stderr, "[verify_slat_out_block0_realw] nonfinite_diffs=%d\n", bad);

    cuMemFree(d_coords_src); cuMemFree(d_coords_dst); cuMemFree(d_cat); cuMemFree(d_t); cuMemFree(d_up);
    cuMemFree(d_index); cuMemFree(d_t_silu); cuMemFree(d_emb); cuMemFree(d_skip);
    cuMemFree(d_h1); cuMemFree(d_h2); cuMemFree(d_h3); cuMemFree(d_h4);
    ...
    cuModuleUnload(mod);
    cuCtxDestroy(cu_ctx);
    free(x); free(skip); free(coords_src); free(coords_dst); free(ref); free(t_emb); free(cat); free(dst);
