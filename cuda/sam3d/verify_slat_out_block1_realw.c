...
int main(int argc, char **argv) {
    ...
    CUdeviceptr d_x = cu_upload_raw(cat, X_in * sizeof(float));
    CUdeviceptr d_t = cu_upload_raw(t_emb, (size_t)dim * sizeof(float));
    CUdeviceptr d_index = 0, d_t_silu = 0, d_emb = 0, d_skip = 0, d_h1 = 0, d_h2 = 0, d_h3 = 0, d_h4 = 0;
    if (!d_coords || !d_x || !d_t ||
        ...
        cuMemAlloc(&d_h1, X_in * sizeof(float)) != CUDA_SUCCESS ||
...
#undef RUN_OUT_BLOCK1
    ...
    cuMemFree(d_coords); cuMemFree(d_x); cuMemFree(d_t); cuMemFree(d_index); cuMemFree(d_t_silu);
    cuMemFree(d_emb); cuMemFree(d_skip); cuMemFree(d_h1); cuMemFree(d_h2); cuMemFree(d_h3); cuMemFree(d_h4);
