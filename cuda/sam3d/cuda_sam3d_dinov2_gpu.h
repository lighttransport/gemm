 ...
 *
 * Single-header. Define CUDA_SAM3D_DINOV2_GPU_IMPLEMENTATION in
 * exactly one translation unit before including. Owns one CUdeviceptr
 ...
 * so each load is a direct safetensors mmap → host_ptr → cuMemcpyHtoD.
 * No dequant pass.
 */

#ifndef CUDA_SAM3D_DINOV2_GPU_H_
#define CUDA_SAM3D_DINOV2_GPU_H_
...
typedef struct {
    CUdeviceptr norm1_w,  norm1_b;        /* [dim]                   */
    CUdeviceptr qkv_w,    qkv_b;          /* [3*dim, dim], [3*dim]   */
    CUdeviceptr proj_w,   proj_b;         /* [dim, dim],   [dim]     */
    CUdeviceptr ls1;                      /* [dim]                   */
    CUdeviceptr norm2_w,  norm2_b;        /* [dim]                   */
    CUdeviceptr fc1_w,    fc1_b;          /* [ffn, dim],   [ffn]     */
    CUdeviceptr fc2_w,    fc2_b;          /* [dim, ffn],   [dim]     */
    CUdeviceptr ls2;                      /* [dim]                   */
...
typedef struct {
    ...
    /* Top-level weights. */
    CUdeviceptr cls_token;                /* [dim]                   */
    CUdeviceptr register_tokens;          /* [n_register, dim] | 0   */
    CUdeviceptr pos_embed;                /* [1+orig_grid^2, dim]    */
    CUdeviceptr patch_w;                  /* [dim, 3, ps, ps]        */
    CUdeviceptr patch_b;                  /* [dim]                   */
    CUdeviceptr norm_w, norm_b;           /* final LN, [dim]         */
...
/* Release every CUdeviceptr held by `g` and zero its fields. Safe to
...
#endif /* CUDA_SAM3D_DINOV2_GPU_H_ */

/* ============================ implementation ============================ */
#ifdef CUDA_SAM3D_DINOV2_GPU_IMPLEMENTATION
                                  ...
                                  CUdeviceptr *out_d, size_t *out_bytes)
{
    ...
    CUdeviceptr d = cu_upload_raw(safetensors_data(st, i), nb);
    if (!d) {
        fprintf(stderr, "cs3d_dinov2_gpu: cuMemAlloc failed for %s (%zu bytes)\n",
                name, nb);
        return -1;
    }
    *out_d = d;
...
{
    if (!g) return;
    #define FREE_D(p) do { if (p) { cuMemFree(p); (p) = 0; } } while (0)
    FREE_D(g->cls_token);
...
#endif /* CUDA_SAM3D_DINOV2_GPU_IMPLEMENTATION */
