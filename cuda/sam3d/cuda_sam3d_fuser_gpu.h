 ...
 *
 * Single-header. Define CUDA_SAM3D_FUSER_GPU_IMPLEMENTATION in exactly
 * one TU before including. Mirrors the cuda_sam3d_ppe_gpu.h shape but
...
#ifndef CUDA_SAM3D_FUSER_GPU_H_
#define CUDA_SAM3D_FUSER_GPU_H_
...
#endif /* CUDA_SAM3D_FUSER_GPU_H_ */

/* ============================ implementation ============================ */
#ifdef CUDA_SAM3D_FUSER_GPU_IMPLEMENTATION
...
#undef UP_
    ...
    for (int p = 0; p < n_pos; p++) {
        ...
        CUdeviceptr d = cu_upload_raw(row, nb);
...
void cs3d_fuser_gpu_free(cs3d_fuser_gpu *g)
{
    ...
    for (int i = 0; i < g->n_modalities; i++) {
        cs3d_fuser_proj_w *gp = &g->projs[i];
        if (gp->ln_w) cuMemFree(gp->ln_w);
        if (gp->ln_b) cuMemFree(gp->ln_b);
        if (gp->w1)   cuMemFree(gp->w1);
        if (gp->w2)   cuMemFree(gp->w2);
        if (gp->w3)   cuMemFree(gp->w3);
    }
    for (int p = 0; p < g->n_pos; p++) {
        if (g->idx_emb_rows[p]) cuMemFree(g->idx_emb_rows[p]);
    }
    memset(g, 0, sizeof(*g));
}

#endif /* CUDA_SAM3D_FUSER_GPU_IMPLEMENTATION */
