...
int main(int argc, char **argv)
{
    ...
    CUdeviceptr d_xs   = cu_upload_raw(x_s,  cs * sizeof(float));
    CUdeviceptr d_xp   = cu_upload_raw(x_p,  cp * sizeof(float));
    CUdeviceptr d_cond = cu_upload_raw(cond, cc * sizeof(float));
    CUdeviceptr d_mod6 = cu_upload_raw(mod6, (size_t)6 * dim * sizeof(float));
    ...
    cuMemcpyDtoH(xs_dst, d_xs, cs * sizeof(float));
    cuMemcpyDtoH(xp_dst, d_xp, cp * sizeof(float));
    ...
    cuMemFree(d_xs); cuMemFree(d_xp); cuMemFree(d_cond); cuMemFree(d_mod6);
