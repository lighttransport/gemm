...
int main(int argc, char **argv)
{
    ...
    CUfunction fn_ln = NULL, fn_gemm = NULL, fn_split = NULL, fn_sdpa = NULL, fn_resadd = NULL;
    if (cuModuleGetFunction(&fn_ln, mod, "layernorm_token_f32") != CUDA_SUCCESS ||
        cuModuleGetFunction(&fn_gemm, mod, "gemm_f32_bias") != CUDA_SUCCESS ||
        cuModuleGetFunction(&fn_split, mod, "kv_split_f32") != CUDA_SUCCESS ||
    ...
    CUdeviceptr d_ln_w = cu_upload_raw(ln_w, (size_t)dim * sizeof(float));
    CUdeviceptr d_ln_b = cu_upload_raw(ln_b, (size_t)dim * sizeof(float));
    CUdeviceptr d_q_w = cu_upload_raw(q_w, (size_t)dim * dim * sizeof(float));
    CUdeviceptr d_q_b = cu_upload_raw(q_b, (size_t)dim * sizeof(float));
    CUdeviceptr d_kv_w = cu_upload_raw(kv_w, (size_t)kv_dim * dim * sizeof(float));
    CUdeviceptr d_kv_b = cu_upload_raw(kv_b, (size_t)kv_dim * sizeof(float));
    ...
    CUdeviceptr d_h = 0, d_q = 0, d_kv = 0, d_K = 0, d_V = 0, d_xa = 0, d_proj = 0;
...
#define RUN_XA() do { \
    ...
    if (cuLaunchKernel(fn_gemm, gxq, gyq, 1, 16, 16, 1, 0, 0, a_q, NULL) != CUDA_SUCCESS) return 6; \
    unsigned gxkv = (Nc + 15) / 16, gykv = (kv_dim + 15) / 16; \
    void *a_kv[] = { &d_kv, &d_cond, &d_kv_w, &d_kv_b, &Nc, &dim, &kv_dim }; \
    if (cuLaunchKernel(fn_gemm, gxkv, gykv, 1, 16, 16, 1, 0, 0, a_kv, NULL) != CUDA_SUCCESS) return 6; \
    void *a_split[] = { &d_K, &d_V, &d_kv, &Nc, &dim }; \
    if (cuLaunchKernel(fn_split, ((int)CD + 255) / 256, 1, 1, 256, 1, 1, 0, 0, a_split, NULL) != CUDA_SUCCESS) return 6; \
...
#undef RUN_XA
    ...
    cuMemFree(d_x0); cuMemFree(d_x); cuMemFree(d_cond); cuMemFree(d_ln_w);
    cuMemFree(d_ln_b); cuMemFree(d_q_w); cuMemFree(d_q_b); cuMemFree(d_kv_w);
    cuMemFree(d_kv_b); cuMemFree(d_out_w); cuMemFree(d_out_b); cuMemFree(d_h);
    cuMemFree(d_q); cuMemFree(d_kv); cuMemFree(d_K); cuMemFree(d_V);
    cuMemFree(d_xa); cuMemFree(d_proj);
    cuModuleUnload(mod);
    cuCtxDestroy(cu_ctx);
    return ok ? 0 : 9;
}
