...
"    Y[(long)t * n_out + o] = y;\n"
"}\n"
/* Optimized pipelined W8A8 GEMM via mma.sync.m16n8k32.s32.s8.s8.s32 (native
 * INT8 tensor cores on sm_120). Clone of gemm_fp8_pipe_perrow_f32: 4x4 CTA
 * swizzle + cp.async double-buffered W + ldmatrix A. Differences: X is quantized
 * inline to int8 per row (inv=127/max), s32 accumulate, and the writeback applies
 * per-row (fwd=max/127) AND per-output-channel (wscale) dequant. The CTA's 256-col
 * wscale window is staged in smem. Gate (caller): n_tok>=16, n_out%256==0, n_in%32==0. */
"#if __CUDA_ARCH__ >= 800\n"
"__global__ __launch_bounds__(128, 4)\n"
"void gemm_int8_pipe_perrow(float *Y, const signed char *W, const float *wscale,\n"
"                            const float *X, const float *bias,\n"
"                            int n_out, int n_in, int n_tok, const float *row_max) {\n"
"    extern __shared__ unsigned char i8pr_smem[];\n"
"    signed char *smX  = (signed char *)i8pr_smem;\n"
"    signed char *smW0 = (signed char *)(i8pr_smem + 1024);\n"          /* double-buffered W */
"    signed char *smW1 = (signed char *)(i8pr_smem + 1024 + 8192);\n"
"    float *smInv = (float *)(i8pr_smem + 1024 + 8192 + 8192);\n"
"    float *smFwd = smInv + 32;\n"
"    float *smWsc = smFwd + 32;\n"  /* 256 per-output-channel scales for this CTA */
...
"        asm volatile(\"cp.async.ca.shared.global [%0],[%1],16;\\n\"::\"r\"(_d1+16),  \"l\"(_s1+16));\\\n"
"    } while (0)\n"
"    LOAD_W_I8(smW0, 0); asm volatile(\"cp.async.commit_group;\\n\");\n"
"    for (int kt = 0; kt < n_ktiles; kt++) {\n"
"        int k = kt * 32;\n"
"        signed char *smW_cur = (kt & 1) ? smW1 : smW0;\n"
"        signed char *smW_nxt = (kt & 1) ? smW0 : smW1;\n"
"        /* Quant this k-tile of X into smX (per-row int8). */\n"
...
"        /* Wait for current W tile (loaded by prev iter / prologue), then sync. */\n"
"        asm volatile(\"cp.async.wait_group 0;\\n\");\n"
"        __syncthreads();\n"
"        /* Prefetch next W tile into the alternate buffer (overlaps the mma). */\n"
"        if (kt + 1 < n_ktiles) { LOAD_W_I8(smW_nxt, kt + 1); asm volatile(\"cp.async.commit_group;\\n\"); }\n"
"        /* Direct A-fragment load per PTX m16n8k32 8-bit layout (no ldmatrix):\n"
"         * lane=(gid,tid4); a0/a1 = K[tid4*4..+3] rows gid/gid+8; a2/a3 = K+16. */\n"
...
"            }\n"
"        }\n"
"        __syncthreads();\n"  /* mma done before next iter overwrites smX / reuses buffer */
"    }\n"
"#undef LOAD_W_I8\n"
"#pragma unroll\n"
"    for (int mt = 0; mt < 2; mt++) {\n"
