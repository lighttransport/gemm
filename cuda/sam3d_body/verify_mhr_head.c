/*
 * verify_mhr_head (CUDA) — diff cuda_sam3d_body_debug_run_norm_and_heads
...
{
    if (n == 0) {
        fprintf(stderr, "[cuda verify_mhr_head] %-22s empty diff FAIL\n",
                label);
        return 1;
    }
...
int main(int argc, char **argv)
{
    ...
    int rc = cuda_sam3d_body_debug_run_norm_and_heads(ctx, tokens_in, N_Q,
