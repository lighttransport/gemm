...
static uint16_t float_to_fp16(float f) {
    ...
    if (exp > 15) {
        ...
        return (uint16_t)(sign << 15);  /* Zero/denormal */
    }
...
/* CPU reference */
static void gemm_reference_fp16(const uint16_t* A, const uint16_t* B, float* C, int M, int N, int K) {
    for (int i = 0; i < M; i++) {
        for (int j = 0; j < N; j++) {
            float sum = 0.0f;
            for (int k = 0; k < K; k++) {
                sum += fp16_to_float(A[i * K + k]) * fp16_to_float(B[k * N + j]);
            }
            C[i * N + j] = sum;
        }
    }
...
int main(int argc, char** argv) {
    ...
    printf("cuBLAS FP16 GEMM: C[%d,%d] = A[%d,%d] x B[%d,%d]\n", M, N, M, K, K, N);
    ...
    /* FP16 tensor core ops per SM per cycle */
    ...
    printf("Theoretical FP16 Peak: %.2f TOPS\n", peak_tops);
    ...
    /* Initialize random FP16 */
    srand(42);
    for (int i = 0; i < M * K; i++) {
        h_A[i] = float_to_fp16(((float)rand() / RAND_MAX - 0.5f) * 2.0f);
    }
    for (int i = 0; i < K * N; i++) {
        h_B[i] = float_to_fp16(((float)rand() / RAND_MAX - 0.5f) * 2.0f);
    }

    if (!skip_verify) {
        printf("Computing CPU reference...\n");
        gemm_reference_fp16(h_A, h_B, h_C_ref, M, N, K);
    }
    ...
    /* Create matmul descriptor - FP16 with FP32 compute */
    ...
    CHECK_CUBLAS(p_cublasLtMatrixLayoutCreate(&layout_A, CUDA_R_16F, K, M, K));
    CHECK_CUBLAS(p_cublasLtMatrixLayoutCreate(&layout_B, CUDA_R_16F, N, K, N));
    CHECK_CUBLAS(p_cublasLtMatrixLayoutCreate(&layout_C, CUDA_R_16F, N, M, N));
    ...
    if (!skip_verify) {
        ...
        for (int i = 0; i < M && errors < 10; i++) {
            for (int j = 0; j < N && errors < 10; j++) {
                float got = fp16_to_float(h_C[j * M + i]);  /* column-major */
