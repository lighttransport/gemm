...
// VENDOR EDIT (gemm/main): __global__ -> __device__ so extern "C" trampolines in
// mmq_kernels.cu can call this with stable names; __launch_bounds__ moved to the
// trampoline (illegal on a __device__ function). Upstream commit 5fd2dc2c4.
...
// VENDOR EDIT (gemm/main): __global__ -> __device__ (callable from trampoline);
// __launch_bounds__ moved to the trampoline in mmq_kernels.cu.
template <ggml_type type, int mmq_x, bool need_check>
static __device__ void mul_mat_q_stream_k_fixup(
