...
typedef struct {
    ...
    /* MTILE=4 (64 rows/CTA) variant of gemm_bf16_pipe_scaled. Doubles MMAs
     * per CTA and halves CTA count along M. SMEM 20 KB; reg-bound at
     * 3 CTAs/SM. Wins when arithmetic-intensity-bound. */
    CUfunction gemm_bf16_pipe_mt4_scaled;
...
static int hy3d_ops_load(hy3d_ops *ops, CUmodule module, int sm_version) {
    ...
    GET_OPT("gemm_bf16_pipe_mt4_scaled_f32",         gemm_bf16_pipe_mt4_scaled);
...
static inline void op_gemm_qw_dit(hy3d_ops *ops, CUstream stream,
    ...
    if (!ops->use_f32_gemm && !ops->use_fp8_w_perrow && ops->use_bf16_gemm &&
        ...
        gx = (gx + 3u) & ~3u;
        gy = (gy + 3u) & ~3u;
        if (ops->use_bf16_mt4 && ops->gemm_bf16_pipe_mt4_scaled) {
            unsigned gy_mt4 = (unsigned)((n_tok + 63) / 64);
            gy_mt4 = (gy_mt4 + 3u) & ~3u;
            size_t smem_bf16_mt4 = 4096 + 8192 * 2;
            cuLaunchKernel(ops->gemm_bf16_pipe_mt4_scaled, gx, gy_mt4, 1,
                           128, 1, 1, smem_bf16_mt4, stream, args, NULL);
        } else if (ops->use_bf16_pipe3 && ops->gemm_bf16_pipe3_scaled) {
