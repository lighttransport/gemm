...
"}\n"
"\n"
"/* ---- Sparse MoE routing kernels (replace dense per-expert pass) ---- */\n"
"/* moe_build_perm_f32: bin tokens by routed expert.\n"
" *   counts[E]            int      (zero-init by caller)\n"
" *   perm  [E*stride]     int      (per-expert list of token indices)\n"
" *   tw    [E*stride]     float    (per-expert list of gate weights)\n"
" *   indices [N_tok*top_k] int     (from moe_topk_softmax)\n"
" *   weights [N_tok*top_k] float\n"
" * Launch: grid=(ceil(N_tok/256)), block=(256). One thread per token.\n"
" * stride = N_tok (worst case all tokens route to one expert).\n"
" */\n"
"__global__ void moe_build_perm_f32(int *counts, int *perm, float *tw,\n"
"                                     const int *indices, const float *weights,\n"
"                                     int N_tok, int E, int top_k, int stride) {\n"
...
"        if (slot < stride) {\n"
"            perm[(long)e * stride + slot] = t;\n"
"            tw  [(long)e * stride + slot] = wr[k];\n"
"        }\n"
"    }\n"
"}\n"
"\n"
"/* moe_gather_rows_f32: out[i,j] = in[perm[i], j]\n"
" *   Launch: grid=(ceil(n_rows*dim/256)), block=(256).\n"
" */\n"
"__global__ void moe_gather_rows_f32(float *out, const float *in,\n"
"                                      const int *perm,\n"
"                                      int n_rows, int dim) {\n"
"    int gi = blockIdx.x * blockDim.x + threadIdx.x;\n"
"    long total = (long)n_rows * dim;\n"
"    if (gi >= total) return;\n"
"    int i = gi / dim;\n"
"    int j = gi - (long)i * dim;\n"
"    int src = perm[i];\n"
"    out[gi] = in[(long)src * dim + j];\n"
"}\n"
"\n"
"/* moe_scatter_weighted_add_f32: acc[perm[i], j] += tw[i] * src[i, j]\n"
" * No atomics needed when called once per expert on a single stream:\n"
" *   - Within one expert's perm, all token indices are unique (each token\n"
" *     appears once in its top_k list, so once per expert).\n"
" *   - Across experts, kernel launches are serialized on the stream.\n"
" * Launch: grid=(ceil(n_rows*dim/256)), block=(256).\n"
" */\n"
"__global__ void moe_scatter_weighted_add_f32(float *acc, const float *src,\n"
"                                               const int *perm, const float *tw,\n"
"                                               int n_rows, int dim) {\n"
"    int gi = blockIdx.x * blockDim.x + threadIdx.x;\n"
"    long total = (long)n_rows * dim;\n"
"    if (gi >= total) return;\n"
