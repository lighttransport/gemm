/*
 * verify_dinov2.c - Verify CUDA DINOv2 encoder against PyTorch reference
 *
 * Usage:
 *   ./verify_dinov2 <conditioner.safetensors> [--ref-dir ref/output/] [--out-dir cuda_output/]
 */
...
int main(int argc, char **argv) {
    if (argc < 2) {
        fprintf(stderr,
            ...
            "Verify DINOv2 CUDA implementation against PyTorch reference.\n"
    ...
    /* 3. Init CUDA and load DINOv2 weights */
    fprintf(stderr, "\nInitializing CUDA...\n");
    ...
    if (cuda_hy3d_load_weights(r, cond_path, NULL, NULL) != 0) {
        ...
        cuda_hy3d_free(r); free(input); free(ref_output);
        return 1;
    }
    ...
    if (cuda_hy3d_run_dinov2(r, input, cuda_output) != 0) {
        fprintf(stderr, "DINOv2 forward pass failed\n");
        cuda_hy3d_free(r); free(input); free(ref_output); free(cuda_output);
        return 1;
    }
    ...
    print_stats("cuda", cuda_output, out_n);

    /* 6. Compare */
    fprintf(stderr, "\nComparison (atol=0.15, rtol=0.05 for F16 weights, 24 layers):\n");
    compare_f32("dinov2_output", ref_output, cuda_output, out_n, 0.15f, 0.05f);

    /* 7. Save CUDA output */
    snprintf(path, sizeof(path), "%s/dinov2_output.npy", out_dir);
    write_npy_f32_2d(path, cuda_output, 1370, 1024);
    fprintf(stderr, "\nSaved CUDA output to %s\n", path);
