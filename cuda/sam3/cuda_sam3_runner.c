/*
 * cuda_sam3_runner.c - HIP/ROCm SAM 3 runner (RDNA4).
 *
...
static int load_text_block(cuda_sam3_ctx *c, int li) {
...
static int load_block(cuda_sam3_ctx *c, int bi) {
    s3_block *b = &c->blk[bi];
...
static int compile_kernels(cuda_sam3_ctx *c) {
    size_t la = strlen(hip_kernels_common_src);
    size_t lb = strlen(cuda_sam3_kernels_src);
    char *src = (char *)malloc(la + lb + 1);
    memcpy(src, hip_kernels_common_src, la);
    memcpy(src + la, cuda_sam3_kernels_src, lb + 1);
...
static int r_gemm(cuda_sam3_ctx *c, void *Y, const void *W, const void *X,
...
static int r_ln_eps(cuda_sam3_ctx *c, void *dst, const void *src, const void *w, const void *b, int n_tok, int dim, float eps) {
...
static int r_ln(cuda_sam3_ctx *c, void *dst, const void *src, const void *w, const void *b, int n_tok, int dim) {
    return r_ln_eps(c, dst, src, w, b, n_tok, dim, S3_EPS);
}
static int r_gelu(cuda_sam3_ctx *c, void *X, int n) {
    struct __attribute__((packed)) { void *X; int n; } p = { X, n };
    return launch(c->fn_gelu, (n+255)/256, 1, 1, 256, 1, 1, 0, &p, sizeof(p));
}
static int r_add(cuda_sam3_ctx *c, void *dst, const void *src, int n) {
...
static int r_part(cuda_sam3_ctx *c, void *dst, const void *src, int H, int W, int D, int ws) {
...
static int r_unpart(cuda_sam3_ctx *c, void *dst, const void *src, int H, int W, int D, int ws) {
    struct __attribute__((packed)) { void *d; const void *s; int H, W, D, ws; } p = { dst, src, H, W, D, ws };
    int total = H * W * D;
    return launch(c->fn_win_unpart, (total+255)/256, 1, 1, 256, 1, 1, 0, &p, sizeof(p));
}
static int r_rope(cuda_sam3_ctx *c, void *qkv, const void *cos_t, const void *sin_t,
...
static int r_kvtx(cuda_sam3_ctx *c, void *Kt, void *Vt, const void *qkv, int n_tok) {
...
static int r_fa(cuda_sam3_ctx *c, void *out, const void *qkv, const void *Kt, const void *Vt, int n_tok) {
...
cuda_sam3_ctx *cuda_sam3_create(const cuda_sam3_config *cfg) {
    ...
    cuda_sam3_ctx *c = (cuda_sam3_ctx *)calloc(1, sizeof(*c));
    c->cfg = *cfg;
    ...
    if (!c->w_patch || !c->w_pos || !c->w_pre_ln_w || !c->w_pre_ln_b) { cuda_sam3_destroy(c); return NULL; }

    if (c->verbose) fprintf(stderr, "sam3: loading %d ViT blocks ...\n", S3_NBLK);
    for (int bi = 0; bi < S3_NBLK; bi++) {
        if (load_block(c, bi)) { cuda_sam3_destroy(c); return NULL; }
    }
    ...
    for (int li = 0; li < S3_FPN_LEV; li++) {
        ...
        if (!L->p1_w || !L->p1_b || !L->p2_w || !L->p2_b) {
            ...
            cuda_sam3_destroy(c); return NULL;
        }
    ...
    for (int li = 0; li < S3_DETR_LAYERS; li++) {
        ...
        if (!qw || !kw || !vw || !qb || !kbn || !vb) { cuda_sam3_destroy(c); return NULL; }
    ...
    if (c->verbose) fprintf(stderr, "sam3: loading DETR decoder ...\n");
    {
        ...
        if (!c->dd_query_embed || !rp) { cuda_sam3_destroy(c); return NULL; }
        ...
        for (int i = 0; i < 3; i++) {
            ...
            if (!c->dd_box_w[i] || !c->dd_box_b[i]) { cuda_sam3_destroy(c); return NULL; }
        }
        ...
        for (int i = 0; i < 3; i++) {
            ...
            if (!c->dd_pres_w[i] || !c->dd_pres_b[i]) { cuda_sam3_destroy(c); return NULL; }
        }
        ...
        for (int i = 0; i < 2; i++) {
            ...
            if (!c->dd_rph_w[i] || !c->dd_rph_b[i]) { cuda_sam3_destroy(c); return NULL; }
        }
        ...
        for (int i = 0; i < 2; i++) {
            ...
            if (!c->dd_rpbx_w[i] || !c->dd_rpby_w[i]) { cuda_sam3_destroy(c); return NULL; }
        }
...
void cuda_sam3_destroy(cuda_sam3_ctx *c) {
    if (!c) return;
...
static int run_patch_pos(cuda_sam3_ctx *c) {
...
int cuda_sam3_set_image(cuda_sam3_ctx *c, const uint8_t *rgb, int h, int w) {
    if (!c || !rgb) return -1;
...
int cuda_sam3_set_pixel_values(cuda_sam3_ctx *c, const float *pv) {
    if (!c || !pv) return -1;
...
static int block_forward(cuda_sam3_ctx *c, int bi) {
    s3_block *b = &c->blk[bi];
    /* LN1: tmp = ln1(tok) */
...
int cuda_sam3_run_vit(cuda_sam3_ctx *c, int stop_at_block) {
...
static int r_tok2nchw(cuda_sam3_ctx *c, void *dst, const void *src,
                        int H, int W, int D) {
...
static int r_c1x1(cuda_sam3_ctx *c, void *y, const void *w, const void *b,
...
static int r_c3x3(cuda_sam3_ctx *c, void *y, const void *w, const void *b,
                    const void *x, int Ci, int Co, int H, int W) {
    struct __attribute__((packed)) { void *y; const void *w, *b, *x; int Ci, Co, H, W; } p = { y, w, b, x, Ci, Co, H, W };
    return launch(c->fn_c3x3, (W+15)/16, (H+15)/16, Co, 16, 16, 1, 0, &p, sizeof(p));
}
static int r_ct2(cuda_sam3_ctx *c, void *y, const void *w, const void *b,
                   const void *x, int Ci, int Co, int H, int W) {
    struct __attribute__((packed)) { void *y; const void *w, *b, *x; int Ci, Co, H, W; } p = { y, w, b, x, Ci, Co, H, W };
    int OH = H * 2, OW = W * 2;
    return launch(c->fn_ct2, (OW+15)/16, (OH+15)/16, Co, 16, 16, 1, 0, &p, sizeof(p));
}
static int r_mp2(cuda_sam3_ctx *c, void *y, const void *x, int C, int H, int W) {
...
int cuda_sam3_run_fpn(cuda_sam3_ctx *c) {
    if (!c) return -1;
...
int cuda_sam3_get_fpn(const cuda_sam3_ctx *c, int level, float *out,
...
int cuda_sam3_set_input_ids(cuda_sam3_ctx *c, const int32_t *ids,
...
static int r_gelu_erf(cuda_sam3_ctx *c, void *X, int n) {
    struct __attribute__((packed)) { void *X; int n; } p = { X, n };
    return launch(c->fn_gelu_erf, (n+255)/256, 1, 1, 256, 1, 1, 0, &p, sizeof(p));
}

static int r_clip_causal(cuda_sam3_ctx *c, void *out, const void *qkv, int T) {
...
int cuda_sam3_run_text(cuda_sam3_ctx *c) {
...
int cuda_sam3_get_text_output(const cuda_sam3_ctx *c, float *out,
...
static int r_nchw2tok(cuda_sam3_ctx *c, void *dst, const void *src,
                       int C, int H, int W) {
...
static int r_add_pos(cuda_sam3_ctx *c, void *dst, const void *src, const void *pos, int n) {
...
static int r_relu(cuda_sam3_ctx *c, void *x, int n) {
    struct __attribute__((packed)) { void *x; int n; } p = { x, n };
    return launch(c->fn_relu, (n + 255) / 256, 1, 1, 256, 1, 1, 0, &p, sizeof(p));
}
/* Launch MHA with online softmax, head_dim=32. */
static int r_mha32_bias(cuda_sam3_ctx *c, void *out, const void *Q, const void *K,
...
static int r_mha32(cuda_sam3_ctx *c, void *out, const void *Q, const void *K,
...
static int extract_qkv_slab(cuda_sam3_ctx *c, void *dst, const void *src,
...
static int detr_enc_layer(cuda_sam3_ctx *c, int li) {
    const int N = S3_NTOK;
...
int cuda_sam3_run_detr_enc(cuda_sam3_ctx *c) {
    if (!c) return -1;
...
int cuda_sam3_get_detr_enc(const cuda_sam3_ctx *c, float *out, int *out_n, int *out_dim) {
...
static int r_tok2chw(cuda_sam3_ctx *c, void *dst, const void *src,
                       int C, int H, int W) {
    struct __attribute__((packed)) { void *d; const void *s; int C, H, W; } p = { dst, src, C, H, W };
    int total = C * H * W;
    return launch(c->fn_tok2chw, (total + 255) / 256, 1, 1, 256, 1, 1, 0, &p, sizeof(p));
}
static int r_groupnorm(cuda_sam3_ctx *c, void *x, const void *w, const void *b,
...
static int r_upnn(cuda_sam3_ctx *c, void *dst, const void *src,
...
static int r_einsum_qchw(cuda_sam3_ctx *c, void *out, const void *me,
...
static int r_rpb_assemble(cuda_sam3_ctx *c, void *rpb, const void *ex,
...
static int dec_layer(cuda_sam3_ctx *c, int li) {
...
int cuda_sam3_run_detr_dec(cuda_sam3_ctx *c) {
...
int cuda_sam3_get_detr_dec_boxes(const cuda_sam3_ctx *c, float *out) {
    if (!c || !c->dd_done || !out) return -1;
    memcpy(out, c->dd_pred_boxes, S3_DETR_Q * 4 * 4);
    return 0;
}
int cuda_sam3_get_detr_dec_presence(const cuda_sam3_ctx *c, float *out) {
    if (!c || !c->dd_done || !out) return -1;
    memcpy(out, c->dd_presence_logits, S3_DETR_LAYERS * 4);
    return 0;
}
int cuda_sam3_get_detr_dec_hidden(const cuda_sam3_ctx *c, float *out) {
    if (!c || !c->dd_done || !out) return -1;
...
int cuda_sam3_run_dot_score(cuda_sam3_ctx *c) {
...
int cuda_sam3_get_dot_scores(const cuda_sam3_ctx *c, float *out) {
    if (!c || !c->ds_done || !out) return -1;
    memcpy(out, c->ds_scores, (size_t)S3_DETR_LAYERS * S3_DETR_Q * 4);
    return 0;
}

int cuda_sam3_run_mask_dec(cuda_sam3_ctx *c) {
    if (!c || !c->dd_done) return -1;
...
int cuda_sam3_get_pred_masks(const cuda_sam3_ctx *c, float *out,
...
int cuda_sam3_get_semantic_seg(const cuda_sam3_ctx *c, float *out,
                                int *out_h, int *out_w) {
    if (!c || !c->md_done || !out) return -1;
...
int cuda_sam3_run_postprocess(cuda_sam3_ctx *c, int target_h, int target_w,
...
const float *cuda_sam3_get_final_scores(const cuda_sam3_ctx *c, int *out_n) {
    if (!c || !c->pp_done) return NULL;
    if (out_n) *out_n = c->pp_n_kept;
    return c->pp_scores;
}
const float *cuda_sam3_get_final_boxes(const cuda_sam3_ctx *c, int *out_n) {
    if (!c || !c->pp_done) return NULL;
    if (out_n) *out_n = c->pp_n_kept;
    return c->pp_boxes;
}
const uint8_t *cuda_sam3_get_final_masks(const cuda_sam3_ctx *c,
...
int cuda_sam3_get_vit_embed(const cuda_sam3_ctx *c, float *out, int *n, int *d) {
