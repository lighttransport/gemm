...
int main(int argc, char **argv) {
    ...
    if (cuda_sam3_1_set_image(ctx, rgb, H, W)) return 5;
    if (cuda_sam3_1_run_vit(ctx, 31))          return 6;
    if (cuda_sam3_1_run_fpn(ctx))              return 7;
    if (cuda_sam3_1_set_input_ids(ctx, ids, mask)) return 8;
    if (cuda_sam3_1_run_text(ctx))             return 9;
    if (cuda_sam3_1_run_detr_enc(ctx))         return 10;
    if (cuda_sam3_1_run_detr_dec(ctx))         return 11;
    if (cuda_sam3_1_run_dot_score(ctx))        return 12;
    if (cuda_sam3_1_run_mask_dec(ctx))         return 13;
    if (cuda_sam3_1_run_postprocess(ctx, H, W, score_thr, mask_thr)) return 14;
