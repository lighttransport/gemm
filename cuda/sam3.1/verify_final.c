...
int main(int argc, char **argv)
{
    ...
    free(px);
    if (cuda_sam3_1_run_vit(ctx, 31)) return 5;
    if (cuda_sam3_1_run_fpn(ctx)) return 6;
    if (cuda_sam3_1_set_input_ids(ctx, ids, amask_p)) return 7;
    if (cuda_sam3_1_run_text(ctx)) return 8;
    if (cuda_sam3_1_run_detr_enc(ctx)) return 9;
    if (cuda_sam3_1_run_detr_dec(ctx)) return 10;
    if (cuda_sam3_1_run_dot_score(ctx)) return 11;
    if (cuda_sam3_1_run_mask_dec(ctx)) return 12;
    if (cuda_sam3_1_run_postprocess(ctx, target_h, target_w, score_th, mask_th)) return 13;
