...
int cuda_sam3_1_get_fpn(const cuda_sam3_1_ctx *ctx, int level, float *out_host,
...
int cuda_sam3_1_set_input_ids(cuda_sam3_1_ctx *ctx, const int32_t *ids,
                            const int32_t *attn_mask);
int cuda_sam3_1_run_text(cuda_sam3_1_ctx *ctx);

/* Copy text output (32, 1024) F32 into out_host (post final LN). */
int cuda_sam3_1_get_text_output(const cuda_sam3_1_ctx *ctx, float *out_host,
...
int cuda_sam3_1_run_detr_dec(cuda_sam3_1_ctx *ctx);
/* Copy final pred_boxes (200, 4) xyxy normalized, 0..1. */
int cuda_sam3_1_get_detr_dec_boxes(const cuda_sam3_1_ctx *ctx, float *out);
/* Copy per-layer presence logits (6,) clamped. */
int cuda_sam3_1_get_detr_dec_presence(const cuda_sam3_1_ctx *ctx, float *out);
/* Copy last-layer output_layer_norm(query_hidden) (200, 256). */
int cuda_sam3_1_get_detr_dec_hidden(const cuda_sam3_1_ctx *ctx, float *out);

/* Dot-product scoring: per-layer query↔text logits (6, 200), clamp ±12. */
int cuda_sam3_1_run_dot_score(cuda_sam3_1_ctx *ctx);
int cuda_sam3_1_get_dot_scores(const cuda_sam3_1_ctx *ctx, float *out);

/* Mask decoder: prompt_cross_attn → pixel_decoder → instance/semantic →
 * mask_embedder → einsum. pred_masks (200, 288, 288) F32. */
int cuda_sam3_1_run_mask_dec(cuda_sam3_1_ctx *ctx);
int cuda_sam3_1_get_pred_masks(const cuda_sam3_1_ctx *ctx, float *out,
                              int *out_q, int *out_h, int *out_w);
int cuda_sam3_1_get_semantic_seg(const cuda_sam3_1_ctx *ctx, float *out,
                                int *out_h, int *out_w);
...
int cuda_sam3_1_run_postprocess(cuda_sam3_1_ctx *ctx, int target_h, int target_w,
                              float score_threshold, float mask_threshold);
const float *cuda_sam3_1_get_final_scores(const cuda_sam3_1_ctx *ctx, int *out_n);
const float *cuda_sam3_1_get_final_boxes(const cuda_sam3_1_ctx *ctx, int *out_n);
const uint8_t *cuda_sam3_1_get_final_masks(const cuda_sam3_1_ctx *ctx,
