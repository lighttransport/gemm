 ...
 * (MTILE=2 × NTILE=8 × 4 warps → 32 rows × 256 N cols per CTA) but loads
 ...
 * W slab (256 rows × 32 K bytes = 8 KB) is prefetched asynchronously into
...
"\n"
"    /* Issue 4 cp.async.16 for one W tile (256 rows × 32 K bytes):\n"
"     * thread t → 2 consecutive rows × 2 × 16-byte chunks. */\n"
...
"         * 128 threads × 8 floats = 1024 floats per pass. 2 × float4 loads per\n"
...
"         * warp-wide instruction. The x4 variant loads 4 × 8×8 b16 sub-matrices\n"
"         * that together tile a 16×32 FP8 A fragment (= one MTILE). */\n"
