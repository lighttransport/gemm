...
"    const unsigned char *row_ptr = mat + (size_t)row * row_bytes;\n"
"    int G = nb * 64;  /* (block, half(2), l(32)) groups, 4 products each */\n"
"    float sum = 0.0f;\n"
...
"        int hi = (l < 16) ? 0 : 1;\n"
"        float sA = d * (float)sc[0 + hi];\n"
"        float sB = d * (float)sc[2 + hi];\n"
"        float sC = d * (float)sc[4 + hi];\n"
"        float sD = d * (float)sc[6 + hi];\n"
