...
"    }\n"
"}\n"
"/* Fused dequant (IQ2_S) + grouped WMMA GEMM: loads IQ2_S expert weights\n"
" * directly instead of reading pre-dequantized BF16 from d_expw_bf16.\n"
" * Eliminates the dequant-write-then-GEMM-read round-trip per projection. */\n"
"__global__ void gemm_dequant_iq2s_grouped(float *Y, const unsigned char *W_iq2s,\n"
"        const bf16_raw *X, const int *offs, int N, int K, long long stride) {\n"
"    int e = blockIdx.z;\n"
"    int m0 = offs[e], m1 = offs[e + 1];\n"
"    int M = m1 - m0;\n"
"    if (M <= 0 || (int)blockIdx.y * 128 >= M) return;\n"
"    const unsigned char *We = W_iq2s + (long long)e * stride;\n"
"    const bf16_raw *Xe = X + (size_t)m0 * K;\n"
"    float *Ye = Y + (size_t)m0 * N;\n"
"    int tid = threadIdx.x;\n"
"    int wave_id = tid >> 5;\n"
...
"            const bf16x8 *sa = (const bf16x8 *)&Xe[(size_t)(cta_m0 + er) * K + k + ek];\n"
"            da[0] = sa[0]; da[1] = sa[1];\n"
"            /* dequant IQ2_S group into smB */\n"
"            int ib32 = (k % 256) / 32;\n"
"            int row = cta_n0 + er;\n"
"            const unsigned char *rp = We + (size_t)row * nb * 82;\n"
"            float d = half_to_float(*(const half_raw *)rp);\n"
"            unsigned char scb = rp[74 + ib32];\n"
"            unsigned char qhb = rp[66 + ib32];\n"
"            for (int g = 0; g < 2; g++) {\n"
"                int l = ek / 8 + g;\n"
"                int gi = rp[2 + ib32*4 + l] | ((qhb << (8 - 2*l)) & 0x300);\n"
"                const unsigned char *grid = (const unsigned char *)&iq2s_grid_dev[gi];\n"
"                unsigned char sgn = rp[34 + ib32*4 + l];\n"
"                float db = (l < 2) ? d * (0.5f + (float)(scb & 0xf)) * 0.25f\n"
"                                   : d * (0.5f + (float)(scb >>  4)) * 0.25f;\n"
"                int bo = g * 8;\n"
"                for (int j = 0; j < 8; j++) {\n"
"                    float v = db * (float)grid[j];\n"
"                    if (sgn & (1 << j)) v = -v;\n"
"                    smB[er * 32 + ek + bo + j] = f32_to_bf16(v);\n"
"                }\n"
"            }\n"
"        } else {\n"
"            for (int it = 0; it < 16; it++) {\n"
...
"                int er = el >> 5, ek = el & 31;\n"
"                int row = cta_m0 + er, kp = k + ek;\n"
"                smA[er * 32 + ek] = (row < M && kp < K) ? Xe[(size_t)row * K + kp] : 0;\n"
"            }\n"
"            for (int it = 0; it < 16; it++) {\n"
"                int el = tid * 16 + it;\n"
"                int er = el >> 5, ek = el & 31;\n"
...
"                if (col < N && kp < K) {\n"
"                    int ib32 = (kp % 256) / 32;\n"
"                    const unsigned char *rp = We + (size_t)col * nb * 82;\n"
"                    float d = half_to_float(*(const half_raw *)rp);\n"
"                    unsigned char scb = rp[74 + ib32];\n"
"                    unsigned char qhb = rp[66 + ib32];\n"
"                    int l = (kp % 32) / 8;\n"
...
"                    float db = (l < 2) ? d * (0.5f + (float)(scb & 0xf)) * 0.25f\n"
"                                       : d * (0.5f + (float)(scb >>  4)) * 0.25f;\n"
"                    float v = db * (float)grid[sub];\n"
"                    if (sgn & (1 << sub)) v = -v;\n"
"                    smB[el] = f32_to_bf16(v);\n"
"                } else {\n"
"                    smB[el] = 0;\n"
"                }\n"
"            }\n"
...
"    }\n"
"}\n"
"/* Fused dequant (IQ3_S) + grouped WMMA GEMM for down projection. */\n"
"__global__ void gemm_dequant_iq3s_grouped(float *Y, const unsigned char *W_iq3s,\n"
"        const bf16_raw *X, const int *offs, int N, int K, long long stride) {\n"
"    int e = blockIdx.z;\n"
"    int m0 = offs[e], m1 = offs[e + 1];\n"
"    int M = m1 - m0;\n"
"    if (M <= 0 || (int)blockIdx.y * 128 >= M) return;\n"
"    const unsigned char *We = W_iq3s + (long long)e * stride;\n"
"    const bf16_raw *Xe = X + (size_t)m0 * K;\n"
"    float *Ye = Y + (size_t)m0 * N;\n"
"    int tid = threadIdx.x;\n"
"    int wave_id = tid >> 5;\n"
...
"            const bf16x8 *sa = (const bf16x8 *)&Xe[(size_t)(cta_m0 + er) * K + k + ek];\n"
"            da[0] = sa[0]; da[1] = sa[1];\n"
"            /* dequant IQ3_S group into smB */\n"
"            int sb = (k % 256) / 32;\n"
"            int row = cta_n0 + er;\n"
"            const unsigned char *rp = We + (size_t)row * nb * 110;\n"
"            float d = half_to_float(*(const half_raw *)rp);\n"
"            unsigned char sc = rp[106 + (sb >> 1)];\n"
"            float db = d * (float)(1 + 2 * ((sb & 1) ? (sc >> 4) : (sc & 0xf)));\n"
"            unsigned char qhb = rp[66 + sb];\n"
"            for (int g = 0; g < 2; g++) {\n"
"                int l = ek / 8 + g;\n"
"                unsigned char qs0 = rp[2 + sb*8 + 2*l];\n"
"                unsigned char qs1 = rp[2 + sb*8 + 2*l + 1];\n"
"                const unsigned char *g1 = (const unsigned char *)&iq3s_grid_dev[qs0 | ((qhb << (8-2*l)) & 256)];\n"
"                const unsigned char *g2 = (const unsigned char *)&iq3s_grid_dev[qs1 | ((qhb << (7-2*l)) & 256)];\n"
"                unsigned char sgn = rp[74 + sb*4 + l];\n"
"                int bo = g * 8;\n"
"                for (int j = 0; j < 4; j++) {\n"
"                    float v0 = db * (float)g1[j]; if (sgn & (1 << j)) v0 = -v0;\n"
"                    float v1 = db * (float)g2[j]; if (sgn & (1 << (j+4))) v1 = -v1;\n"
"                    smB[er * 32 + ek + bo + j]     = f32_to_bf16(v0);\n"
"                    smB[er * 32 + ek + bo + 4 + j] = f32_to_bf16(v1);\n"
"                }\n"
"            }\n"
"        } else {\n"
"            for (int it = 0; it < 16; it++) {\n"
...
"                if (col < N && kp < K) {\n"
"                    int sb = (kp % 256) / 32;\n"
"                    const unsigned char *rp = We + (size_t)col * nb * 110;\n"
"                    float d = half_to_float(*(const half_raw *)rp);\n"
"                    unsigned char sc = rp[106 + (sb >> 1)];\n"
"                    float db = d * (float)(1 + 2 * ((sb & 1) ? (sc >> 4) : (sc & 0xf)));\n"
"                    unsigned char qhb = rp[66 + sb];\n"
...
"                    int half_g = sub / 4;\n"
"                    int sub2 = sub % 4;\n"
"                    unsigned char qs = rp[2 + sb*8 + 2*l + half_g];\n"
"                    int idx = qs | ((qhb << (8 - 2*l - half_g)) & 256);\n"
"                    const unsigned char *gv = (const unsigned char *)&iq3s_grid_dev[idx];\n"
"                    unsigned char sgn = rp[74 + sb*4 + l];\n"
"                    float v = db * (float)gv[sub2];\n"
"                    if (sgn & (1 << (sub))) v = -v;\n"
"                    smB[el] = f32_to_bf16(v);\n"
"                } else { smB[el] = 0; }\n"
"            }\n"
...
struct hip_llm_runner {
    ...
    hipFunction_t fn_gemm_dequant_iq2s_grouped; /* fused IQ2_S dequant + grouped GEMM */
    hipFunction_t fn_gemm_dequant_iq3s_grouped; /* fused IQ3_S dequant + grouped GEMM */
...
static int compile_kernels(hip_llm_runner *r) {
    ...
    GET_FUNC(gemm_dequant_iq2s_grouped);
    GET_FUNC(gemm_dequant_iq3s_grouped);
...
static int forward_moe_ffn_batched(hip_llm_runner *r, hip_layer *cl, int M) {
    ...
    if (use_wmma_exp && r->d_expw_bf16 &&
        ...
        /* gate (fused IQ2_S dequant + GEMM) */
        { void *a[] = { &r->d_moe_eg, &cl->moe_gate_exps_w, &r->d_moe_gather_in_bf16,
                        &r->d_moe_offs, &eff, &n_embd, &sgu };
          LAUNCH(r->fn_gemm_dequant_iq2s_grouped, (unsigned)((eff + 127) / 128), mtiles, ne, 256, 1, 1, 0, r->stream, a); }
        /* up (fused IQ2_S dequant + GEMM) */
        { void *a[] = { &r->d_moe_eu, &cl->moe_up_exps_w, &r->d_moe_gather_in_bf16,
                        &r->d_moe_offs, &eff, &n_embd, &sgu };
          LAUNCH(r->fn_gemm_dequant_iq2s_grouped, (unsigned)((eff + 127) / 128), mtiles, ne, 256, 1, 1, 0, r->stream, a); }
        launch_silu_mul(r, r->d_moe_eg, r->d_moe_eu, total * eff);
        launch_pack_bf16_from_f32(r, r->d_moe_esilu_bf16, r->d_moe_eg, total * eff);
        /* down (fused IQ3_S dequant + GEMM) */
        { void *a[] = { &r->d_moe_eout, &cl->moe_down_exps_w, &r->d_moe_esilu_bf16,
                        &r->d_moe_offs, &n_embd, &eff, &sdn };
          LAUNCH(r->fn_gemm_dequant_iq3s_grouped, (unsigned)((n_embd + 127) / 128), mtiles, ne, 256, 1, 1, 0, r->stream, a); }
        goto experts_done;
    }
    if (use_wmma_exp)
        launch_pack_bf16_from_f32(r, r->d_moe_gather_in_bf16, r->d_moe_gather_in, total * n_embd);
