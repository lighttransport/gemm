...
"    }\n"
"}\n"
"__global__ void matvec_bf16_f32_batch(float *dst, const unsigned short *mat, const float *x,\n"
"                                int n_rows, int n_cols) {\n"
"    dst+=(size_t)blockIdx.y*n_rows; x+=(size_t)blockIdx.y*n_cols;\n"
"    int row=blockIdx.x, tid=threadIdx.x; if(row>=n_rows)return;\n"
"    const unsigned short *rp=mat+(size_t)row*n_cols; float sum=0.0f;\n"
"    for(int j=tid;j<n_cols;j+=blockDim.x){unsigned int bits=(unsigned int)rp[j]<<16;float w;\n"
"        __builtin_memcpy(&w,&bits,sizeof(w));sum+=w*x[j];}\n"
"    for(int o=16;o>0;o>>=1)sum+=__shfl_down(sum,o);\n"
"    __shared__ float ws[8];int wid=tid/32,lane=tid&31;if(lane==0)ws[wid]=sum;__syncthreads();\n"
"    if(tid==0){float total=0.0f;for(int w=0;w<(blockDim.x+31)/32;w++)total+=ws[w];dst[row]=total;}\n"
"}\n"
"\n"
