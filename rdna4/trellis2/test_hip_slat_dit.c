/*
 * test_hip_slat_dit — TRELLIS.2 SLAT DiT HIP/RDNA4 verify harness.
 *
 * Mirrors cpu/trellis2/verify_slat_dit.c but exercises hip_trellis2_runner's
 * SLAT path (Phase 2 of #12). Loads:
 *   <dump_dir>/06b_slat_dit_step_x_t.npy        [N, 32]    f32
 *   <dump_dir>/06b_slat_dit_step_coords.npy     [N, 4]     i32
 *   <dump_dir>/06b_slat_dit_step_t.npy          [1]        f32
 *   <dump_dir>/06b_slat_dit_step_cond.npy       [1, M, C]  f32
 *   <dump_dir>/06b_slat_dit_step_velocity.npy   [N, 32]    f32 (reference)
 *
 * Pass criterion: cosine >= 0.999 vs PyT-ROCm reference.
 *
 * Usage:
 *   test_hip_slat_dit <slat_dit.safetensors> <dump_dir>
 */
...
static void *load_npy_or_die(const char *dir, const char *name,
    ...
    if (!p) {
        fprintf(stderr, "[test_hip_slat_dit] missing/unreadable: %s\n", path);
        exit(2);
    }
    return p;
}
...
int main(int argc, char **argv) {
    ...
    fprintf(stderr, "[test_hip_slat_dit] safetensors: %s\n", st_path);
    fprintf(stderr, "[test_hip_slat_dit] dump_dir   : %s\n", dir);

    hip_trellis2_runner *r = hip_trellis2_init(0, 1);
    if (!r) { fprintf(stderr, "[test_hip_slat_dit] runner init failed\n"); return 3; }

    if (hip_trellis2_load_slat_dit(r, st_path) != 0) {
        fprintf(stderr, "[test_hip_slat_dit] load failed\n");
        hip_trellis2_free(r);
        return 3;
    }
    ...
    float *x_t = (float *)load_npy_or_die(dir, "06b_slat_dit_step_x_t.npy",
                                          &nd, dims, &is_f32);
    if (!is_f32 || nd != 2 || dims[1] != 32) {
        fprintf(stderr, "[test_hip_slat_dit] bad x_t shape\n");
        return 4;
    }
    int N = dims[0];
    ...
    if (is_f32 || nd != 2 || dims[0] != N || dims[1] != 4) {
        fprintf(stderr, "[test_hip_slat_dit] bad coords shape\n");
        return 4;
    }
    ...
    if (nd == 3) {
        if (dims[0] != 1) {
            fprintf(stderr, "[test_hip_slat_dit] cond batch != 1 (got %d)\n", dims[0]);
            return 4;
        }
        ...
        fprintf(stderr, "[test_hip_slat_dit] bad cond ndim=%d\n", nd);
        return 4;
    }
    if (cond_C != 1024) {
        fprintf(stderr, "[test_hip_slat_dit] cond_dim mismatch: file=%d expect=1024\n", cond_C);
        return 4;
    }
    ...
    if (!is_f32 || nd != 2 || dims[0] != N || dims[1] != 32) {
        fprintf(stderr, "[test_hip_slat_dit] bad velocity shape\n");
        return 4;
    }
    ...
    printf("[test_hip_slat_dit] N=%d t=%.6f cosine=%.6f max_abs=%.6g %s\n",
