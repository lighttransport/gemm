/*
 * test_hip_sam3d_body — CLI for the CUDA SAM 3D Body runner.
 *
 * Usage:
 *   test_hip_sam3d_body --safetensors-dir <DIR> --image <image.jpg>
 *       --mhr-assets <DIR>
 *       [--bbox x0 y0 x1 y1] [--focal F]
 *       [-o body.obj] [--precision bf16|fp16] [--device N] [-v]
 *
 *   test_hip_sam3d_body <safetensors-dir> <image.jpg> ...
 */
...
/* SAFETENSORS_IMPLEMENTATION is supplied by hip_sam3d_body_runner.c. */
...
int main(int argc, char **argv)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if      (!strcmp(a, "--help") || !strcmp(a, "-h")) {
            ...
            const char *v = argv[++i];
            if      (!strcmp(v, "dinov3")) backbone = HIP_SAM3D_BODY_BACKBONE_DINOV3;
            else if (!strcmp(v, "vith"))   backbone = HIP_SAM3D_BODY_BACKBONE_VITH;
    ...
    if (auto_bbox && !has_bbox) {
        ...
        if (!det) {
            fprintf(stderr, "[test_hip_sam3d_body] rt_detr_load failed: %s\n",
                    rt_detr_model);
        ...
        rt_detr_free(det);
        if (rc != 0) {
            fprintf(stderr, "[test_hip_sam3d_body] no person detected "
                    "(thresh=%.2f)\n", auto_thresh);
            stbi_image_free(pixels);
            return 4;
        }
        ...
        has_bbox = 1;
        if (verbose)
            fprintf(stderr, "[test_hip_sam3d_body] auto-bbox score=%.4f "
    ...
    t0 = cli_time_ms();
    hip_sam3d_body_set_image(ctx, pixels, iw, ih, has_bbox ? bbox : NULL);
    if (focal_hint > 0) hip_sam3d_body_set_focal(ctx, focal_hint);
    t_set_image_ms = cli_time_ms() - t0;

    t0 = cli_time_ms();
    int rc = hip_sam3d_body_run_all(ctx);
    t_run_all_ms = cli_time_ms() - t0;
    if (rc != 0) {
        fprintf(stderr, "[test_hip_sam3d_body] run_all rc=%d\n", rc);
    ...
    int nv = 0, nf = 0;
    hip_sam3d_body_get_vertices(ctx, NULL, &nv);
    hip_sam3d_body_get_faces(ctx, NULL, &nf);
    if (nv > 0 && nf > 0) {
        t0 = cli_time_ms();
        ...
        if (rc == 0)
            ...
            fprintf(stderr, "[test_hip_sam3d_body] obj_write %s failed\n",
                    out_path);
        t_write_obj_ms = cli_time_ms() - t0;
    } else {
        fprintf(stderr, "[test_hip_sam3d_body] empty output: V=%d F=%d\n", nv, nf);
        rc = 1;
    }

    if (verbose) {
        fprintf(stderr,
                "[test_hip_sam3d_body][timing] total %.3f ms "
        ...
        if (auto_bbox) {
            fprintf(stderr,
                    "[test_hip_sam3d_body][timing] auto_bbox split: "
