...
#ifndef HIP_SAM3D_BODY_KERNELS_H_
#define HIP_SAM3D_BODY_KERNELS_H_

/* The shared hip_kernels_common_src opens an `extern "C" {` block that
...
static const char hip_sam3d_body_kernels_src[] =
    ...
    /* gemm_f16w_bf16a_wmma_t: BF16-WMMA encoder GEMM (opt-in, gfx12).
     * Drop-in for gemm_tiled_f16_f32 (same {Y,W,X,bias,n_out,n_in,n_tok}
     * params). F16 weights via half_to_float, X+W RNE-truncated to bf16,
     * F32 accumulate. Requires n_in % 16 == 0. Adapted from rdna4/hy3d. */
    ...
    "            int col = cta_n0 + er;\n"
    "            int kp  = k + ek;\n"
    "            float wv = (col < n_out && kp < n_in) ? half_to_float(W[(long)col * n_in + kp]) : 0.f;\n"
    ...
    /* flash_attn_tiled_wmma_f32: BF16-WMMA self-attention, head_dim=64 (opt-in,
     * gfx12). Drop-in for the scalar flash_attn_tiled_f32 (identical signature
     * + IO: Q from interleaved qkv[qi*3*dim + h*hd], K_t/V_t per-head
     * [n_heads,n_tok,hd], out[qi*dim + h*hd]). FA2 online softmax in F32; only
     * QK^T and P·V matmul inputs are bf16 (RNE-truncated). Block 128 = 4 waves×
     * 32 lanes; each wave owns 16 q rows (Br=64), Bc=16. Adapted from
     * rdna4/qimg flash_attn_sa_wmma_f32 (hd=128 → 64: k-loop 8→4, O-tiles 8→4). */
    "#if defined(__gfx1200__) || defined(__gfx1201__)\n"
    "__global__ void flash_attn_tiled_wmma_f32(float *out, const float *qkv,\n"
    "    const float *K_t, const float *V_t,\n"
    "    int n_tok, int dim, int n_heads, int head_dim, float scale) {\n"
    "    (void)head_dim; (void)n_heads;  /* head_dim fixed 64 */\n"
    "    const int HD = 64;\n"
    "    int h = blockIdx.x;\n"
    ...
    "    int idx  = lane & 15;\n"
    "    int dim3 = 3 * dim;\n"
    "    const float *kt_h = K_t + (size_t)h * n_tok * HD;\n"
    "    const float *vt_h = V_t + (size_t)h * n_tok * HD;\n"
    "    __shared__ short smK[16*64];\n"
    ...
    "        int d = e & 63;\n"
    "        int qi = wq0 + r;\n"
    "        float qv = (qi < n_tok) ? qkv[(long)qi*dim3 + h*HD + d] : 0.f;\n"
    "        unsigned int b; __builtin_memcpy(&b, &qv, 4);\n"
    "        b += ((b >> 16) & 1u) + 0x7FFFu;\n"
    "        myQ[r*64 + d] = (short)(b >> 16);\n"
    "    }\n"
    "    float8 O0 = {0,0,0,0,0,0,0,0};\n"
    "    float8 O1 = O0, O2 = O0, O3 = O0;\n"
    "    float m_i[8], l_i[8];\n"
    "    for (int i = 0; i < 8; i++) { m_i[i] = -1e30f; l_i[i] = 0.f; }\n"
    "    int n_kv = (n_tok + 15) / 16;\n"
    ...
    "            int d = e & 63;\n"
    "            int kvi = kv_base + r;\n"
    "            float v = (kvi < n_tok) ? kt_h[(long)kvi*64 + d] : 0.f;\n"
    "            unsigned int b; __builtin_memcpy(&b, &v, 4);\n"
    "            b += ((b >> 16) & 1u) + 0x7FFFu;\n"
    "            smK[r*64 + d] = (short)(b >> 16);\n"
    "        }\n"
    "        for (int it = 0; it < 8; it++) {\n"
    "            int e = tid * 8 + it;\n"
    "            int r = e >> 6;\n"
    "            int d = e & 63;\n"
    "            int kvi = kv_base + r;\n"
    "            float v = (kvi < n_tok) ? vt_h[(long)kvi*64 + d] : 0.f;\n"
    "            unsigned int b; __builtin_memcpy(&b, &v, 4);\n"
    "            b += ((b >> 16) & 1u) + 0x7FFFu;\n"
    ...
    "            int row = half*8 + i;\n"
    "            float pv = S[i];\n"
    "            unsigned int b; __builtin_memcpy(&b, &pv, 4);\n"
    "            b += ((b >> 16) & 1u) + 0x7FFFu;\n"
    ...
    "            int qi = wq0 + half*8 + i;\n"
    "            if (qi < n_tok)\n"
    "                out[(long)qi*dim + h*HD + col] = (*Os[t])[i] * inv_l[i];\n"
    "        }\n"
    "    }\n"
    "}\n"
    "#else\n"
...
#endif /* HIP_SAM3D_BODY_KERNELS_H_ */
