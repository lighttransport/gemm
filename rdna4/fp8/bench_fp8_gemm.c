...
int main(int argc, char **argv) {
    ...
    const char *modes_all[] = {"baseline", "pipe", "pipe32", "pipe32_occ", "pipe32_hoist", "pipe32_prio", "pipe32_u32", "pipe32_pgr2"};
    int n_modes = !strcmp(mode, "all") ? 8 : 1;
    const char *modes[8];
    if (n_modes == 1) modes[0] = mode;
    else for (int i = 0; i < 8; i++) modes[i] = modes_all[i];
