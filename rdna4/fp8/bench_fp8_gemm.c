...
"    __asm__ __volatile__(\"s_setprio 3\" ::: \"memory\");\n"
"}\n"
"__device__ __forceinline__ void setprio_lo() {\n"
"    __asm__ __volatile__(\"s_setprio 0\" ::: \"memory\");\n"
"}\n"
...
"    #define SPLIT_HI(v) __builtin_shufflevector(v, v, 8,9,10,11,12,13,14,15)\n"
"    #define SPLIT_LO(v) __builtin_shufflevector(v, v, 0,1,2,3,4,5,6,7)\n"
...
"\n"
"\n"
"/* pipe32_prio: identical layout to pipe32_hoist, but inserts:\n"
" *   sched_fence() between LDS reads and WMMAs (prevents compiler from\n"
" *     interleaving WMMAs with the partial s_wait_dscnt 0x6/0x5/0x4...).\n"
" *   wait_lds_all() to force a single s_wait_dscnt 0x0 once.\n"
" *   setprio_hi() at start of WMMA burst, setprio_lo() at end (RDNA4 wave\n"
" *     priority hint to the SIMD scheduler).\n"
" * Theory: the 122 wait/scheduler instructions per K-step in pipe32 asm hide\n"
" * the 32 back-to-back WMMA window the WMMA pipe wants to see. */\n"
"__global__ __launch_bounds__(128, 1)\n"
"void gemm_fp8_pipe32_prio(float *Y, const u8 *W, const u8 *X,\n"
"                          const float *bias, int N, int K, int M) {\n"
"    int tid = threadIdx.x;\n"
"    int wave_id = tid >> 5;\n"
"    int lane = tid & 31;\n"
...
"\n"
"\n"
"/* pipe32_u32: same structure as pipe32_prio, but reroutes the FP8 data through\n"
" * u32 lanes everywhere. Global loads as u32x4 (16 B / lane), LDS stored as\n"
" * u32x2 (8 B / lane). This avoids clang representing u8x16 as 16 separate i8\n"
" * lanes, which forces ~100 v_perm_b32 + v_lshrrev_b32 + v_lshl_or per K-step\n"
" * just to do a no-op SPLIT_LO/HI on packed bytes. With u32 elements, splitting\n"
" * into hi/lo halves is a free VGPR-pair reinterpretation. */\n"
"__device__ __forceinline__ u32x2 SPLIT_LO_U32(u32x4 v) {\n"
"    return __builtin_shufflevector(v, v, 0, 1);\n"
"}\n"
"__device__ __forceinline__ u32x2 SPLIT_HI_U32(u32x4 v) {\n"
"    return __builtin_shufflevector(v, v, 2, 3);\n"
"}\n"
"__global__ __launch_bounds__(128, 1)\n"
"void gemm_fp8_pipe32_u32(float *Y, const u8 *W, const u8 *X,\n"
"                         const float *bias, int N, int K, int M) {\n"
"    int tid = threadIdx.x;\n"
"    int wave_id = tid >> 5;\n"
"    int lane = tid & 31;\n"
...
"    int r0 = e0 >> 1, r1 = e1 >> 1;\n"
"    int c0 = (e0 & 1) * 16, c1 = (e1 & 1) * 16;\n"
"    __shared__ u32x2 smA[2 * 128 * 4];\n"
"    __shared__ u32x2 smB[2 * 128 * 4];\n"
"    {\n"
"        u32x4 a0v = *((const u32x4 *)(X + (size_t)(cta_m0 + r0) * K + c0));\n"
"        u32x4 a1v = *((const u32x4 *)(X + (size_t)(cta_m0 + r1) * K + c1));\n"
"        u32x4 b0v = *((const u32x4 *)(W + (size_t)(cta_n0 + r0) * K + c0));\n"
"        u32x4 b1v = *((const u32x4 *)(W + (size_t)(cta_n0 + r1) * K + c1));\n"
"        smA[(c0>>3) * 128 + r0]    = SPLIT_LO_U32(a0v);\n"
"        smA[((c0>>3)+1)*128 + r0]  = SPLIT_HI_U32(a0v);\n"
"        smA[(c1>>3) * 128 + r1]    = SPLIT_LO_U32(a1v);\n"
"        smA[((c1>>3)+1)*128 + r1]  = SPLIT_HI_U32(a1v);\n"
"        smB[(c0>>3) * 128 + r0]    = SPLIT_LO_U32(b0v);\n"
"        smB[((c0>>3)+1)*128 + r0]  = SPLIT_HI_U32(b0v);\n"
"        smB[(c1>>3) * 128 + r1]    = SPLIT_LO_U32(b1v);\n"
"        smB[((c1>>3)+1)*128 + r1]  = SPLIT_HI_U32(b1v);\n"
"    }\n"
"    lds_barrier();\n"
"    float8 z = {0,0,0,0,0,0,0,0};\n"
"    float8 cv00=z,cv01=z,cv02=z,cv03=z;\n"
...
"        if (has_next) {\n"
"            int nk = k + 32;\n"
"            na0 = *((const u32x4 *)(X + (size_t)(cta_m0 + r0) * K + nk + c0));\n"
"            na1 = *((const u32x4 *)(X + (size_t)(cta_m0 + r1) * K + nk + c1));\n"
"            nb0 = *((const u32x4 *)(W + (size_t)(cta_n0 + r0) * K + nk + c0));\n"
"            nb1 = *((const u32x4 *)(W + (size_t)(cta_n0 + r1) * K + nk + c1));\n"
"        }\n"
"        int base = buf * (128 * 4);\n"
"        int a_base = wM * 64;\n"
"        int b_base = wN * 64;\n"
"        int ks0 = (0  + k_off) >> 3;\n"
"        int ks1 = (16 + k_off) >> 3;\n"
"        u32x2 ra00 = smA[base + ks0 * 128 + (a_base + 0  + idx)];\n"
"        u32x2 ra01 = smA[base + ks0 * 128 + (a_base + 16 + idx)];\n"
"        u32x2 ra02 = smA[base + ks0 * 128 + (a_base + 32 + idx)];\n"
"        u32x2 ra03 = smA[base + ks0 * 128 + (a_base + 48 + idx)];\n"
"        u32x2 rb00 = smB[base + ks0 * 128 + (b_base + 0  + idx)];\n"
"        u32x2 rb01 = smB[base + ks0 * 128 + (b_base + 16 + idx)];\n"
"        u32x2 rb02 = smB[base + ks0 * 128 + (b_base + 32 + idx)];\n"
"        u32x2 rb03 = smB[base + ks0 * 128 + (b_base + 48 + idx)];\n"
"        u32x2 ra10 = smA[base + ks1 * 128 + (a_base + 0  + idx)];\n"
"        u32x2 ra11 = smA[base + ks1 * 128 + (a_base + 16 + idx)];\n"
"        u32x2 ra12 = smA[base + ks1 * 128 + (a_base + 32 + idx)];\n"
"        u32x2 ra13 = smA[base + ks1 * 128 + (a_base + 48 + idx)];\n"
"        u32x2 rb10 = smB[base + ks1 * 128 + (b_base + 0  + idx)];\n"
"        u32x2 rb11 = smB[base + ks1 * 128 + (b_base + 16 + idx)];\n"
"        u32x2 rb12 = smB[base + ks1 * 128 + (b_base + 32 + idx)];\n"
"        u32x2 rb13 = smB[base + ks1 * 128 + (b_base + 48 + idx)];\n"
"        sched_fence();\n"
"        wait_lds_all();\n"
"        setprio_hi();\n"
"        WMMA_FP8(ra00, rb00, cv00); WMMA_FP8(ra00, rb01, cv01);\n"
...
"            int nb = 1 - buf;\n"
"            int nbase = nb * (128 * 4);\n"
"            smA[nbase + (c0>>3)*128 + r0]     = SPLIT_LO_U32(na0);\n"
"            smA[nbase + ((c0>>3)+1)*128 + r0] = SPLIT_HI_U32(na0);\n"
"            smA[nbase + (c1>>3)*128 + r1]     = SPLIT_LO_U32(na1);\n"
"            smA[nbase + ((c1>>3)+1)*128 + r1] = SPLIT_HI_U32(na1);\n"
"            smB[nbase + (c0>>3)*128 + r0]     = SPLIT_LO_U32(nb0);\n"
"            smB[nbase + ((c0>>3)+1)*128 + r0] = SPLIT_HI_U32(nb0);\n"
"            smB[nbase + (c1>>3)*128 + r1]     = SPLIT_LO_U32(nb1);\n"
"            smB[nbase + ((c1>>3)+1)*128 + r1] = SPLIT_HI_U32(nb1);\n"
"            lds_barrier_signal();\n"
"            buf = nb;\n"
"            lds_barrier_wait();\n"
"        }\n"
...
"\n"
"\n"
"/* pipe32_asm: identical layout to pipe32_prio, but the entire 32-WMMA inner\n"
" * block is one inline-asm statement so the LLVM scheduler cannot interleave\n"
" * s_wait_alu / s_wait_dscnt between WMMAs. We emit one s_wait_dscnt 0x0 and\n"
" * one s_setprio 3 up front, then 32 back-to-back v_wmma_f32_16x16x16_fp8_fp8,\n"
" * then s_setprio 0. By the time the second K-chunk reuses cv00, 16 distinct\n"
" * accums have been issued so the WMMA scoreboard is naturally clear. */\n"
"__global__ __launch_bounds__(128, 1)\n"
"void gemm_fp8_pipe32_asm(float *Y, const u8 *W, const u8 *X,\n"
"                         const float *bias, int N, int K, int M) {\n"
"    int tid = threadIdx.x;\n"
"    int wave_id = tid >> 5;\n"
"    int lane = tid & 31;\n"
...
"          \"s_wait_dscnt 0x0\\n\"\n"
"          \"s_setprio 3\\n\"\n"
"          \"v_wmma_f32_16x16x16_fp8_fp8 %0,  %16, %24, %0\\n\"\n"
"          \"v_wmma_f32_16x16x16_fp8_fp8 %1,  %16, %25, %1\\n\"\n"
"          \"v_wmma_f32_16x16x16_fp8_fp8 %2,  %16, %26, %2\\n\"\n"
"          \"v_wmma_f32_16x16x16_fp8_fp8 %3,  %16, %27, %3\\n\"\n"
"          \"v_wmma_f32_16x16x16_fp8_fp8 %4,  %17, %24, %4\\n\"\n"
"          \"v_wmma_f32_16x16x16_fp8_fp8 %5,  %17, %25, %5\\n\"\n"
"          \"v_wmma_f32_16x16x16_fp8_fp8 %6,  %17, %26, %6\\n\"\n"
"          \"v_wmma_f32_16x16x16_fp8_fp8 %7,  %17, %27, %7\\n\"\n"
"          \"v_wmma_f32_16x16x16_fp8_fp8 %8,  %18, %24, %8\\n\"\n"
"          \"v_wmma_f32_16x16x16_fp8_fp8 %9,  %18, %25, %9\\n\"\n"
"          \"v_wmma_f32_16x16x16_fp8_fp8 %10, %18, %26, %10\\n\"\n"
"          \"v_wmma_f32_16x16x16_fp8_fp8 %11, %18, %27, %11\\n\"\n"
"          \"v_wmma_f32_16x16x16_fp8_fp8 %12, %19, %24, %12\\n\"\n"
"          \"v_wmma_f32_16x16x16_fp8_fp8 %13, %19, %25, %13\\n\"\n"
"          \"v_wmma_f32_16x16x16_fp8_fp8 %14, %19, %26, %14\\n\"\n"
"          \"v_wmma_f32_16x16x16_fp8_fp8 %15, %19, %27, %15\\n\"\n"
"          \"v_wmma_f32_16x16x16_fp8_fp8 %0,  %20, %28, %0\\n\"\n"
"          \"v_wmma_f32_16x16x16_fp8_fp8 %1,  %20, %29, %1\\n\"\n"
"          \"v_wmma_f32_16x16x16_fp8_fp8 %2,  %20, %30, %2\\n\"\n"
"          \"v_wmma_f32_16x16x16_fp8_fp8 %3,  %20, %31, %3\\n\"\n"
"          \"v_wmma_f32_16x16x16_fp8_fp8 %4,  %21, %28, %4\\n\"\n"
"          \"v_wmma_f32_16x16x16_fp8_fp8 %5,  %21, %29, %5\\n\"\n"
"          \"v_wmma_f32_16x16x16_fp8_fp8 %6,  %21, %30, %6\\n\"\n"
"          \"v_wmma_f32_16x16x16_fp8_fp8 %7,  %21, %31, %7\\n\"\n"
"          \"v_wmma_f32_16x16x16_fp8_fp8 %8,  %22, %28, %8\\n\"\n"
"          \"v_wmma_f32_16x16x16_fp8_fp8 %9,  %22, %29, %9\\n\"\n"
"          \"v_wmma_f32_16x16x16_fp8_fp8 %10, %22, %30, %10\\n\"\n"
"          \"v_wmma_f32_16x16x16_fp8_fp8 %11, %22, %31, %11\\n\"\n"
"          \"v_wmma_f32_16x16x16_fp8_fp8 %12, %23, %28, %12\\n\"\n"
"          \"v_wmma_f32_16x16x16_fp8_fp8 %13, %23, %29, %13\\n\"\n"
"          \"v_wmma_f32_16x16x16_fp8_fp8 %14, %23, %30, %14\\n\"\n"
"          \"v_wmma_f32_16x16x16_fp8_fp8 %15, %23, %31, %15\\n\"\n"
"          \"s_setprio 0\\n\"\n"
"          : \"+v\"(cv00), \"+v\"(cv01), \"+v\"(cv02), \"+v\"(cv03),\n"
"            \"+v\"(cv10), \"+v\"(cv11), \"+v\"(cv12), \"+v\"(cv13),\n"
"            \"+v\"(cv20), \"+v\"(cv21), \"+v\"(cv22), \"+v\"(cv23),\n"
"            \"+v\"(cv30), \"+v\"(cv31), \"+v\"(cv32), \"+v\"(cv33)\n"
"          : \"v\"(ra00), \"v\"(ra01), \"v\"(ra02), \"v\"(ra03),\n"
"            \"v\"(ra10), \"v\"(ra11), \"v\"(ra12), \"v\"(ra13),\n"
"            \"v\"(rb00), \"v\"(rb01), \"v\"(rb02), \"v\"(rb03),\n"
"            \"v\"(rb10), \"v\"(rb11), \"v\"(rb12), \"v\"(rb13)\n"
"          : \"memory\");\n"
"        if (has_next) {\n"
"            int nb = 1 - buf;\n"
"            int nbase = nb * (128 * 4);\n"
...
static int load_kernels(int dev, fp8_kernels *out, int verbose) {
    ...
    HIP_CHECK(hipModuleGetFunction(&out->pipe32_prio, mod, "gemm_fp8_pipe32_prio"));
...
static int run_shape(const fp8_kernels *kn, const gemm_shape *s,
    ...
    else if (!strcmp(mode, "pipe32_prio"))  fn = kn->pipe32_prio;
