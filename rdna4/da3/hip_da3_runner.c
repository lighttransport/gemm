...
struct hip_da3_runner {
    ...
    int           use_wmma;       /* 1 iff DA3_WMMA!=0 and gemm_wmma resolved */
...
#define GET_FN(name) do { \
    ...
    /* Optional BF16-WMMA GEMM (gfx12). Default on; DA3_WMMA=0 disables. */
    r->use_wmma = 0;
    r->fn_gemm_wmma = NULL;
    {
        const char *e = getenv("DA3_WMMA");
        if (!e || e[0] != '0') {
...
static void kl_gemm(hip_da3_runner *r, void *Y, void *W_f16,
    ...
    /* BF16 WMMA when enabled and dims 16-aligned; same arg layout. Grid is
     * 128x128-tiled. Else the scalar 16x16 gemm_tiled_f16_f32 fallback. */
    if (r->use_wmma && r->fn_gemm_wmma &&
        (n_out % 16 == 0) && (n_in % 16 == 0)) {
        unsigned gx = (unsigned)((n_out + 127) / 128);
        unsigned gy = (unsigned)((n_tok + 127) / 128);
        KL(r->fn_gemm_wmma, gx, gy, 1, 256, 1, 1, 0, r->stream, args);
        return;
    }
