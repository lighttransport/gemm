/*
 * test_hip_da3.c - Test harness for HIP/ROCm DA3 depth estimation runner (RDNA4)
 *
 * Loads a DA3 model, runs HIP inference, prints statistics for all outputs.
 * Supports full DA3NESTED-GIANT-LARGE-1.1 outputs: depth, pose, rays, gaussians.
 *
 * Usage: ./test_hip_da3 <da3.gguf|model.safetensors> [-i input.ppm] [-o output.pgm]
 *        [--full] [--pose] [--rays] [--gaussians] [-d device_id]
 *
 * Compile with gcc (no hipcc needed).
 *
 * Port of test_cuda_da3.c for AMD ROCm/HIP.
 */
...
int main(int argc, char **argv) {
    ...
    fprintf(stderr, "\n=== Initializing HIP runner (device %d) ===\n", device_id);
    hip_da3_runner *gpu = hip_da3_init(device_id, verbose);
    if (!gpu) {
    ...
    if (result.depth && npix > 0) {
        ...
        if (dmin == dmax) { fprintf(stderr, "WARNING: all depth values are identical (%.4f)\n", dmin); pass = 0; }
        if (dmin < 0) { fprintf(stderr, "WARNING: negative depth values (min=%.4f)\n", dmin); pass = 0; }
        if (result.confidence && cmin < 1.0f - 1e-5f) { fprintf(stderr, "WARNING: confidence below 1.0 (min=%.4f)\n", cmin); pass = 0; }
        ...
        if (output_pgm && result.depth) {
            ...
            if (is_exr) {
                if (output_flags != DA3_OUTPUT_DEPTH)
                    ...
                    write_exr_depth(output_pgm, result.depth, result.confidence, result.width, result.height);
            } else if (is_png) {
            ...
            { char base[512]; strncpy(base, output_pgm, sizeof(base)-1); base[sizeof(base)-1] = '\0';
                char *dot = strrchr(base, '.'); if (dot) *dot = '\0';
                if (!is_png) { char fpath[512]; snprintf(fpath, sizeof(fpath), "%s_falsecolor.png", base); img_write_depth_png(fpath, result.depth, result.width, result.height); }
                if (!is_exr) { char fpath[512]; snprintf(fpath, sizeof(fpath), "%s_depth.exr", base); img_write_depth_exr(fpath, result.depth, result.width, result.height); }
            }
        }
    ...
    if (result.has_rays && result.rays) {
        fprintf(stderr, "\n--- Rays (Aux DPT) ---\n");
        for (int c = 0; c < 6; c++) {
            char label[32]; snprintf(label, sizeof(label), "ray_ch%d", c);
            print_stats(label, result.rays + (size_t)c * npix, npix);
        }
        if (result.ray_confidence) print_stats("ray_conf", result.ray_confidence, npix);
        if (result.sky_seg) print_stats("sky_seg", result.sky_seg, npix);
    }

    if (result.has_gaussians && result.gaussians) {
        fprintf(stderr, "\n--- Gaussians (GSDPT, 38 channels) ---\n");
        const char *gs_names[] = { "offset_x", "offset_y", "scale_0", "scale_1", "scale_2",
            "rot_0", "rot_1", "rot_2", "rot_3", NULL };
        for (int c = 0; c < 38; c++) {
            char label[32];
            if (c < 9 && gs_names[c]) snprintf(label, sizeof(label), "%s", gs_names[c]);
            else if (c < 36) snprintf(label, sizeof(label), "sh_%d", c - 9);
            else if (c == 36) snprintf(label, sizeof(label), "off_depth");
            else snprintf(label, sizeof(label), "opacity");
    ...
    if (npy_dir && npix > 0) {
        char path[512];
        if (result.has_pose) { snprintf(path, sizeof(path), "%s/pose_gpu.npy", npy_dir); write_npy_f32_1d(path, result.pose, 9); }
        if (result.has_rays && result.rays) {
            snprintf(path, sizeof(path), "%s/rays_gpu.npy", npy_dir); write_npy_f32_3d(path, result.rays, 6, result.height, result.width);
            if (result.ray_confidence) { snprintf(path, sizeof(path), "%s/ray_conf_gpu.npy", npy_dir); write_npy_f32(path, result.ray_confidence, result.width, result.height); }
        }
        if (result.has_gaussians && result.gaussians) {
            snprintf(path, sizeof(path), "%s/gaussians_gpu.npy", npy_dir); write_npy_f32_3d(path, result.gaussians, 38, result.height, result.width);
        }
    }
