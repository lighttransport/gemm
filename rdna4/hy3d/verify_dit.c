/*
 * verify_dit.c - Verify HIP DiT components against PyTorch reference
 *
...
static int test_timestep_embed(hip_hy3d_runner *r, const char *ref_dir, const char *out_dir) {
    ...
    /* Run HIP timestep embedding: t=0.5, dim=2048 */
     ...
     *   out = [sin(emb), cos(emb)]  */
    float *hip_out = (float *)malloc((size_t)dim * sizeof(float));
    int half = dim / 2;
    ...
    for (int i = 0; i < half; i++) {
        ...
        hip_out[i] = sinf(emb);
        hip_out[half + i] = cosf(emb);
    }

    /* Compare CPU-computed embedding against PyTorch reference */
    int ok = compare_f32("timestep_embed", ref, hip_out, dim, 1e-5f, 1e-5f);

    snprintf(path, sizeof(path), "%s/dit_timestep_embed.npy", out_dir);
    write_npy_f32(path, hip_out, dim);
...
static int test_dit_forward(hip_hy3d_runner *r, const char *model_path,
    ...
    if (hip_hy3d_load_weights(r, NULL, model_path, NULL) != 0) {
    ...
    /* Stats */
    float mn = hip_output[0], mx = hip_output[0], sm = 0;
    for (int i = 0; i < out_n; i++) {
        if (hip_output[i] < mn) mn = hip_output[i];
        if (hip_output[i] > mx) mx = hip_output[i];
        sm += hip_output[i];
    }
    ...
    if (fp) {
        ...
        fputc('\n', fp);
        fwrite(hip_output, sizeof(float), (size_t)out_n, fp);
        fclose(fp);
    }
...
int main(int argc, char **argv) {
    ...
    /* Init HIP */
    fprintf(stderr, "Initializing HIP...\n");
    hip_hy3d_runner *r = hip_hy3d_init(0, 1);
    if (use_f32) {
        hip_hy3d_set_f32_gemm(r, 1);
