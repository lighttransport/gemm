/*
 * verify_vae.c - Verify HIP ShapeVAE decoder against PyTorch reference
 *
 * Usage:
...
int main(int argc, char **argv) {
    ...
    if (hip_hy3d_load_weights(r, NULL, NULL, vae_path) != 0) {
        fprintf(stderr, "Failed to load VAE weights\n");
        hip_hy3d_free(r); free(latents); free(ref_sdf); return 1;
    }

    /* Run VAE */
    fprintf(stderr, "\nRunning ShapeVAE on GPU (grid %d^3)...\n", grid_res);
    int total_pts = grid_res * grid_res * grid_res;
    float *hip_sdf = (float *)calloc((size_t)total_pts, sizeof(float));

    if (hip_hy3d_run_vae(r, latents, grid_res, hip_sdf) != 0) {
        fprintf(stderr, "VAE forward pass failed\n");
        hip_hy3d_free(r); free(latents); free(ref_sdf); free(hip_sdf); return 1;
    }

    /* Stats */
    fprintf(stderr, "\nHIP SDF stats:\n");
    float mn = hip_sdf[0], mx = hip_sdf[0], sm = 0;
    for (int i = 0; i < total_pts; i++) {
        if (hip_sdf[i] < mn) mn = hip_sdf[i];
        if (hip_sdf[i] > mx) mx = hip_sdf[i];
        sm += hip_sdf[i];
    }
    ...
    if (ref_sdf && ref_grid_res == grid_res) {
        fprintf(stderr, "\nComparison:\n");
        compare_f32("vae_sdf_grid", ref_sdf, hip_sdf, total_pts, 1e-2f, 1e-2f);
    } else if (ref_sdf) {
    ...
    write_npy_f32_3d(path, hip_sdf, grid_res, grid_res, grid_res);
    fprintf(stderr, "Saved to %s\n", path);

    free(latents); free(ref_sdf); free(hip_sdf);
    hip_hy3d_free(r);
    return 0;
}
