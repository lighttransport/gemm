...
int main(int argc, char **argv) {
    ...
    hipFunction_t fn_gemm = NULL, fn_up = NULL, fn_idx = NULL, fn_ln = NULL, fn_silu = NULL;
    hipFunction_t fn_conv = NULL, fn_modln = NULL, fn_resadd = NULL;
    if (hipModuleGetFunction(&fn_gemm, mod, "gemm_f32_bias") != hipSuccess ||
        hipModuleGetFunction(&fn_up, mod, "slat_upsample2_nearest_f32") != hipSuccess ||
    ...
    hipDeviceptr_t d_coords_src = hip_upload_raw(coords_src, (size_t)N_src * 4 * sizeof(int32_t));
    hipDeviceptr_t d_coords_dst = hip_upload_raw(coords_dst, (size_t)N_dst * 4 * sizeof(int32_t));
    hipDeviceptr_t d_cat = hip_upload_raw(cat, X_src * sizeof(float));
    hipDeviceptr_t d_t = hip_upload_raw(t_emb, (size_t)dim * sizeof(float));
    hipDeviceptr_t d_up = 0, d_index = 0, d_t_silu = 0, d_emb = 0, d_skip = 0, d_h1 = 0, d_h2 = 0, d_h3 = 0, d_h4 = 0;
    if (!d_coords_src || !d_coords_dst || !d_cat || !d_t ||
        hipMalloc(&d_up, X_up * sizeof(float)) != hipSuccess ||
        ...
        hipMalloc(&d_h1, X_up * sizeof(float)) != hipSuccess ||
...
#define RUN_OUT_BLOCK0() do { \
    ...
    if (hipModuleLaunchKernel(fn_up, (N_dst + 127) / 128, 1, 1, 128, 1, 1, 0, 0, a_up, NULL) != hipSuccess) return 8; \
    ...
    if (hipModuleLaunchKernel(fn_idx, (N_dst + 255) / 256, 1, 1, 256, 1, 1, 0, 0, a_idx, NULL) != hipSuccess) return 8; \
    ...
    if (hipModuleLaunchKernel(fn_ln, N_dst, 1, 1, threads, 1, 1, (unsigned)ln_smem, 0, a_ln, NULL) != hipSuccess) return 8; \
    ...
    if (hipModuleLaunchKernel(fn_modln, N_dst, 1, 1, threads, 1, 1, (unsigned)ln_smem, 0, a_ml, NULL) != hipSuccess) return 8; \
...
#undef RUN_OUT_BLOCK0
    ...
    hipFree(d_coords_src); hipFree(d_coords_dst); hipFree(d_cat); hipFree(d_t); hipFree(d_up);
    hipFree(d_index); hipFree(d_t_silu); hipFree(d_emb); hipFree(d_skip);
