 ...
 * test_layernorm_token; not yet wired into any hip_sam3d_run_* path.
 *
 * Subsequent phases append per-stage kernels — see
 * cuda/sam3d_body/hip_sam3d_body_kernels.h and
 * cuda/trellis2/cuda_trellis2_kernels.h for templates.
 */

#ifndef HIP_SAM3D_KERNELS_H_
#define HIP_SAM3D_KERNELS_H_

static const char *hip_sam3d_kernel_src =
...
/* gemm_f32_bias_wmma — BF16-WMMA drop-in for gemm_f32_bias (gfx12).
 *
 *   Y(N, D_out) = X(N, D_in) @ W^T(D_out, D_in) + b(D_out)
 *
 * Identical signature/layout/semantics to gemm_f32_bias, but computes via
 * the RDNA4 v_wmma_f32_16x16x16_bf16 matrix engine. X, W, Y, b are F32; X and
 * W are truncated to BF16 (top-16-bits) at SMEM load. CTA tile 128x128, 256
 * threads = 8 waves in 2x4 (M x N); each wave does 4x2 = 8 WMMA ops.
 * Requires D_in % 16 == 0; N and D_out are bounds-checked. Adapted from
 * rdna4/hy3d hip_hy3d_kernels.h gemm_f16w_bf16a_wmma_t (F32 weights here).
 *
 * Grid: (ceil(D_out/128), ceil(N/128))   Block: (256,).
 */
"#if defined(__gfx1200__) || defined(__gfx1201__)\n"
"extern \"C\" __global__ void gemm_f32_bias_wmma(\n"
"    float *Y, const float *X, const float *W, const float *b,\n"
"    int N, int D_in, int D_out) {\n"
"    int tid = threadIdx.x;\n"
"    int wave_id = tid >> 5;\n"
...
"            int row = cta_m0 + er;\n"
"            int kp  = k + ek;\n"
"            float xv = (row < N && kp < D_in) ? X[(long)row * D_in + kp] : 0.f;\n"
"            unsigned int xbits; __builtin_memcpy(&xbits, &xv, 4);\n"
"            xbits += ((xbits >> 16) & 1u) + 0x7FFFu;  /* round-to-nearest-even */\n"
...
"            int col = cta_n0 + er;\n"
"            int kp  = k + ek;\n"
"            float wv = (col < D_out && kp < D_in) ? W[(long)col * D_in + kp] : 0.f;\n"
"            unsigned int wbits; __builtin_memcpy(&wbits, &wv, 4);\n"
"            wbits += ((wbits >> 16) & 1u) + 0x7FFFu;  /* round-to-nearest-even */\n"
...
"extern \"C\" __global__ void gemm_f32_bias_wmma(\n"
"    float *Y, const float *X, const float *W, const float *b,\n"
"    int N, int D_in, int D_out) { /* non-gfx12: host falls back to scalar */ }\n"
"#endif\n"
;

#endif /* HIP_SAM3D_KERNELS_H_ */
