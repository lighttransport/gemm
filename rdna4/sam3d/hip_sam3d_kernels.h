...
"    /* NaN/Inf detection via bit pattern (exponent == 0xFF). isfinite() is\n"
"     * miscompiled to constant-true under -ffast-math (-ffinite-math-only),\n"
"     * so MoGe's invalid (NaN) pixels would otherwise leak through. */\n"
"    unsigned ux, uy, uz;\n"
"    __builtin_memcpy(&ux, &x, 4);\n"
"    __builtin_memcpy(&uy, &y, 4);\n"
"    __builtin_memcpy(&uz, &z, 4);\n"
"    int nonfinite = (((ux >> 23) & 0xFFu) == 0xFFu) ||\n"
"                    (((uy >> 23) & 0xFFu) == 0xFFu) ||\n"
"                    (((uz >> 23) & 0xFFu) == 0xFFu);\n"
"    int valid = !nonfinite;\n"
...
"}\n"
/* ---- gemm_bf16w_bf16a_wmma_t: F32-weight x F32-act GEMM via BF16 WMMA (gfx12).
 * Ported verbatim from rdna4/trellis2/hip_trellis2_kernels.h. Same transposed-B
 * layout as gemm_f32_bias: Y[n_tok,n_out] = X[n_tok,n_in] * W[n_out,n_in]^T + bias.
 * CTA tile 128x128, 256 threads. Requires n_out%16==0, n_in%16==0; caller falls
 * back to gemm_f32_bias otherwise. Activations & weights BF16-truncated at SMEM
 * load; accumulation in F32. */
"#if defined(__gfx1200__) || defined(__gfx1201__)\n"
"extern \"C\" __global__ void gemm_bf16w_bf16a_wmma_t(float *Y, const float *W, const float *X,\n"
...
"            xbits += 0x7FFFu + ((xbits >> 16) & 1u);  /* round-to-nearest-even */\n"
...
"            wbits += 0x7FFFu + ((wbits >> 16) & 1u);  /* round-to-nearest-even */\n"
