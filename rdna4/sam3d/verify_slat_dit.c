 ...
 * hip_sam3d_debug_slat_dit_forward, isolating per-call DiT drift from
...
int main(int argc, char **argv)
{
    ...
    if (hip_sam3d_slat_dit_info(ctx, &in_ch, &out_ch, &cond_ch) != 0) {
        fprintf(stderr, "hip_sam3d_slat_dit_info failed\n");
    ...
    if (!cond || !is_f32 || nd != 3 || dims[2] != cond_ch) {
        ...
        hip_sam3d_destroy(ctx); return 4;
    }
    int n_cond = dims[1];
    ...
    int rc = 0;
    if (hip_sam3d_debug_slat_dit_forward(ctx, coords, feats, N, t_val,
                                          cond, n_cond, out) != 0) {
        fprintf(stderr, "hip_sam3d_debug_slat_dit_forward failed\n");
        rc = 6;
        goto cleanup;
    }
