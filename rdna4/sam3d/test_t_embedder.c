...
int main(int argc, char **argv)
{
    ...
    hipFunction_t fn_ts = 0, fn_gemm = 0, fn_silu = 0;
    if (hipModuleGetFunction(&fn_ts,   mod, "timestep_embed_cossin_f32") != hipSuccess) return 4;
    ...
    hipDeviceptr_t d_W1 = hip_upload_raw(W1, (size_t)dim * freq_dim * sizeof(float));
    hipDeviceptr_t d_b1 = hip_upload_raw(b1, (size_t)dim * sizeof(float));
    hipDeviceptr_t d_W2 = hip_upload_raw(W2, (size_t)dim * dim * sizeof(float));
    hipDeviceptr_t d_b2 = hip_upload_raw(b2, (size_t)dim * sizeof(float));
    hipDeviceptr_t d_emb = 0, d_h1 = 0, d_out = 0;
    hipMalloc(&d_emb, (size_t)freq_dim * sizeof(float));
    hipMalloc(&d_h1,  (size_t)dim      * sizeof(float));
    hipMalloc(&d_out, (size_t)dim      * sizeof(float));
    ...
    for (int k = 0; k < n_ts; k++) {
        ...
        /* Step 1: sinusoidal embed → d_emb [freq_dim]. */
        {
            ...
            if (hipModuleLaunchKernel(fn_ts, grid, 1, 1, 256, 1, 1, 0, 0, args, NULL) != hipSuccess) { ok = 0; break; }
        }
        /* Step 2: h1 = W1 @ emb + b1, gemm grid (1, ceil(dim/16)). */
        {
            unsigned gx = (N + 15) / 16, gy = (dim + 15) / 16;
            void *args[] = { &d_h1, &d_emb, &d_W1, &d_b1, &N, &freq_dim, &dim };
            if (hipModuleLaunchKernel(fn_gemm, gx, gy, 1, 16, 16, 1, 0, 0, args, NULL) != hipSuccess) { ok = 0; break; }
        }
        /* Step 3: silu_inplace(h1). */
        {
            unsigned grid = (unsigned)((dim + 255) / 256);
            void *args[] = { &d_h1, &dim };
            if (hipModuleLaunchKernel(fn_silu, grid, 1, 1, 256, 1, 1, 0, 0, args, NULL) != hipSuccess) { ok = 0; break; }
        }
        /* Step 4: out = W2 @ h1 + b2. */
        {
            unsigned gx = (N + 15) / 16, gy = (dim + 15) / 16;
        ...
        hipMemcpyDtoH(h_dst, d_out, (size_t)dim * sizeof(float));
    ...
    hipFree(d_emb); hipFree(d_h1); hipFree(d_out);
    hipModuleUnload(mod);
