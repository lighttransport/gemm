...
int main(int argc, char **argv)
{
    ...
    if (hipModuleGetFunction(&fn, mod, "slat_upsample2_nearest_f32") != hipSuccess) {
        fprintf(stderr, "lookup slat_upsample2_nearest_f32 failed\n");
        return 4;
    }

    hipDeviceptr_t d_src_coords = hip_upload_raw(src_coords, (size_t)src_N * 4 * sizeof(int32_t));
    hipDeviceptr_t d_src_feats = hip_upload_raw(src_feats, (size_t)src_N * C * sizeof(float));
    hipDeviceptr_t d_target_coords = hip_upload_raw(target_coords, (size_t)target_N * 4 * sizeof(int32_t));
    hipDeviceptr_t d_out = 0;
    if (!d_src_coords || !d_src_feats || !d_target_coords ||
        hipMalloc(&d_out, (size_t)target_N * C * sizeof(float)) != hipSuccess) {
    ...
    if (hipModuleLaunchKernel(fn, grid, 1, 1, 128, 1, 1, 0, 0, args, NULL) != hipSuccess) {
        fprintf(stderr, "launch failed\n");
        return 6;
    }
    hipDeviceSynchronize();
    hipMemcpyDtoH(dst_feats, d_out, (size_t)target_N * C * sizeof(float));
    ...
    hipFree(d_src_coords); hipFree(d_src_feats); hipFree(d_target_coords); hipFree(d_out);
    hipModuleUnload(mod);
