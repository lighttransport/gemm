...
int main(int argc, char **argv)
{
    ...
    if (hipModuleGetFunction(&fn, mod, "gemm_f32_bias") != hipSuccess) {
    ...
    hipDeviceptr_t d_b = hip_upload_raw(h_b, (size_t)D_out * sizeof(float));
    hipDeviceptr_t d_Y = 0;
    if (hipMalloc(&d_Y, n_Y * sizeof(float)) != hipSuccess) {
        fprintf(stderr, "hipMalloc d_Y failed\n"); return 5;
    }
    ...
    hipFree(d_X); hipFree(d_W); hipFree(d_b); hipFree(d_Y);
    hipModuleUnload(mod);
