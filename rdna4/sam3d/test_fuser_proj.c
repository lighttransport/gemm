...
int main(int argc, char **argv)
{
    ...
    if (sm < 0) return 4;
    hipFunction_t fn_ln = 0, fn_gemm = 0, fn_silu = 0;
    ...
    if (hipModuleGetFunction(&fn_silu, mod, "silu_mul_f32")         != hipSuccess) return 4;
    ...
    hipDeviceptr_t d_g  = hip_upload_raw(h_g,  (size_t)D_in * sizeof(float));
    hipDeviceptr_t d_b  = hip_upload_raw(h_b,  (size_t)D_in * sizeof(float));
    hipDeviceptr_t d_w1 = hip_upload_raw(h_w1, (size_t)ffn  * D_in  * sizeof(float));
    hipDeviceptr_t d_w3 = hip_upload_raw(h_w3, (size_t)ffn  * D_in  * sizeof(float));
    hipDeviceptr_t d_w2 = hip_upload_raw(h_w2, (size_t)D_out* ffn   * sizeof(float));
    hipDeviceptr_t d_ln = 0, d_t1 = 0, d_t3 = 0, d_y = 0;
    hipMalloc(&d_ln, n_x * sizeof(float));
    hipMalloc(&d_t1, n_h * sizeof(float));
    hipMalloc(&d_t3, n_h * sizeof(float));
    hipMalloc(&d_y,  n_y * sizeof(float));
    ...
    /* layernorm_token_f32 */
    {
        ...
        if (hipModuleLaunchKernel(fn_ln, n_tok, 1, 1, threads, 1, 1, shmem, 0, args, NULL) != hipSuccess) return 6;
    }
    /* gemm w1 → t1 */
    hipDeviceptr_t d_null = 0;
    {
        void *args[] = { &d_t1, &d_ln, &d_w1, &d_null, &n_tok, &D_in, &ffn };
        if (hipModuleLaunchKernel(fn_gemm, (n_tok+15)/16, (ffn+15)/16, 1, 16, 16, 1, 0, 0, args, NULL) != hipSuccess) return 6;
    }
    /* gemm w3 → t3 */
    {
    ...
    /* silu_mul: t1 = silu(t1) * t3 */
    {
        ...
        if (hipModuleLaunchKernel(fn_silu, blocks, 1, 1, threads, 1, 1, 0, 0, args, NULL) != hipSuccess) return 6;
    }
    /* gemm w2 → y */
    {
        void *args[] = { &d_y, &d_t1, &d_w2, &d_null, &n_tok, &ffn, &D_out };
        if (hipModuleLaunchKernel(fn_gemm, (n_tok+15)/16, (D_out+15)/16, 1, 16, 16, 1, 0, 0, args, NULL) != hipSuccess) return 6;
    }
    hipDeviceSynchronize();
    hipMemcpyDtoH(h_dst, d_y, n_y * sizeof(float));
    ...
    hipFree(d_x); hipFree(d_g); hipFree(d_b);
    hipFree(d_w1); hipFree(d_w3); hipFree(d_w2);
    hipFree(d_ln); hipFree(d_t1); hipFree(d_t3); hipFree(d_y);
    hipModuleUnload(mod);
