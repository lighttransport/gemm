...
- **5b.28 / 6b.10 — SLAT coord cache + SLAT-to-GS device mirror
  ...
  `/tmp/hip_sam3d_5b28_6b10_timed.ply` in real=118.12 s; that PLY is
  byte-identical to `/tmp/hip_sam3d_6b9_timed.ply`.
- **6b.9 — SLAT GS device-connected transformer-to-PLY path (GREEN).**
  ...
  `hip_sam3d_run_slat_gs_decode` now keeps GS coords plus final
  ...
  mean_abs=1.011e-07. Full sampled E2E writes 38016 gaussians to
  `/tmp/hip_sam3d_6b9_timed.ply` in real=118.01 s; that PLY is
  byte-identical to `/tmp/hip_sam3d_6b8_timed.ply`.
- **6b.8 — SLAT GS full-transformer CUDA hook (GREEN).** Added
  ...
  gaussians to `/tmp/hip_sam3d_6b8_timed.ply` in real=118.09 s.
- **6b.7 — SLAT GS full-stack CUDA hook (GREEN).** Added
  ...
  max_abs=1.040e-04, mean_abs=4.753e-06; CUDA packed_ply vs CPU pack
  max_abs=2.861e-06, mean_abs=1.011e-07. Full sampled E2E writes 38016
  gaussians to `/tmp/hip_sam3d_6b7_timed.ply` in real=118.09 s.
- **6b.6 — SLAT GS full-block CUDA hook (GREEN).** Added
  ...
  `/tmp/hip_sam3d_6b6_timed.ply` in real=118.09 s.
- **6b.5 — SLAT GS self-attention subblock CUDA hook (GREEN).** Added
  ...
  max_abs=1.040e-04, mean_abs=4.753e-06; CUDA packed_ply vs CPU pack
  max_abs=2.861e-06, mean_abs=1.011e-07. Full sampled E2E writes 38016
  gaussians to `/tmp/hip_sam3d_6b5_timed.ply` in real=118.14 s.
- **6b.4 — SLAT GS MLP CUDA hook (GREEN).** Added
  ...
  `/tmp/hip_sam3d_6b4_timed.ply` in real=118.34 s.
- **6b.3 — SLAT GS shifted-window attention CUDA hook (GREEN).** Added
  ...
  max_abs=2.861e-06, mean_abs=1.011e-07. Full sampled E2E writes 38016
  gaussians to `/tmp/hip_sam3d_6b3_timed.ply` in real=118.72 s.
- **6b.2 — SLAT GS dense-end CUDA hooks (GREEN).** Added
  ...
  sampled E2E writes 38016 gaussians to
  `/tmp/hip_sam3d_6b2_timed.ply` in real=119.55 s.
- **6b.1 — SLAT GS CUDA representation + PLY pack (GREEN).** Added
  ...
  `/tmp/hip_sam3d_5b27_6b1_timed.ply` in real=120.52 s.
...
- **5b.26 — combined SLAT APE + transformer CUDA hook (GREEN).** Added
  ...
  writes 38016 gaussians to `/tmp/hip_sam3d_5b26_timed.ply` in
...
- **5b.25 — SLAT final-head CUDA runner hook (GREEN).** Added
  ...
  sampled E2E writes 38016 gaussians to
  `/tmp/hip_sam3d_5b25_timed.ply` in real=120.62 s
...
- **5b.24 — persistent SLAT sparse IO hook scratch buffers (GREEN).**
  ...
  `hip_sam3d_ctx`, growing them only when a later block needs more
  ...
  `/tmp/hip_sam3d_5b24_timed.ply` in real=120.40 s; this is a small
...
- **5b.23 — SLAT sparse IO CUDA runner hook (GREEN).** Added
  ...
  hook and wired `hip_sam3d_runner.c` to install a CUDA implementation
  ...
  mean_abs=4.471638e-06. Full sampled E2E writes 38016 gaussians to
  `/tmp/hip_sam3d_5b23_timed.ply` in real=120.65 s
  (`/usr/bin/time -p`).
...
- **5b.18 — persistent SLAT hook scratch buffers (GREEN).** The hybrid
  ...
  `hip_sam3d_ctx`, growing them only when a later call needs more
  ...
  E2E writes 38016 gaussians to `/tmp/hip_sam3d_5b18_timed.ply` in
...
- **5b.17 — resident cond-token reuse in SLAT transformer hook (GREEN).**
  ...
  `/tmp/hip_sam3d_5b17_timed.ply` in real=126.92 s
  (`/usr/bin/time -p`).
- **5b.16 — hybrid SLAT ODE body with GPU transformer stack (GREEN).**
  ...
  and wired `hip_sam3d_runner.c` to install it around
  ...
  gaussians to `/tmp/hip_sam3d_5b16_timed.ply` in real=128.73 s
  (`/usr/bin/time -p`).
- **5b.15 — reusable SLAT DiT transformer stack forward driver
  (GREEN).** Added `hip_sam3d_slat_dit_forward.h` with
...
- **5b.14 — persistent SLAT DiT transformer GPU weight container
  (GREEN).** Added `hip_sam3d_slat_dit_gpu.h` with
...
- **5b.0 — SLAT occupancy prune on GPU (GREEN).** Added
  ...
  `/tmp/hip_sam3d_phase5b0.ply`, elapsed=231.06 s.
- **7a.1 — sampled E2E unblocked (GREEN).** Fixed SS sampler wrapper
  ...
  gaussians to `/tmp/hip_sam3d_phase4b.ply`, elapsed=267.37 s.
- **4b.3 — SS-VAE decoder runner swap (GREEN).** Added
  `hip_sam3d_ss_decoder_gpu.h` (281.0 MiB device weights) and
  `hip_sam3d_ss_decoder_forward.h`, composed from
  `conv3d_k3_pad1_f32`, `channel_layernorm_3d_f32`,
  `silu_inplace_f32`, `pixel_shuffle_3d_f32`, and `residual_add_f32`.
  `hip_sam3d_run_ss_decode` now calls the GPU path and keeps the host
