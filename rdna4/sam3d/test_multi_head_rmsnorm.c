...
static int run_case(hipFunction_t fn, const char *label, int n_tok, int n_heads,
                    int head_dim, int stride, float threshold, int verbose)
{
    ...
    hipDeviceptr_t d_x = hip_upload_raw(x_init, Nx * sizeof(float));
    hipDeviceptr_t d_g = hip_upload_raw(gamma,  (size_t)n_heads * head_dim * sizeof(float));

    unsigned threads = 64;
    if (head_dim < 64) threads = 32;
    void *args[] = { &d_x, &d_g, &n_tok, &n_heads, &head_dim, &stride };
    size_t smem = threads * sizeof(float);
    if (hipModuleLaunchKernel(fn, (unsigned)n_heads, (unsigned)n_tok, 1,
                       threads, 1, 1, (unsigned)smem, 0, args, NULL) != hipSuccess) {
        fprintf(stderr, "launch failed (%s)\n", label); return 1;
    }
    hipDeviceSynchronize();

    float *h_dst = (float *)malloc(Nx * sizeof(float));
    hipMemcpyDtoH(h_dst, d_x, Nx * sizeof(float));
...
int main(int argc, char **argv)
{
    ...
    hipFunction_t fn = 0;
    if (hipModuleGetFunction(&fn, mod, "multi_head_rmsnorm_f32") != hipSuccess) {
