...
int main(int argc, char **argv)
{
    ...
    if (hipModuleGetFunction(&fn, mod, "slat_ape_add_f32") != hipSuccess) {
        fprintf(stderr, "lookup slat_ape_add_f32 failed\n");
        return 4;
    }

    hipDeviceptr_t d_feat = hip_upload_raw(h_in, n_feat * sizeof(float));
    ...
    hipMemcpyDtoH(h_dst, d_feat, n_feat * sizeof(float));
    ...
    double avg_ms = 0.0;
    if (repeat > 0) {
        hipMemcpyHtoD(d_feat, h_in, n_feat * sizeof(float));
        hipDeviceSynchronize();
        double t0 = now_ms();
