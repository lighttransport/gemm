 ...
 * HIP_SAM3D_E_NOT_IMPLEMENTED until their NVRTC kernels land.
 *
...
#ifndef HIP_SAM3D_RUNNER_H_
#define HIP_SAM3D_RUNNER_H_
...
typedef struct hip_sam3d_ctx hip_sam3d_ctx;
 ...
 * were injected via hip_sam3d_debug_override_*). */
...
int hip_sam3d_get_cond_tokens(hip_sam3d_ctx *ctx, float *out,
                               int *out_n, int *out_c);
int hip_sam3d_get_ss_latent(hip_sam3d_ctx *ctx, float *out, int *out_dims /*4*/);
int hip_sam3d_get_occupancy(hip_sam3d_ctx *ctx, float *out, int *out_dims /*3*/);
 ...
 * are 5 buffers per modality (sized via hip_sam3d_ss_dit_lat_elts).
 * cond: [n_cond × cond_channels] f32; query channels via
 * hip_sam3d_ss_dit_info(). */
...
int hip_sam3d_ss_dit_n_latents(void);
int hip_sam3d_ss_dit_lat_elts (int modality_id);
 ...
 * hip_sam3d_debug_slat_gs_transformer) into per-gaussian buffers. Each
 * out pointer may be NULL to skip. Sizes:
 *   xyz [N*G, 3], dc [N*G, 3], scaling [N*G, 3], rotation [N*G, 4],
 *   opacity [N*G]. G = hip_sam3d_slat_gs_info().num_gaussians. */
 ...
 * out_feats. Returns malloc'd [N*G, HIP_SAM3D_GS_STRIDE] f32. */
...
/* Status codes. */
#define HIP_SAM3D_E_OK              ( 0)
#define HIP_SAM3D_E_INVAL           (-1)
#define HIP_SAM3D_E_NOT_IMPLEMENTED (-2)
#define HIP_SAM3D_E_LOAD            (-3)
#define HIP_SAM3D_E_NO_INPUT        (-4)

#ifdef __cplusplus
}
#endif

#endif /* HIP_SAM3D_RUNNER_H_ */
