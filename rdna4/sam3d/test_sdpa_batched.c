...
int main(int argc, char **argv)
{
    ...
    if (sm < 0) return 4;
    hipFunction_t fn_b = 0, fn_s = 0;
    if (hipModuleGetFunction(&fn_b, mod, "sdpa_batched_f32") != hipSuccess) return 4;
    if (hipModuleGetFunction(&fn_s, mod, "sdpa_f32")         != hipSuccess) return 4;
    ...
    hipDeviceptr_t d_q = hip_upload_raw(h_q, n_total * sizeof(float));
    hipDeviceptr_t d_k = hip_upload_raw(h_k, n_total * sizeof(float));
    hipDeviceptr_t d_v = hip_upload_raw(h_v, n_total * sizeof(float));
    hipDeviceptr_t d_o = 0;
    if (hipMalloc(&d_o, n_total * sizeof(float)) != hipSuccess) return 5;
    ...
    /* Batched run. */
    {
        ...
        if (hipModuleLaunchKernel(fn_b, N, H, B, threads, 1, 1, shmem, 0, args, NULL) != hipSuccess) {
            fprintf(stderr, "batched launch failed\n"); return 6;
        }
        hipDeviceSynchronize();
        hipMemcpyDtoH(h_o_b, d_o, n_total * sizeof(float));
    }
    ...
    /* Reference: per-batch sdpa_f32. */
    {
        ...
        size_t shmem = (threads + N) * sizeof(float);
        for (int b = 0; b < B; b++) {
            hipDeviceptr_t q_b = d_q + b * n_per * sizeof(float);
            hipDeviceptr_t k_b = d_k + b * n_per * sizeof(float);
            hipDeviceptr_t v_b = d_v + b * n_per * sizeof(float);
            hipDeviceptr_t o_b = d_o + b * n_per * sizeof(float);
            void *args[] = { &o_b, &q_b, &k_b, &v_b, &N, &N, &H, &D_h, &scale };
            if (hipModuleLaunchKernel(fn_s, N, H, 1, threads, 1, 1, shmem, 0, args, NULL) != hipSuccess) {
                fprintf(stderr, "sdpa_f32 launch (batch %d) failed\n", b); return 6;
            }
        }
        hipDeviceSynchronize();
        hipMemcpyDtoH(h_o_s, d_o, n_total * sizeof(float));
    }
