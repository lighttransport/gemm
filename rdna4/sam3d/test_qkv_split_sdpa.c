...
int main(int argc, char **argv)
{
    ...
    if (hipModuleGetFunction(&fn_sdpa, mod, "sdpa_f32") != hipSuccess) {
    ...
    hipDeviceptr_t d_qkv = hip_upload_raw(h_qkv, n_qkv * sizeof(float));
    hipDeviceptr_t d_Q = 0, d_K = 0, d_V = 0, d_out = 0;
    if (hipMalloc(&d_Q,   n_act * sizeof(float)) != hipSuccess ||
        hipMalloc(&d_K,   n_act * sizeof(float)) != hipSuccess ||
        hipMalloc(&d_V,   n_act * sizeof(float)) != hipSuccess ||
        hipMalloc(&d_out, n_act * sizeof(float)) != hipSuccess) {
    ...
    /* qkv_split: 1D grid over N*dim. */
    {
        ...
        if (hipModuleLaunchKernel(fn_split,
                           blocks, 1, 1,
                           threads, 1, 1,
     ...
     * as an SDPA drift. */
    hipMemcpyDtoH(h_Q_d, d_Q, n_act * sizeof(float));
    hipMemcpyDtoH(h_K_d, d_K, n_act * sizeof(float));
    hipMemcpyDtoH(h_V_d, d_V, n_act * sizeof(float));
    double dummy = 0.0;
    ...
    /* sdpa: grid (N, H), block 256, shmem (256 + N_k) * 4. */
    {
        ...
        if (hipModuleLaunchKernel(fn_sdpa,
                           N_q, H, 1,
                           threads, 1, 1,
                           shmem, 0, args, NULL) != hipSuccess) {
            fprintf(stderr, "launch sdpa failed\n"); return 6;
        }
    }
    hipDeviceSynchronize();
    hipMemcpyDtoH(h_dst, d_out, n_act * sizeof(float));
