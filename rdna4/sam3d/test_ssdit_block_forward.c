...
typedef struct {
    hipFunction_t gemm, mod_ln, ln, qkv_split, kv_split, mhrms, sdpa, gelu, gated, resadd;
} fn_t;

static int upload_sw(hipDeviceptr_t *bufs, const sw_t *w, int dim, int H_dim) {
    int i = 0, D2 = 2 * dim, D3 = 3 * dim;
    bufs[i++] = hip_upload_raw(w->qkv_w,    (size_t)D3 * dim * sizeof(float));
    bufs[i++] = hip_upload_raw(w->qkv_b,    (size_t)D3 * sizeof(float));
    bufs[i++] = hip_upload_raw(w->gamma_q,  (size_t)dim * sizeof(float));
    bufs[i++] = hip_upload_raw(w->gamma_k,  (size_t)dim * sizeof(float));
    bufs[i++] = hip_upload_raw(w->sa_out_w, (size_t)dim * dim * sizeof(float));
    bufs[i++] = hip_upload_raw(w->sa_out_b, (size_t)dim * sizeof(float));
    bufs[i++] = hip_upload_raw(w->n2_w,     (size_t)dim * sizeof(float));
    bufs[i++] = hip_upload_raw(w->n2_b,     (size_t)dim * sizeof(float));
    bufs[i++] = hip_upload_raw(w->xq_w,     (size_t)dim * dim * sizeof(float));
    bufs[i++] = hip_upload_raw(w->xq_b,     (size_t)dim * sizeof(float));
    bufs[i++] = hip_upload_raw(w->xkv_w,    (size_t)D2 * dim * sizeof(float));
    bufs[i++] = hip_upload_raw(w->xkv_b,    (size_t)D2 * sizeof(float));
    bufs[i++] = hip_upload_raw(w->xo_w,     (size_t)dim * dim * sizeof(float));
    bufs[i++] = hip_upload_raw(w->xo_b,     (size_t)dim * sizeof(float));
    bufs[i++] = hip_upload_raw(w->fc1_w,    (size_t)H_dim * dim * sizeof(float));
    bufs[i++] = hip_upload_raw(w->fc1_b,    (size_t)H_dim * sizeof(float));
    bufs[i++] = hip_upload_raw(w->fc2_w,    (size_t)dim * H_dim * sizeof(float));
    bufs[i++] = hip_upload_raw(w->fc2_b,    (size_t)dim * sizeof(float));
    return i;
}
...
static int gpu_sa_qkv(const fn_t *fn, const hipDeviceptr_t *swb,
                      hipDeviceptr_t d_x, int N, int dim, int H, int D_h,
                      hipDeviceptr_t d_qkv, hipDeviceptr_t d_q, hipDeviceptr_t d_k, hipDeviceptr_t d_v)
{
    int D3 = 3 * dim;
    {
        unsigned gx = (N + 15) / 16, gy = (D3 + 15) / 16;
        void *args[] = { &d_qkv, &d_x, (void *)&swb[SWB_QKVW], (void *)&swb[SWB_QKVB], &N, &dim, &D3 };
        if (hipModuleLaunchKernel(fn->gemm, gx, gy, 1, 16, 16, 1, 0, 0, args, NULL) != hipSuccess) return 1;
    }
    {
        unsigned grid = (unsigned)((N * dim + 255) / 256);
        void *args[] = { &d_q, &d_k, &d_v, &d_qkv, &N, &dim };
        if (hipModuleLaunchKernel(fn->qkv_split, grid, 1, 1, 256, 1, 1, 0, 0, args, NULL) != hipSuccess) return 1;
    }
    unsigned threads = 64;
    ...
    int stride = dim;
    {
        ...
        if (hipModuleLaunchKernel(fn->mhrms, (unsigned)H, (unsigned)N, 1, threads, 1, 1,
...
int main(int argc, char **argv)
{
    ...
    fn_t fn;
    if (hipModuleGetFunction(&fn.gemm,      mod, "gemm_f32_bias")           != hipSuccess) return 4;
    if (hipModuleGetFunction(&fn.mod_ln,    mod, "modulated_ln_f32")        != hipSuccess) return 4;
    if (hipModuleGetFunction(&fn.ln,        mod, "layernorm_token_f32")     != hipSuccess) return 4;
    if (hipModuleGetFunction(&fn.qkv_split, mod, "qkv_split_f32")           != hipSuccess) return 4;
    if (hipModuleGetFunction(&fn.kv_split,  mod, "kv_split_f32")            != hipSuccess) return 4;
    if (hipModuleGetFunction(&fn.mhrms,     mod, "multi_head_rmsnorm_f32")  != hipSuccess) return 4;
    if (hipModuleGetFunction(&fn.sdpa,      mod, "sdpa_f32")                != hipSuccess) return 4;
    if (hipModuleGetFunction(&fn.gelu,      mod, "gelu_tanh_inplace_f32")   != hipSuccess) return 4;
    if (hipModuleGetFunction(&fn.gated,     mod, "gated_residual_add_f32")  != hipSuccess) return 4;
    if (hipModuleGetFunction(&fn.resadd,    mod, "residual_add_f32")        != hipSuccess) return 4;
    ...
    hipDeviceptr_t d_smsa = hip_upload_raw(shift_msa, (size_t)dim * sizeof(float));
    hipDeviceptr_t d_cmsa = hip_upload_raw(scale_msa, (size_t)dim * sizeof(float));
    hipDeviceptr_t d_gmsa = hip_upload_raw(gate_msa,  (size_t)dim * sizeof(float));
    hipDeviceptr_t d_smlp = hip_upload_raw(shift_mlp, (size_t)dim * sizeof(float));
    hipDeviceptr_t d_cmlp = hip_upload_raw(scale_mlp, (size_t)dim * sizeof(float));
    hipDeviceptr_t d_gmlp = hip_upload_raw(gate_mlp,  (size_t)dim * sizeof(float));
    hipDeviceptr_t swsb[SWB_N], swpb[SWB_N];
    upload_sw(swsb, &ws, dim, H_dim);
    upload_sw(swpb, &wp, dim, H_dim);

    /* Scratch buffers. */
    hipDeviceptr_t d_hs = 0, d_hp = 0;
    hipDeviceptr_t d_qkvs = 0, d_qkvp = 0;
    hipDeviceptr_t d_qs = 0, d_ks = 0, d_vs = 0, d_qp = 0, d_kp = 0, d_vp = 0;
    hipDeviceptr_t d_kpkv = 0, d_vpkv = 0;
    hipDeviceptr_t d_sdpa_s = 0, d_sdpa_p = 0;
    hipDeviceptr_t d_t_s = 0, d_t_p = 0;
    hipDeviceptr_t d_kvs_x = 0, d_kvp_x = 0;
    hipDeviceptr_t d_Ks = 0, d_Vs = 0, d_Kp = 0, d_Vp = 0;
    hipDeviceptr_t d_qx_s = 0, d_qx_p = 0, d_o_s = 0, d_o_p = 0;
    hipDeviceptr_t d_m1s = 0, d_m1p = 0;
    hipMalloc(&d_hs, (size_t)N_s * dim * sizeof(float));
    hipMalloc(&d_hp, (size_t)N_p * dim * sizeof(float));
    hipMalloc(&d_qkvs, (size_t)N_s * D3 * sizeof(float));
    hipMalloc(&d_qkvp, (size_t)N_p * D3 * sizeof(float));
    ...
    hipMalloc(&d_sdpa_s, (size_t)N_s * dim * sizeof(float));
    hipMalloc(&d_sdpa_p, (size_t)N_p * dim * sizeof(float));
    hipMalloc(&d_t_s, (size_t)N_s * dim * sizeof(float));
    hipMalloc(&d_t_p, (size_t)N_p * dim * sizeof(float));
    hipMalloc(&d_kvs_x, (size_t)N_c * D2 * sizeof(float));
    hipMalloc(&d_kvp_x, (size_t)N_c * D2 * sizeof(float));
    hipMalloc(&d_Ks, (size_t)N_c * dim * sizeof(float));
    hipMalloc(&d_Vs, (size_t)N_c * dim * sizeof(float));
    hipMalloc(&d_Kp, (size_t)N_c * dim * sizeof(float));
    hipMalloc(&d_Vp, (size_t)N_c * dim * sizeof(float));
    hipMalloc(&d_qx_s, (size_t)N_s * dim * sizeof(float));
    hipMalloc(&d_qx_p, (size_t)N_p * dim * sizeof(float));
    hipMalloc(&d_o_s, (size_t)N_s * dim * sizeof(float));
    hipMalloc(&d_o_p, (size_t)N_p * dim * sizeof(float));
    hipMalloc(&d_m1s, (size_t)N_s * H_dim * sizeof(float));
    hipMalloc(&d_m1p, (size_t)N_p * H_dim * sizeof(float));
    ...
    /* === norm1 + adaLN_msa === */
    {
        ...
        if (hipModuleLaunchKernel(fn.mod_ln, (unsigned)N_s, 1, 1, threads, 1, 1,
    ...
    }
    {
        ...
        if (hipModuleLaunchKernel(fn.mod_ln, (unsigned)N_p, 1, 1, threads, 1, 1,
                           (unsigned)smem, 0, args, NULL) != hipSuccess) return 5;
    }
    ...
    /* shape SDPA self only. */
    {
        ...
        if (hipModuleLaunchKernel(fn.sdpa, (unsigned)N_s, (unsigned)H, 1,
                           threads, 1, 1, (unsigned)sm, 0, args, NULL) != hipSuccess) return 5;
    }
    ...
    hipMemcpyDtoD(d_vpkv + (size_t)N_p * dim * sizeof(float),   d_vs, (size_t)N_s * dim * sizeof(float));
    {
        ...
        if (hipModuleLaunchKernel(fn.sdpa, (unsigned)N_p, (unsigned)H, 1,
                           threads, 1, 1, (unsigned)sm, 0, args, NULL) != hipSuccess) return 5;
    }
    /* sa out_proj per stream. */
    {
        ...
        if (hipModuleLaunchKernel(fn.gemm, gx, gy, 1, 16, 16, 1, 0, 0, args, NULL) != hipSuccess) return 5;
    }
    {
    ...
    /* gated residual add. */
    {
        ...
        if (hipModuleLaunchKernel(fn.gated, grid, 1, 1, 256, 1, 1, 0, 0, args, NULL) != hipSuccess) return 5;
    }
    {
    ...
    /* === norm2 (affine) === */
    {
        ...
        if (hipModuleLaunchKernel(fn.ln, (unsigned)N_s, 1, 1, threads, 1, 1, (unsigned)smem, 0, args, NULL) != hipSuccess) return 5;
    }
    {
        unsigned threads = 256; size_t smem = 2 * threads * sizeof(float);
        void *args[] = { &d_hp, &d_xp, &swpb[SWB_N2W], &swpb[SWB_N2B], &N_p, &dim, &eps, &affine };
        if (hipModuleLaunchKernel(fn.ln, (unsigned)N_p, 1, 1, threads, 1, 1, (unsigned)smem, 0, args, NULL) != hipSuccess) return 5;
    }
    /* === cross-attn === */
    {
        unsigned gx = (N_s + 15) / 16, gy = (dim + 15) / 16;
    ...
    }
    {
        ...
        if (hipModuleLaunchKernel(fn.kv_split, grid, 1, 1, 256, 1, 1, 0, 0, args1, NULL) != hipSuccess) return 5;
        void *args2[] = { &d_Kp, &d_Vp, &d_kvp_x, &N_c, &dim };
        if (hipModuleLaunchKernel(fn.kv_split, grid, 1, 1, 256, 1, 1, 0, 0, args2, NULL) != hipSuccess) return 5;
    }
    {
    ...
    /* residual (no gate). */
    {
        ...
        if (hipModuleLaunchKernel(fn.resadd, grid, 1, 1, 256, 1, 1, 0, 0, args, NULL) != hipSuccess) return 5;
    }
    {
        int total = N_p * dim;
        unsigned grid = (unsigned)((total + 255) / 256);
    ...
    /* === norm3 + adaLN_mlp + FFN + gated residual === */
    {
        ...
        if (hipModuleLaunchKernel(fn.mod_ln, (unsigned)N_s, 1, 1, threads, 1, 1, (unsigned)smem, 0, args, NULL) != hipSuccess) return 5;
    }
    {
        unsigned threads = 256; size_t smem = 2 * threads * sizeof(float);
        void *args[] = { &d_hp, &d_xp, &d_smlp, &d_cmlp, &N_p, &dim, &eps };
        if (hipModuleLaunchKernel(fn.mod_ln, (unsigned)N_p, 1, 1, threads, 1, 1, (unsigned)smem, 0, args, NULL) != hipSuccess) return 5;
    }
    /* fc1 */
    {
    ...
    /* gelu */
    {
        ...
        if (hipModuleLaunchKernel(fn.gelu, grid, 1, 1, 256, 1, 1, 0, 0, args, NULL) != hipSuccess) return 5;
    }
    {
        int total = N_p * H_dim;
        unsigned grid = (unsigned)((total + 255) / 256);
    ...
    hipMemcpyDtoH(xs_dst, d_xs, (size_t)N_s * dim * sizeof(float));
    hipMemcpyDtoH(xp_dst, d_xp, (size_t)N_p * dim * sizeof(float));
    ...
    hipFree(d_xs); hipFree(d_xp); hipFree(d_cond);
    hipFree(d_smsa); hipFree(d_cmsa); hipFree(d_gmsa);
    hipFree(d_smlp); hipFree(d_cmlp); hipFree(d_gmlp);
    for (int i = 0; i < SWB_N; i++) { hipFree(swsb[i]); hipFree(swpb[i]); }
    hipFree(d_hs); hipFree(d_hp);
    ...
    hipFree(d_sdpa_s); hipFree(d_sdpa_p);
    hipFree(d_t_s); hipFree(d_t_p);
    hipFree(d_kvs_x); hipFree(d_kvp_x);
    hipFree(d_Ks); hipFree(d_Vs); hipFree(d_Kp); hipFree(d_Vp);
    hipFree(d_qx_s); hipFree(d_qx_p); hipFree(d_o_s); hipFree(d_o_p);
    hipFree(d_m1s); hipFree(d_m1p);
    hipModuleUnload(mod);
