...
int main(int argc, char **argv)
{
    ...
    if (hipModuleGetFunction(&fn_split, mod, "kv_split_f32") != hipSuccess ||
        hipModuleGetFunction(&fn_sdpa, mod, "sdpa_f32") != hipSuccess) {
        fprintf(stderr, "kernel lookup failed\n");
        return 4;
    }

    hipDeviceptr_t d_q = hip_upload_raw(q, q_n * sizeof(float));
    hipDeviceptr_t d_kv = hip_upload_raw(kv, kv_n * sizeof(float));
    hipDeviceptr_t d_K = 0, d_V = 0, d_out = 0;
    if (!d_q || !d_kv ||
        hipMalloc(&d_K, c_n * sizeof(float)) != hipSuccess ||
        hipMalloc(&d_V, c_n * sizeof(float)) != hipSuccess ||
        hipMalloc(&d_out, q_n * sizeof(float)) != hipSuccess) {
        fprintf(stderr, "device alloc/upload failed\n");
        return 5;
    }

    void *split_args[] = { &d_K, &d_V, &d_kv, &N_c, &dim };
    if (hipModuleLaunchKernel(fn_split, (int)((c_n + 255) / 256), 1, 1,
    ...
    hipMemcpyDtoH(dst, d_out, q_n * sizeof(float));
    ...
    hipFree(d_q); hipFree(d_kv); hipFree(d_K); hipFree(d_V); hipFree(d_out);
    hipModuleUnload(mod);
