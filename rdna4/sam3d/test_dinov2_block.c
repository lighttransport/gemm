 ...
 * defined in hip_sam3d_dinov2_forward.h) on DINOv2-L geometry:
...
int main(int argc, char **argv)
{
    ...
    bw.ln1_w  = hip_upload_raw(h_ln1w,  (size_t)dim * sizeof(float));
    bw.ln1_b  = hip_upload_raw(h_ln1b,  (size_t)dim * sizeof(float));
    bw.qkv_w  = hip_upload_raw(h_qkvw,  n_qkvw * sizeof(float));
    bw.qkv_b  = hip_upload_raw(h_qkvb,  (size_t)(3 * dim) * sizeof(float));
    bw.proj_w = hip_upload_raw(h_projw, n_projw * sizeof(float));
    bw.proj_b = hip_upload_raw(h_projb, (size_t)dim * sizeof(float));
    bw.ls1    = hip_upload_raw(h_ls1,   (size_t)dim * sizeof(float));
    bw.ln2_w  = hip_upload_raw(h_ln2w,  (size_t)dim * sizeof(float));
    bw.ln2_b  = hip_upload_raw(h_ln2b,  (size_t)dim * sizeof(float));
    bw.fc1_w  = hip_upload_raw(h_fc1w,  n_fc1w * sizeof(float));
    bw.fc1_b  = hip_upload_raw(h_fc1b,  (size_t)ffn * sizeof(float));
    bw.fc2_w  = hip_upload_raw(h_fc2w,  n_fc2w * sizeof(float));
    bw.fc2_b  = hip_upload_raw(h_fc2b,  (size_t)dim * sizeof(float));
    bw.ls2    = hip_upload_raw(h_ls2,   (size_t)dim * sizeof(float));
    ...
    hipFree(bw.ln1_w);  hipFree(bw.ln1_b);
    hipFree(bw.qkv_w);  hipFree(bw.qkv_b);
    hipFree(bw.proj_w); hipFree(bw.proj_b);
    hipFree(bw.ls1);
    hipFree(bw.ln2_w);  hipFree(bw.ln2_b);
    hipFree(bw.fc1_w);  hipFree(bw.fc1_b);
    hipFree(bw.fc2_w);  hipFree(bw.fc2_b);
    hipFree(bw.ls2);
    hipFree(d_hidden);
