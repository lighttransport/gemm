...
int main(int argc, char **argv) {
    ...
    hipFunction_t fn_gemm = NULL, fn_down = NULL, fn_idx = NULL, fn_ln = NULL, fn_silu = NULL;
    hipFunction_t fn_conv = NULL, fn_modln = NULL, fn_resadd = NULL;
    if (hipModuleGetFunction(&fn_gemm, mod, "gemm_f32_bias") != hipSuccess ||
        hipModuleGetFunction(&fn_down, mod, "slat_downsample2_mean_include_self_serial_f32") != hipSuccess ||
    ...
    hipDeviceptr_t d_coords0 = hip_upload_raw(coords0, (size_t)N0 * 4 * sizeof(int32_t));
    hipDeviceptr_t d_x0 = hip_upload_raw(x0, X0 * sizeof(float));
    hipDeviceptr_t d_t = hip_upload_raw(t_emb, (size_t)dim * sizeof(float));
    hipDeviceptr_t d_coords1 = 0, d_down = 0, d_counts = 0, d_outN = 0, d_index = 0;
    hipDeviceptr_t d_t_silu = 0, d_emb = 0, d_skip = 0, d_h1 = 0, d_h2 = 0, d_h3 = 0, d_h4 = 0;
    if (!d_coords0 || !d_x0 || !d_t ||
        hipMalloc(&d_coords1, (size_t)N0 * 4 * sizeof(int32_t)) != hipSuccess ||
        hipMalloc(&d_down, X1i * sizeof(float)) != hipSuccess ||
        hipMalloc(&d_counts, (size_t)N0 * sizeof(int)) != hipSuccess ||
        hipMalloc(&d_outN, sizeof(int)) != hipSuccess ||
        ...
        hipMalloc(&d_skip, X1o * sizeof(float)) != hipSuccess ||
        hipMalloc(&d_h1, (size_t)N1_ref * C_in * sizeof(float)) != hipSuccess ||
        hipMalloc(&d_h2, X1o * sizeof(float)) != hipSuccess ||
        hipMalloc(&d_h3, X1o * sizeof(float)) != hipSuccess ||
        hipMalloc(&d_h4, X1o * sizeof(float)) != hipSuccess)
        return 5;
...
#define RUN_BLOCK1() do { \
    ...
    if (hipModuleLaunchKernel(fn_down, 1, 1, 1, 1, 1, 1, 0, 0, a_down, NULL) != hipSuccess) return 8; \
    int hostN = 0; \
    if (hipMemcpyDtoH(&hostN, d_outN, sizeof(int)) != hipSuccess || hostN != N1_ref) return 8; \
    ...
    if (hipModuleLaunchKernel(fn_idx, (N1 + 255) / 256, 1, 1, 256, 1, 1, 0, 0, a_idx, NULL) != hipSuccess) return 8; \
    ...
    if (hipModuleLaunchKernel(fn_ln, N1, 1, 1, threads, 1, 1, (unsigned)ln_smem, 0, a_ln, NULL) != hipSuccess) return 8; \
    ...
    if (hipModuleLaunchKernel(fn_modln, N1, 1, 1, threads, 1, 1, (unsigned)ln_smem, 0, a_ml, NULL) != hipSuccess) return 8; \
    ...
    if (hipMemcpyDtoH(dst, d_h4, X1o * sizeof(float)) != hipSuccess ||
        hipMemcpyDtoH(coords1, d_coords1, (size_t)N1_ref * 4 * sizeof(int32_t)) != hipSuccess)
        return 8;
    int coord_bad = 0;
...
#undef RUN_BLOCK1
    ...
    hipFree(d_coords0); hipFree(d_x0); hipFree(d_t); hipFree(d_coords1); hipFree(d_down);
    hipFree(d_counts); hipFree(d_outN); hipFree(d_index); hipFree(d_t_silu); hipFree(d_emb);
    hipFree(d_skip); hipFree(d_h1); hipFree(d_h2); hipFree(d_h3); hipFree(d_h4);
