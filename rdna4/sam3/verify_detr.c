...
int main(int argc, char **argv)
{
    ...
    if (hip_sam3_set_pixel_values(ctx, px) != 0) return 5;
    free(px);
    fprintf(stderr, "running ViT (32) ...\n");
    if (hip_sam3_run_vit(ctx, 31) != 0) return 6;
    fprintf(stderr, "running FPN ...\n");
    if (hip_sam3_run_fpn(ctx) != 0) return 7;
    if (hip_sam3_set_input_ids(ctx, ids, amask_p) != 0) return 8;
    fprintf(stderr, "running CLIP text ...\n");
    if (hip_sam3_run_text(ctx) != 0) return 9;
    fprintf(stderr, "running DETR encoder (6 layers) ...\n");
    if (hip_sam3_run_detr_enc(ctx) != 0) return 10;

    int on, od;
    size_t cap = (size_t)5184 * 256;
    float *ours = (float *)malloc(cap * 4);
    hip_sam3_get_detr_enc(ctx, ours, &on, &od);
