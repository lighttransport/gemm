...
int main(int argc, char **argv)
{
    ...
    if (!px) {
        ...
        hip_sam3_destroy(ctx); return 3;
    }
    if (hip_sam3_set_pixel_values(ctx, px)) {
        fprintf(stderr, "set_pixel_values failed\n"); free(px);
        hip_sam3_destroy(ctx); return 4;
    }
    free(px);

    int n_tok = 0, D = 0;
    float *ours = (float *)malloc((size_t)5184 * 1024 * sizeof(float));
    if (hip_sam3_get_vit_embed(ctx, ours, &n_tok, &D)) {
        fprintf(stderr, "get_vit_embed failed\n"); free(ours);
        hip_sam3_destroy(ctx); return 5;
    }
