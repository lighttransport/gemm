...
int main(int argc, char **argv)
{
    ...
    if (hip_sam3_run_dot_score(ctx)) return 11;
    if (hip_sam3_run_mask_dec(ctx)) return 12;
    if (hip_sam3_run_postprocess(ctx, target_h, target_w, 0.3f, 0.5f)) return 13;

    int n_ours = 0;
    const float *scores = hip_sam3_get_final_scores(ctx, &n_ours);
    int bn; const float *boxes = hip_sam3_get_final_boxes(ctx, &bn);
    int mn, mh, mw; const uint8_t *masks = hip_sam3_get_final_masks(ctx, &mn, &mh, &mw);
