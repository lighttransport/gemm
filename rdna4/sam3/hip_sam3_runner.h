 ...
 * After hip_sam3_run_vit(bi), this returns the stream after block bi.  */
...
int hip_sam3_run_vit(hip_sam3_ctx *ctx, int stop_at_block);
 ...
 *   level 2: (256,  72,  72)
 *   level 3: (256,  36,  36) */
int hip_sam3_run_fpn(hip_sam3_ctx *ctx);

/* Copy FPN feature map at `level` to host as (C, H, W) F32. */
int hip_sam3_get_fpn(const hip_sam3_ctx *ctx, int level, float *out_host,
...
int hip_sam3_set_input_ids(hip_sam3_ctx *ctx, const int32_t *ids,
                            const int32_t *attn_mask);
int hip_sam3_run_text(hip_sam3_ctx *ctx);

/* Copy text output (32, 1024) F32 into out_host (post final LN). */
int hip_sam3_get_text_output(const hip_sam3_ctx *ctx, float *out_host,
                               int *out_len, int *out_dim);

/* DETR encoder (6 layers). Requires ViT, FPN, text to have run.
 * Uses FPN level 2 (256, 72, 72) as vision input. */
int hip_sam3_run_detr_enc(hip_sam3_ctx *ctx);
int hip_sam3_get_detr_enc(const hip_sam3_ctx *ctx, float *out_host,
                            int *out_n, int *out_dim);

/* DETR decoder (6 layers, 200 queries + 1 presence). Requires detr_enc. */
int hip_sam3_run_detr_dec(hip_sam3_ctx *ctx);
/* Copy final pred_boxes (200, 4) xyxy normalized, 0..1. */
int hip_sam3_get_detr_dec_boxes(const hip_sam3_ctx *ctx, float *out);
/* Copy per-layer presence logits (6,) clamped. */
int hip_sam3_get_detr_dec_presence(const hip_sam3_ctx *ctx, float *out);
/* Copy last-layer output_layer_norm(query_hidden) (200, 256). */
int hip_sam3_get_detr_dec_hidden(const hip_sam3_ctx *ctx, float *out);

/* Dot-product scoring: per-layer query↔text logits (6, 200), clamp ±12. */
int hip_sam3_run_dot_score(hip_sam3_ctx *ctx);
int hip_sam3_get_dot_scores(const hip_sam3_ctx *ctx, float *out);

/* Mask decoder: prompt_cross_attn → pixel_decoder → instance/semantic →
 * mask_embedder → einsum. pred_masks (200, 288, 288) F32. */
int hip_sam3_run_mask_dec(hip_sam3_ctx *ctx);
int hip_sam3_get_pred_masks(const hip_sam3_ctx *ctx, float *out,
                              int *out_q, int *out_h, int *out_w);
int hip_sam3_get_semantic_seg(const hip_sam3_ctx *ctx, float *out,
                                int *out_h, int *out_w);
...
int hip_sam3_run_postprocess(hip_sam3_ctx *ctx, int target_h, int target_w,
                              float score_threshold, float mask_threshold);
const float *hip_sam3_get_final_scores(const hip_sam3_ctx *ctx, int *out_n);
const float *hip_sam3_get_final_boxes(const hip_sam3_ctx *ctx, int *out_n);
const uint8_t *hip_sam3_get_final_masks(const hip_sam3_ctx *ctx,
