...
int main(int argc, char **argv)
{
    ...
    free(px);
    if (hip_sam3_run_vit(ctx, 31) != 0) return 5;
    if (hip_sam3_run_fpn(ctx) != 0) return 6;
    if (hip_sam3_set_input_ids(ctx, ids, amask_p) != 0) return 7;
    if (hip_sam3_run_text(ctx) != 0) return 8;
    if (hip_sam3_run_detr_enc(ctx) != 0) return 9;
    fprintf(stderr, "running DETR decoder (6 layers, 200 queries) ...\n");
    if (hip_sam3_run_detr_dec(ctx) != 0) return 10;

    float ours_boxes[200 * 4], ours_pres[6];
    hip_sam3_get_detr_dec_boxes(ctx, ours_boxes);
    hip_sam3_get_detr_dec_presence(ctx, ours_pres);
