...
int main(int argc, char **argv)
{
    ...
    if (ref_px) {
        ...
        if (hip_sam3_set_pixel_values(ctx, ref_px) != 0) return 3;
        free(ref_px);
    } else if (img_path) {
        int H, W, C;
        unsigned char *rgb = stbi_load(img_path, &W, &H, &C, 3);
        if (!rgb) { fprintf(stderr, "can't load %s\n", img_path); return 3; }
        if (hip_sam3_set_image(ctx, rgb, H, W) != 0) return 3;
        stbi_image_free(rgb);
    } else {
    ...
    fprintf(stderr, "running HIP ViT through block %d (target=%s) ...\n",
            stop_at, target);
    if (hip_sam3_run_vit(ctx, stop_at) != 0) {
        fprintf(stderr, "hip_sam3_run_vit failed\n"); return 4;
    }

    int n_tok, dim;
    float *ours = (float *)malloc((size_t)5184 * 1024 * 4);
    if (hip_sam3_get_vit_embed(ctx, ours, &n_tok, &dim) != 0) return 5;
