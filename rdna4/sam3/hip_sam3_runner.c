/*
 * hip_sam3_runner.c - HIP/ROCm SAM 3 runner (RDNA4).
 *
 ...
 * the `detector_model.` prefix stripped.
 *
...
struct hip_sam3_ctx {
    ...
    /* kernels */
    hipFunction_t fn_resize, fn_ln, fn_gemm, fn_gelu, fn_add;
...
static int st_find_pfx(const st_context *st, const char *s) {
    char k[256]; snprintf(k, sizeof(k), "detector_model.%s", s);
...
static void *upload_f32(const st_context *st, const char *s, size_t *n) {
    ...
    if (strcmp(safetensors_dtype(st, i), "F32")) { fprintf(stderr, "sam3: %s not F32\n", s); return NULL; }
...
static int load_text_block(hip_sam3_ctx *c, int li) {
...
static int load_block(hip_sam3_ctx *c, int bi) {
    s3_block *b = &c->blk[bi];
...
static int compile_kernels(hip_sam3_ctx *c) {
    ...
    #define GET(f, s) do { \
        if (hipModuleGetFunction(&c->f, c->mod, s) != hipSuccess) { \
            fprintf(stderr, "sam3: missing %s\n", s); return -1; } \
    } while (0)
    GET(fn_resize,    "resize_normalize");
    GET(fn_ln,        "layernorm_f32");
    GET(fn_gemm,      "gemm_tiled_f16_f32");
...
static int launch(hipFunction_t fn, unsigned gx, unsigned gy, unsigned gz,
    ...
    if (e != hipSuccess) {
        fprintf(stderr, "sam3: launch err=%d grid=%ux%ux%u block=%ux%ux%u\n",
...
static int r_gemm(hip_sam3_ctx *c, void *Y, const void *W, const void *X,
                   const void *bias, int n_out, int n_in, int n_tok) {
    struct { void *Y; const void *W, *X, *bias; int n_out, n_in, n_tok; } p =
        { Y, W, X, bias, n_out, n_in, n_tok };
    return launch(c->fn_gemm, (n_out+63)/64, (n_tok+15)/16, 1, 16, 16, 1, 0, &p, sizeof(p));
}
static int r_ln_eps(hip_sam3_ctx *c, void *dst, const void *src, const void *w, const void *b, int n_tok, int dim, float eps) {
    struct { void *dst; const void *src, *w, *b; int dim; float eps; } p =
        { dst, src, w, b, dim, eps };
    return launch(c->fn_ln, n_tok, 1, 1, 256, 1, 1, 256*4, &p, sizeof(p));
}
static int r_ln(hip_sam3_ctx *c, void *dst, const void *src, const void *w, const void *b, int n_tok, int dim) {
    return r_ln_eps(c, dst, src, w, b, n_tok, dim, S3_EPS);
}
static int r_gelu(hip_sam3_ctx *c, void *X, int n) {
    struct { void *X; int n; } p = { X, n };
    return launch(c->fn_gelu, (n+255)/256, 1, 1, 256, 1, 1, 0, &p, sizeof(p));
}
static int r_add(hip_sam3_ctx *c, void *dst, const void *src, int n) {
    struct { void *dst; const void *src; int n; } p = { dst, src, n };
    return launch(c->fn_add, (n+255)/256, 1, 1, 256, 1, 1, 0, &p, sizeof(p));
}
static int r_part(hip_sam3_ctx *c, void *dst, const void *src, int H, int W, int D, int ws) {
    struct { void *d; const void *s; int H, W, D, ws; } p = { dst, src, H, W, D, ws };
    int total = H * W * D;
    return launch(c->fn_win_part, (total+255)/256, 1, 1, 256, 1, 1, 0, &p, sizeof(p));
}
static int r_unpart(hip_sam3_ctx *c, void *dst, const void *src, int H, int W, int D, int ws) {
    struct { void *d; const void *s; int H, W, D, ws; } p = { dst, src, H, W, D, ws };
    int total = H * W * D;
    return launch(c->fn_win_unpart, (total+255)/256, 1, 1, 256, 1, 1, 0, &p, sizeof(p));
}
static int r_rope(hip_sam3_ctx *c, void *qkv, const void *cos_t, const void *sin_t,
                   int rope_seq, int n_tok) {
    struct { void *qkv; const void *c, *s; int rope_seq, n_tok, heads, hd, D; } p =
        { qkv, cos_t, sin_t, rope_seq, n_tok, S3_HEADS, S3_HEAD, S3_DIM };
    return launch(c->fn_rope, n_tok, 1, 1, S3_HEADS, 1, 1, 0, &p, sizeof(p));
}
static int r_kvtx(hip_sam3_ctx *c, void *Kt, void *Vt, const void *qkv, int n_tok) {
    struct { void *K, *V; const void *qkv; int n_tok, dim, heads, hd; } p =
...
static int r_fa(hip_sam3_ctx *c, void *out, const void *qkv, const void *Kt, const void *Vt, int n_tok) {
    float scale = 1.0f / sqrtf((float)S3_HEAD);
    struct { void *out; const void *qkv, *K, *V; int n_tok, dim, heads, hd; float sc; } p =
...
hip_sam3_ctx *hip_sam3_create(const hip_sam3_config *cfg) {
    ...
    if (c->cfg.image_size != S3_IMG) { fprintf(stderr, "sam3: image_size must be %d\n", S3_IMG); free(c); return NULL; }
    ...
    if (!c->st) { fprintf(stderr, "sam3: safetensors_open failed\n"); free(c); return NULL; }
    ...
    if (!c->w_patch || !c->w_pos || !c->w_pre_ln_w || !c->w_pre_ln_b) { hip_sam3_destroy(c); return NULL; }

    if (c->verbose) fprintf(stderr, "sam3: loading %d ViT blocks ...\n", S3_NBLK);
    for (int bi = 0; bi < S3_NBLK; bi++) {
        if (load_block(c, bi)) { hip_sam3_destroy(c); return NULL; }
    }

    /* FPN neck. */
    if (c->verbose) fprintf(stderr, "sam3: loading FPN neck ...\n");
    ...
    for (int li = 0; li < S3_FPN_LEV; li++) {
        ...
        if (!L->p1_w || !L->p1_b || !L->p2_w || !L->p2_b) {
            fprintf(stderr, "sam3: fpn level %d missing proj weights\n", li);
    ...
    /* CLIP text encoder. */
    if (c->verbose) fprintf(stderr, "sam3: loading CLIP text encoder ...\n");
    ...
    if (!c->h_tok_embed || !c->h_pos_embed || !c->t_final_ln_w || !c->t_final_ln_b) {
        fprintf(stderr, "sam3: missing CLIP text encoder weights\n");
        hip_sam3_destroy(c); return NULL;
    }
    for (int li = 0; li < S3_T_LAYERS; li++) {
        if (load_text_block(c, li)) {
            fprintf(stderr, "sam3: failed to load text block %d\n", li);
            hip_sam3_destroy(c); return NULL;
        }
    }
    ...
    if (hipMalloc(&c->d_text_tok,  text_tok_b) != hipSuccess ||
        ...
        fprintf(stderr, "sam3: text buffer alloc failed\n");
        hip_sam3_destroy(c); return NULL;
    }
    ...
    if (!c->w_text_proj_w || !c->w_text_proj_b) {
        fprintf(stderr, "sam3: missing text_projection weights\n");
        hip_sam3_destroy(c); return NULL;
    }

    /* DETR encoder layers. */
    if (c->verbose) fprintf(stderr, "sam3: loading DETR encoder (%d layers) ...\n", S3_DETR_LAYERS);
    for (int li = 0; li < S3_DETR_LAYERS; li++) {
        ...
        if (!qw || !kw || !vw || !qb || !kbn || !vb) { hip_sam3_destroy(c); return NULL; }
        ...
        if (!c->detr_enc[li].o_w || !c->detr_enc[li].fc2_w) {
            fprintf(stderr, "sam3: detr layer %d missing weights\n", li);
            hip_sam3_destroy(c); return NULL;
        }
    }
    ...
    /* DETR activation buffers. */
    {
        ...
        if (hipMalloc(&c->d_detr_vis,        N * DD * 4) != hipSuccess ||
            ...
            fprintf(stderr, "sam3: detr buffer alloc failed\n");
            hip_sam3_destroy(c); return NULL;
        }
    }
    ...
    /* DETR decoder. */
    if (c->verbose) fprintf(stderr, "sam3: loading DETR decoder ...\n");
    {
        /* Static tensors. */
        ...
        if (!c->dd_query_embed || !rp) { hip_sam3_destroy(c); return NULL; }
        ...
        for (int i = 0; i < 3; i++) {
            ...
            if (!c->dd_box_w[i] || !c->dd_box_b[i]) { hip_sam3_destroy(c); return NULL; }
        }
        ...
        for (int i = 0; i < 3; i++) {
            ...
            if (!c->dd_pres_w[i] || !c->dd_pres_b[i]) { hip_sam3_destroy(c); return NULL; }
        }
        ...
        for (int i = 0; i < 2; i++) {
            ...
            if (!c->dd_rph_w[i] || !c->dd_rph_b[i]) { hip_sam3_destroy(c); return NULL; }
        }
        ...
        for (int i = 0; i < 2; i++) {
            ...
            if (!c->dd_rpbx_w[i] || !c->dd_rpby_w[i]) { hip_sam3_destroy(c); return NULL; }
        }
        ...
        #define DLOAD_F16(dst, fmt, ...) do { \
            ...
            if (!dst) { fprintf(stderr, "sam3: missing %s\n", k); hip_sam3_destroy(c); return NULL; } \
        } while (0)
        ...
        if (hipMalloc(&c->d_dd_hs,      Nt * DD * 4) != hipSuccess ||
            ...
            fprintf(stderr, "sam3: decoder buffer alloc failed\n");
            hip_sam3_destroy(c); return NULL;
        }
    }

    /* dot_product_scoring weights + runtime buffers. */
    if (c->verbose) fprintf(stderr, "sam3: loading dot_product_scoring ...\n");
    {
        ...
        if (hipMalloc(&c->d_ds_tmlp_h,   T * MLP * 4)  != hipSuccess ||
            ...
            fprintf(stderr, "sam3: dot_product_scoring buffer alloc failed\n");
            hip_sam3_destroy(c); return NULL;
        }
    }

    /* Mask decoder weights + runtime buffers. */
    if (c->verbose) fprintf(stderr, "sam3: loading mask_decoder ...\n");
    {
        ...
        if (hipMalloc(&c->d_md_enc_ln,    N  * DD * 4)         != hipSuccess ||
            ...
            fprintf(stderr, "sam3: mask_decoder buffer alloc failed\n");
            hip_sam3_destroy(c); return NULL;
        }
    }
    ...
    if (c->verbose) fprintf(stderr, "sam3: ready (patch+pos ready, 32 ViT blocks loaded)\n");
    return c;
}
...
static int run_patch_pos(hip_sam3_ctx *c) {
    ...
    struct { void *t; const void *p; int g, P, D; } pp =
...
int hip_sam3_set_pixel_values(hip_sam3_ctx *c, const float *pv) {
    if (!c || !pv) return -1;
...
static int block_forward(hip_sam3_ctx *c, int bi) {
    s3_block *b = &c->blk[bi];
    /* LN1: tmp = ln1(tok) */
...
int hip_sam3_run_vit(hip_sam3_ctx *c, int stop_at_block) {
...
static int r_tok2nchw(hip_sam3_ctx *c, void *dst, const void *src,
                        int H, int W, int D) {
    struct { void *d; const void *s; int H, W, D; } p = { dst, src, H, W, D };
    int total = H * W * D;
    return launch(c->fn_tok2nchw, (total+255)/256, 1, 1, 256, 1, 1, 0, &p, sizeof(p));
}
static int r_c1x1(hip_sam3_ctx *c, void *y, const void *w, const void *b,
                    const void *x, int Ci, int Co, int H, int W) {
    struct { void *y; const void *w, *b, *x; int Ci, Co, H, W; } p = { y, w, b, x, Ci, Co, H, W };
    return launch(c->fn_c1x1, (W+15)/16, (H+15)/16, Co, 16, 16, 1, 0, &p, sizeof(p));
}
static int r_c3x3(hip_sam3_ctx *c, void *y, const void *w, const void *b,
                    const void *x, int Ci, int Co, int H, int W) {
    struct { void *y; const void *w, *b, *x; int Ci, Co, H, W; } p = { y, w, b, x, Ci, Co, H, W };
    return launch(c->fn_c3x3, (W+15)/16, (H+15)/16, Co, 16, 16, 1, 0, &p, sizeof(p));
}
static int r_ct2(hip_sam3_ctx *c, void *y, const void *w, const void *b,
                   const void *x, int Ci, int Co, int H, int W) {
    struct { void *y; const void *w, *b, *x; int Ci, Co, H, W; } p = { y, w, b, x, Ci, Co, H, W };
    int OH = H * 2, OW = W * 2;
    return launch(c->fn_ct2, (OW+15)/16, (OH+15)/16, Co, 16, 16, 1, 0, &p, sizeof(p));
}
static int r_mp2(hip_sam3_ctx *c, void *y, const void *x, int C, int H, int W) {
    struct { void *y; const void *x; int C, H, W; } p = { y, x, C, H, W };
    int OH = H / 2, OW = W / 2;
    return launch(c->fn_mp2, (OW+15)/16, (OH+15)/16, C, 16, 16, 1, 0, &p, sizeof(p));
}

int hip_sam3_run_fpn(hip_sam3_ctx *c) {
    if (!c) return -1;
    if (c->last_block != S3_NBLK - 1) {
        fprintf(stderr, "sam3: run_fpn requires ViT through block 31\n");
        return -1;
    }
...
int hip_sam3_get_fpn(const hip_sam3_ctx *c, int level, float *out,
...
int hip_sam3_set_input_ids(hip_sam3_ctx *c, const int32_t *ids,
...
static int r_gelu_erf(hip_sam3_ctx *c, void *X, int n) {
    struct { void *X; int n; } p = { X, n };
    return launch(c->fn_gelu_erf, (n+255)/256, 1, 1, 256, 1, 1, 0, &p, sizeof(p));
}

static int r_clip_causal(hip_sam3_ctx *c, void *out, const void *qkv, int T) {
    float scale = 1.0f / sqrtf((float)S3_T_HD);
    struct { void *out; const void *qkv; int T, D, heads, hd; float sc; } p =
        { out, qkv, T, S3_T_DIM, S3_T_HEADS, S3_T_HD, scale };
    return launch(c->fn_clip_causal, S3_T_HEADS, 1, 1, T, 1, 1, 0, &p, sizeof(p));
}

int hip_sam3_run_text(hip_sam3_ctx *c) {
...
int hip_sam3_get_text_output(const hip_sam3_ctx *c, float *out,
...
static int r_nchw2tok(hip_sam3_ctx *c, void *dst, const void *src,
                       int C, int H, int W) {
    struct { void *d; const void *s; int C, H, W; } p = { dst, src, C, H, W };
    int total = C * H * W;
    return launch(c->fn_nchw2tok, (total + 255) / 256, 1, 1, 256, 1, 1, 0, &p, sizeof(p));
}
static int r_add_pos(hip_sam3_ctx *c, void *dst, const void *src, const void *pos, int n) {
    struct { void *d; const void *s, *p; int n; } p = { dst, src, pos, n };
    return launch(c->fn_add_pos, (n + 255) / 256, 1, 1, 256, 1, 1, 0, &p, sizeof(p));
}
static int r_relu(hip_sam3_ctx *c, void *x, int n) {
    struct { void *x; int n; } p = { x, n };
    return launch(c->fn_relu, (n + 255) / 256, 1, 1, 256, 1, 1, 0, &p, sizeof(p));
}
/* Launch MHA with online softmax, head_dim=32. */
static int r_mha32_bias(hip_sam3_ctx *c, void *out, const void *Q, const void *K,
                         const void *V, const void *key_mask, const void *bias_hqk,
                         int Nq, int Nk) {
    float scale = 1.0f / sqrtf((float)S3_DETR_HD);
    struct { void *out; const void *Q, *K, *V, *key_mask, *bias_hqk;
...
static int r_mha32(hip_sam3_ctx *c, void *out, const void *Q, const void *K,
...
static int extract_qkv_slab(hip_sam3_ctx *c, void *dst, const void *src,
...
static int detr_enc_layer(hip_sam3_ctx *c, int li) {
    const int N = S3_NTOK;
...
int hip_sam3_run_detr_enc(hip_sam3_ctx *c) {
    if (!c) return -1;
    if (c->last_block != S3_NBLK - 1 || !c->fpn_ready || !c->text_done) {
        fprintf(stderr, "sam3: run_detr_enc needs ViT+FPN+text first\n");
        return -1;
    }
...
int hip_sam3_get_detr_enc(const hip_sam3_ctx *c, float *out, int *out_n, int *out_dim) {
...
static int r_tok2chw(hip_sam3_ctx *c, void *dst, const void *src,
                       int C, int H, int W) {
    struct { void *d; const void *s; int C, H, W; } p = { dst, src, C, H, W };
    int total = C * H * W;
    return launch(c->fn_tok2chw, (total + 255) / 256, 1, 1, 256, 1, 1, 0, &p, sizeof(p));
}
static int r_groupnorm(hip_sam3_ctx *c, void *x, const void *w, const void *b,
                         int C, int H, int W, int G) {
    struct { void *x; const void *w, *b; int C, H, W, G; float eps; } p =
        { x, w, b, C, H, W, G, 1e-5f };
    return launch(c->fn_groupnorm, G, 1, 1, 256, 1, 1, 256 * 2 * 4, &p, sizeof(p));
}
static int r_upnn(hip_sam3_ctx *c, void *dst, const void *src,
                    int C, int Hi, int Wi, int Ho, int Wo) {
    struct { void *d; const void *s; int C, Hi, Wi, Ho, Wo; } p =
        { dst, src, C, Hi, Wi, Ho, Wo };
    int total = C * Ho * Wo;
    return launch(c->fn_upnn, (total + 255) / 256, 1, 1, 256, 1, 1, 0, &p, sizeof(p));
}
static int r_einsum_qchw(hip_sam3_ctx *c, void *out, const void *me,
                           const void *instance, int Nq, int D, int H, int W) {
    struct { void *o; const void *m, *i; int Nq, D, H, W; } p =
        { out, me, instance, Nq, D, H, W };
    int HW = H * W;
    return launch(c->fn_einsum_qchw, (HW + 255) / 256, Nq, 1, 256, 1, 1, 0, &p, sizeof(p));
}
static int r_rpb_assemble(hip_sam3_ctx *c, void *rpb, const void *ex,
                           const void *ey, int Nq, int H, int W) {
    struct { void *rpb; const void *ex, *ey; int Nq, H, W, heads; } p =
...
static int dec_layer(hip_sam3_ctx *c, int li) {
...
int hip_sam3_run_detr_dec(hip_sam3_ctx *c) {
...
int hip_sam3_get_detr_dec_boxes(const hip_sam3_ctx *c, float *out) {
    if (!c || !c->dd_done || !out) return -1;
    memcpy(out, c->dd_pred_boxes, S3_DETR_Q * 4 * 4);
    return 0;
}
int hip_sam3_get_detr_dec_presence(const hip_sam3_ctx *c, float *out) {
    if (!c || !c->dd_done || !out) return -1;
    memcpy(out, c->dd_presence_logits, S3_DETR_LAYERS * 4);
    return 0;
}
int hip_sam3_get_detr_dec_hidden(const hip_sam3_ctx *c, float *out) {
    if (!c || !c->dd_done || !out) return -1;
...
int hip_sam3_run_dot_score(hip_sam3_ctx *c) {
...
int hip_sam3_get_dot_scores(const hip_sam3_ctx *c, float *out) {
    if (!c || !c->ds_done || !out) return -1;
    memcpy(out, c->ds_scores, (size_t)S3_DETR_LAYERS * S3_DETR_Q * 4);
    return 0;
}

int hip_sam3_run_mask_dec(hip_sam3_ctx *c) {
    if (!c || !c->dd_done) return -1;
...
int hip_sam3_get_pred_masks(const hip_sam3_ctx *c, float *out,
...
int hip_sam3_get_semantic_seg(const hip_sam3_ctx *c, float *out,
                                int *out_h, int *out_w) {
    if (!c || !c->md_done || !out) return -1;
...
int hip_sam3_run_postprocess(hip_sam3_ctx *c, int target_h, int target_w,
...
const float *hip_sam3_get_final_scores(const hip_sam3_ctx *c, int *out_n) {
    if (!c || !c->pp_done) return NULL;
    if (out_n) *out_n = c->pp_n_kept;
    return c->pp_scores;
}
const float *hip_sam3_get_final_boxes(const hip_sam3_ctx *c, int *out_n) {
    if (!c || !c->pp_done) return NULL;
    if (out_n) *out_n = c->pp_n_kept;
    return c->pp_boxes;
}
const uint8_t *hip_sam3_get_final_masks(const hip_sam3_ctx *c,
