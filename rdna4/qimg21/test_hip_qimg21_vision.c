...
static int vision_linear(cuda_qimg_runner *r, CUfunction epilogue,
    ...
    if (!weight || !bias || !in_bf || launch_cast(r, in_bf, in, rows * ni) ||
        gemm(r, out, weight, in_bf, rows, no, ni) ||
        cuLaunchKernel(epilogue, (rows * no + 255) / 256, 1, 1,
                       256, 1, 1, 0, r->stream,
                       (void *[]){&out, &out, &bias, &no, &rows}, NULL) ||
        cuStreamSynchronize(r->stream)) goto done;
    (void)epilogue;
    rc = 0;
done:
    free_d(&weight); free_d(&bias); free_d(&in_bf);
    return rc;
}
...
static int run_merger(cuda_qimg_runner *r, CUfunction layer_norm,
    ...
    int gelu_status = (int)cuLaunchKernel(gelu_exact, (hidden_count + 255) / 256,
                                         1, 1, 256, 1, 1, 0, r->stream, ga, NULL);
...
int main(int argc, char **argv) {
    ...
    host_out = malloc((size_t)count * 4);
    if (!x || !weight || !bias || !pos || !host_pos || !host_out ||
        cuCtxSynchronize() ||
