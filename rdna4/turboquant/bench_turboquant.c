/*
 * bench_turboquant.c - RDNA4 TurboQuant3 standalone kernels.
 *
 * HIPRTC kernels mirror cpu/turboquant's WHT/PolarQuant 3-bit block format.
...
"}\n"
"__device__ __forceinline__ int unpack3(const u8 *qs, int i) {\n"
"    int bit = i * 3, byte = bit >> 3, shift = bit & 7;\n"
"    u32 w = qs[byte]; if (byte + 1 < TQ3_PACKED_BYTES) w |= ((u32)qs[byte + 1] << 8);\n"
"    return (int)((w >> shift) & 7u);\n"
"}\n"
"__device__ __forceinline__ void pack3(u8 *qs, const u8 *idx) {\n"
"    for (int i = 0; i < TQ3_PACKED_BYTES; i++) qs[i] = 0;\n"
...
"void tq3_quantize_f32(u8 *dst, const float *src, int rows, int cols, u64 seed) {\n"
...
"    float norm2 = red[0];\n"
"    if (norm2 <= 0.0f) { if (tid == 0) { u8 *o = dst + (long)g * 52; for (int i=0;i<52;i++) o[i]=0; } return; }\n"
...
"    int qi = nearest_centroid(rv); idx[tid] = (u8)qi; red[tid] = tq3_centroids_dev[qi] * tq3_centroids_dev[qi]; __syncthreads();\n"
"    for (int s = 64; s > 0; s >>= 1) { if (tid < s) red[tid] += red[tid+s]; __syncthreads(); }\n"
"    if (tid == 0) { u8 *o = dst + (long)g * 52; float scale = sqrtf(norm2) / fmaxf(sqrtf(red[0]), 1.0e-20f); u16 h = f32_to_f16(scale); o[0]=(u8)h; o[1]=(u8)(h>>8); o[2]=0; o[3]=0; pack3(o+4, idx); }\n"
"}\n"
"\n"
"__global__ __launch_bounds__(128, 1)\n"
"void tq3_dequant_f32(float *dst, const u8 *src, int rows, int cols, u64 seed) {\n"
"    int nb = cols >> 7; int g = blockIdx.x; int row = g / nb; int cb = g - row * nb;\n"
"    int tid = threadIdx.x; if (row >= rows) return;\n"
"    __shared__ float v[128]; const u8 *blk = src + (long)g * 52;\n"
"    u16 h = (u16)blk[0] | ((u16)blk[1] << 8); float scale = f16_to_f32(h);\n"
"    int qi = unpack3(blk + 4, tid);\n"
"    float y = tq3_centroids_dev[qi] * scale;\n"
...
"\n"
"__global__ __launch_bounds__(128, 1)\n"
"void tq3_dot_partial(float *partial, const u8 *src, const float *x, int rows, int cols, u64 seed) {\n"
"    int nb = cols >> 7; int g = blockIdx.x; int row = g / nb; int cb = g - row * nb;\n"
"    int tid = threadIdx.x; if (row >= rows) return;\n"
"    __shared__ float v[128]; __shared__ float red[128]; const u8 *blk = src + (long)g * 52;\n"
...
"    u16 h = (u16)blk[0] | ((u16)blk[1] << 8); float scale = f16_to_f32(h);\n"
"    int qi = unpack3(blk + 4, tid); red[tid] = tq3_centroids_dev[qi] * scale * xrot; __syncthreads();\n"
"    for (int s = 64; s > 0; s >>= 1) { if (tid < s) red[tid] += red[tid+s]; __syncthreads(); }\n"
"    if (tid == 0) partial[g] = red[0];\n"
"}\n"
"\n"
"__global__ __launch_bounds__(128, 1)\n"
"void tq3_dequant_bf16(u16 *dst, const u8 *src, int rows, int cols, u64 seed) {\n"
"    int nb = cols >> 7; int g = blockIdx.x; int row = g / nb; int cb = g - row * nb; int tid = threadIdx.x; if (row >= rows) return;\n"
"    __shared__ float v[128]; const u8 *blk = src + (long)g * 52; u16 h = (u16)blk[0] | ((u16)blk[1] << 8); float scale = f16_to_f32(h);\n"
"    int qi = unpack3(blk + 4, tid); float y = tq3_centroids_dev[qi] * scale; v[tid] = sign_bit(seed, (u64)cb, 1, tid) ? -y : y; __syncthreads(); fwht128_shared(v, tid);\n"
"    float out = sign_bit(seed, (u64)cb, 0, tid) ? -v[tid] : v[tid]; dst[(long)row * cols + cb * 128 + tid] = f32_to_bf16(out);\n"
"}\n"
"__global__ __launch_bounds__(128, 1)\n"
"void tq3_dequant_f16(u16 *dst, const u8 *src, int rows, int cols, u64 seed) {\n"
"    int nb = cols >> 7; int g = blockIdx.x; int row = g / nb; int cb = g - row * nb; int tid = threadIdx.x; if (row >= rows) return;\n"
"    __shared__ float v[128]; const u8 *blk = src + (long)g * 52; u16 h = (u16)blk[0] | ((u16)blk[1] << 8); float scale = f16_to_f32(h);\n"
"    int qi = unpack3(blk + 4, tid); float y = tq3_centroids_dev[qi] * scale; v[tid] = sign_bit(seed, (u64)cb, 1, tid) ? -y : y; __syncthreads(); fwht128_shared(v, tid);\n"
...
"}\n"
"__global__ __launch_bounds__(128, 1)\n"
"void tq3_dequant_fp8(u8 *dst, const u8 *src, int rows, int cols, u64 seed) {\n"
"    int nb = cols >> 7; int g = blockIdx.x; int row = g / nb; int cb = g - row * nb; int tid = threadIdx.x; if (row >= rows) return;\n"
"    __shared__ float v[128]; const u8 *blk = src + (long)g * 52; u16 h = (u16)blk[0] | ((u16)blk[1] << 8); float scale = f16_to_f32(h);\n"
"    int qi = unpack3(blk + 4, tid); float y = tq3_centroids_dev[qi] * scale; v[tid] = sign_bit(seed, (u64)cb, 1, tid) ? -y : y; __syncthreads(); fwht128_shared(v, tid);\n"
...
int main(int argc, char **argv) {
    int rows = 1024, cols = 128, iters = 200, device = 0, verify = 1, verbose = 0;
    uint64_t seed = 42;
    ...
    size_t q_bytes = (size_t)rows * (size_t)tq3_row_bytes(cols);
    ...
    for (int r = 0; r < rows; r++) {
        tq3_quantize_row_f32((tq3_block *)(h_q_cpu + (size_t)r * tq3_row_bytes(cols)),
                             h_x + (size_t)r * cols, cols, seed);
        tq3_dequantize_row_f32(h_deq + (size_t)r * cols,
                               (const tq3_block *)(h_q_cpu + (size_t)r * tq3_row_bytes(cols)),
                               cols, seed);
    }
    ...
    void *qargs[] = { &d_q, &d_x, &rows, &cols, &seed };
    void *dargs[] = { &d_deq, &d_q, &rows, &cols, &seed };
    void *dotargs[] = { &d_partial, &d_q, &d_query, &rows, &cols, &seed };
    void *stageargs[] = { &d_stage, &d_q, &rows, &cols, &seed };
    ...
    if (verify) {
        ...
        for (int r = 0; r < rows; r++) {
            ...
            float cd = tq3_dot_row_f32((const tq3_block *)(h_q_cpu + (size_t)r * tq3_row_bytes(cols)),
                                       h_query, cols, seed);
        ...
        if (mismatches || max_abs > 1.0e-5 || dot_max_abs > 1.0e-4 * (double)nb) {
    ...
    printf("rows=%d cols=%d blocks=%u bytes=%zu compression_vs_f16=%.3fx\n",
           rows, cols, gx, q_bytes, ((double)elems * 2.0) / (double)q_bytes);
