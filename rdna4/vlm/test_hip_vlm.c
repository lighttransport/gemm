...
int main(int argc, char **argv) {
    ...
    fprintf(stderr, "=== HIP VLM Pipeline (RDNA4) ===\n");
    ...
    float *vision_embd = hip_vision_encode(hip_vis, rgb_norm, img_w, img_h);
    ...
    int total_embd = hip_vision_total_embd(hip_vis);
    int proj_dim = hip_vision_proj_dim(hip_vis);
    ...
    fprintf(stderr, "Initializing HIP LLM runner (max_seq_len=%d)...\n", max_seq_len);
    hip_llm_runner *llm = hip_llm_init(0, 1);
    if (!llm) { fprintf(stderr, "HIP LLM init failed\n"); return 1; }
    if (hip_llm_load_weights(llm, gguf_llm, max_seq_len) != 0) {
        fprintf(stderr, "HIP LLM weight load failed\n");
        return 1;
    }

    int n_embd = hip_llm_n_embd(llm);
    int n_vocab = hip_llm_n_vocab(llm);
    fprintf(stderr, "LLM: n_embd=%d n_vocab=%d n_layers=%d\n",
            n_embd, n_vocab, hip_llm_n_layers(llm));
    ...
    t0 = get_time_ms();
    for (int i = 0; i < n_before; i++) {
        hip_llm_forward(llm, tokens_before[i], pos);
        pos++;
    }
    t1 = get_time_ms();
    ...
    for (int i = 0; i < n_after; i++) {
        if (i == n_after - 1) {
            logits = hip_llm_forward_logits(llm, tokens_after[i], pos);
        } else {
            hip_llm_forward(llm, tokens_after[i], pos);
        }
        pos++;
    }
    t1 = get_time_ms();
    ...
    for (int g = 0; g < max_gen; g++) {
        ...
        /* Next step */
        logits = hip_llm_forward_logits(llm, next_token, pos);
        pos++;
        if (!logits) break;
    ...
    fprintf(stderr, "\nCleaning up...\n");
    free(vision_embd);
    if (hip_vis) hip_vision_free(hip_vis);
    vision_free(vm);
    hip_llm_free(llm);
