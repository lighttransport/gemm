...
"}\n"
"\n"
"/* ==== flash_attn_wmma_f16_4w_pre: BQ=64 4-wave F16 with double-buffered K/V ==== */\n"
"__global__ void flash_attn_wmma_f16_4w_pre(float *out, const float *qkv,\n"
"    const _Float16 *K_t, const _Float16 *V_t,\n"
"    int n_tok, int dim, int n_heads, int head_dim, float scale) {\n"
"    int h = blockIdx.x;\n"
"    int qb = blockIdx.y;\n"
...
struct hip_vision_runner {
    ...
    hipFunction_t fn_flash_attn_wmma_f16_4w_pre;
...
#define GET_FN(name) do { \
    ...
    GET_FN(flash_attn_wmma_f16_4w_pre);
    GET_FN(kv_transpose);
...
float *hip_vision_encode(hip_vision_runner *r, const float *rgb_norm, int width, int height) {
    ...
    for (int l = 0; l < r->n_blocks; l++) {
        ...
        /* Multi-head self-attention (flash attention) */
        {
             ...
             *   HIP_VLM_FA=wmma_f16_4w_pre  -> WMMA F16 BQ=64 4-wave w/ pre-packed F16 K/V
            ...
            if ((use_wmma_bf16_2w_pre || use_wmma_f16_2w_pre || use_wmma_bf16_4w_pre || use_wmma_f16_4w_pre) && head_dim <= 80) {
            ...
            if ((use_wmma_bf16 || use_wmma_f16) && head_dim <= 80) {
            ...
            } else if ((use_wmma_bf16_4w_pre || use_wmma_f16_4w_pre) && head_dim <= 80) {
                ...
                hipFunction_t fn = use_wmma_bf16_4w_pre ? r->fn_flash_attn_wmma_bf16_4w_pre
                                                        : r->fn_flash_attn_wmma_f16_4w_pre;
                hipModuleLaunchKernel(fn,
                               n_heads, n_q_blocks, 1,
