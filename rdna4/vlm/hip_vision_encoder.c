...
typedef struct {
    ...
    void *w_f16;   /* F16 weight [n_out, n_in] (if use_f16) */
...
int hip_vision_load_weights(hip_vision_runner *r, gguf_context *g) {
    ...
    fprintf(stderr, "hip_vlm: dim=%d heads=%d blocks=%d ffn=%d patch=%d image=%d patches=%d merged=%d proj=%d f16=%d max_patches=%d\n",
    ...
    if (t_pw1.data)
        fprintf(stderr, "hip_vlm: loaded dual conv2d patch embeddings\n");
