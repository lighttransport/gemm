...
lib.px_test_postprocess_profile.argtypes=[ip,fp,fp,C.c_int,C.c_char_p,C.c_char_p,C.c_char_p]
lib.px_test_error.restype=C.c_char_p
a.output.parent.mkdir(parents=True,exist_ok=True)
start=time.monotonic()
if a.profile_json:
    a.profile_json.parent.mkdir(parents=True,exist_ok=True)
    rc=lib.px_test_postprocess_profile(shape['coords'],shape['feats'],texture['feats'],len(shape['coords']),str(a.output).encode(),str(a.dump_dir).encode(),str(a.profile_json).encode())
else:
