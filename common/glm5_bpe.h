...
static int glm5_bpe_load(const char *path, glm5_bpe *b) {
    ...
    if(fread(json,1,(size_t)sz,f)!=(size_t)sz){free(json);fclose(f);return -4;}fclose(f);json[sz]='\0';memset(b,0,sizeof(*b));
...
static int glm5_bpe_encode(const glm5_bpe *b,const char *text,int *out,int cap) {
    ...
    while(*p){
        const char *specials[]={"[gMASK]","<sop>","<|system|>","<|user|>","<|assistant|>","<|observation|>",NULL}; int sid[]={b->gmask,b->sop,b->system,b->user,b->assistant,b->observation};int found=-1;
        ...
        size_t m=0; while(p[m]&&m+1<sizeof(piece)&&!strchr("[]<>",p[m])){if(isspace((unsigned char)p[m])&&m&&p[m+1]&&!isspace((unsigned char)p[m+1])){m++;break;}m++;}
        if(!m){m=1;} if(m>=sizeof(piece))return -1;memcpy(piece,p,m);piece[m]='\0';p+=m;
        size_t u=0;for(size_t i=0;i<m;i++){unsigned char c=(unsigned char)piece[i];if(c>=33&&c<=126){bu[u++]=(char)c;}else{ /* GPT-2 byte-to-unicode fallback */ bu[u++]=(char)(0xc0|(c>>6));bu[u++]=(char)(0x80|(c&63));}}bu[u]='\0';
        char *parts[512];int np=0;for(size_t i=0;i<u&&np<512;){size_t l=(bu[i]&0x80)?2:1;parts[np]=glm5_bpe_strndup(bu+i,l);if(!parts[np++])return -1;i+=l;}if(np==512&&u>0)return -1;
