/*
 * glm5_mxfp8.h - MXFP8 (FP8 E4M3 + E8M0 per-[1,32]-block scale) matvec for A64FX.
 *
 * MiniMax-GLM5-MXFP8 stores each quantized weight as F8_E4M3 [rows,cols] (1 byte/elem) plus
 ...
 * the exact f32 bits from the LUT, then svmla. A scalar reference (glm5_*_ref) validates it.
 *
 * NOTE: the E8M0 bias/"scale_inv" sign convention is validated numerically against a Python
 * reference (a64fx/glm5/glm5_mxfp8_test.c) before wiring into the forward.
 */
#ifndef GLM5_MXFP8_H
#define GLM5_MXFP8_H
#include <stdint.h>
...
#define GLM5_MX_BLK 32   /* E8M0 scale block size (weight_block_size = [1,32]) */

/* FP8 E4M3 (fn: one NaN code, no inf) -> f32 bits. Exact. */
static inline uint32_t glm5_fp8_e4glm5_bits(uint8_t x){
...
static inline void glm5_init_fp8_lut(uint32_t *lut){ for(int i=0;i<256;i++) lut[i]=glm5_fp8_e4glm5_bits((uint8_t)i); }
/* E8M0 byte -> f32 power-of-2 scale = 2^(b-127). b=0 -> 0, b=255 -> NaN (OCP). */
static inline float glm5_e8m0(uint8_t b){ uint32_t bits=(uint32_t)b<<23; float f; memcpy(&f,&bits,4); return f; }

/* scalar reference: dst[8] = sum_c lut(w_j[c])*2^(s_j[c/32]-127) * x[c], for 8 rows. */
static inline void glm5_matvec_mxfp8_8row_ref(float*dst,
    ...
    for(int j=0;j<8;j++){ double a=0;
        for(int b=0;b<n;b+=GLM5_MX_BLK){ float sc=glm5_e8m0(s[j][b/GLM5_MX_BLK]); int e=b+GLM5_MX_BLK<n?b+GLM5_MX_BLK:n;
...
static inline void glm5_matvec_mxfp8_8row(float*dst,
        const uint8_t*w0,const uint8_t*w1,const uint8_t*w2,const uint8_t*w3,
        const uint8_t*w4,const uint8_t*w5,const uint8_t*w6,const uint8_t*w7,
        const uint8_t*s0,const uint8_t*s1,const uint8_t*s2,const uint8_t*s3,
        const uint8_t*s4,const uint8_t*s5,const uint8_t*s6,const uint8_t*s7,
    ...
    for(int b=0;b<n;b+=GLM5_MX_BLK){
        int bend=b+GLM5_MX_BLK<n?b+GLM5_MX_BLK:n; int blk=b/GLM5_MX_BLK;
        float c0=glm5_e8m0(s0[blk]),c1=glm5_e8m0(s1[blk]),c2=glm5_e8m0(s2[blk]),c3=glm5_e8m0(s3[blk]);
        float c4=glm5_e8m0(s4[blk]),c5=glm5_e8m0(s5[blk]),c6=glm5_e8m0(s6[blk]),c7=glm5_e8m0(s7[blk]);
...
static inline void glm5_mxfp8_decode_row_bf16(uint16_t*restrict dst, const uint8_t*w,
        const uint8_t*s, int col0, int kl, const uint32_t*lut){
    int vl=(int)svcntw();
    for(int b=0;b<kl;b+=GLM5_MX_BLK){
        int absc=col0+b; float sc=glm5_e8m0(s[absc/GLM5_MX_BLK]);
        int bend=b+GLM5_MX_BLK<kl?b+GLM5_MX_BLK:kl;
        for(int c=b;c<bend;c+=vl){
...
#else
#define glm5_matvec_mxfp8_8row glm5_matvec_mxfp8_8row_ref
/* scalar decode-to-bf16 fallback (exact: low 16 bits of f32 are zero for fp8*2^k) */
static inline void glm5_mxfp8_decode_row_bf16(uint16_t*restrict dst, const uint8_t*w,
        const uint8_t*s, int col0, int kl, const uint32_t*lut){
    for(int i=0;i<kl;i++){ int absc=col0+i; float sc=glm5_e8m0(s[absc/GLM5_MX_BLK]);
