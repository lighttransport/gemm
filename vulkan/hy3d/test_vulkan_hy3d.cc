/*
 * test_vulkan_hy3d.cc - Test harness for Vulkan Hunyuan3D-2.1 runner
 *
 * Usage:
 *   ./test_vulkan_hy3d <conditioner.safetensors> <model.safetensors> <vae.safetensors>
...
static uint8_t *read_ppm(const char *path, int *w, int *h) {
    ...
    ungetc(c, f);
    int maxval;
    if (fscanf(f, "%d %d %d", w, h, &maxval) != 3) { fclose(f); return nullptr; }
    fgetc(f);
    size_t sz = (size_t)(*w) * (*h) * 3;
    uint8_t *data = (uint8_t *)malloc(sz);
    if (fread(data, 1, sz, f) != sz) { free(data); fclose(f); return nullptr; }
    fclose(f);
    return data;
}
...
int main(int argc, char **argv) {
    if (argc < 4) {
        fprintf(stderr,
            "Hunyuan3D-2.1 Vulkan runner (SPIR-V compute shaders)\n"
            "\n"
    ...
    fprintf(stderr, "Initializing Vulkan Hunyuan3D runner...\n");
    vulkan_hy3d_runner *r = vulkan_hy3d_init(device_id, verbose);
    if (!r) { fprintf(stderr, "Failed to init Vulkan\n"); return 1; }

    fprintf(stderr, "Loading weights...\n");
    if (vulkan_hy3d_load_weights(r, cond_path, model_path, vae_path) != 0) {
    ...
    if (img_path) {
        ...
        hy3d_mesh mesh = vulkan_hy3d_predict(r, rgb, w, h, n_steps, guidance, grid_res, seed);
        free(rgb);
